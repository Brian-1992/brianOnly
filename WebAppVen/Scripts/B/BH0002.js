...
Ext.onReady(function () {
    ...
    var PoStore = viewModel.getStore('PO');
    var T1Store = viewModel.getStore('WB_MM_PO_M');
    ...
    function T2Load() {
        //T2Store.getProxy().setExtraParam("PO_NO", po);
        if (T1LastRec) {
                ...
                items: [
                    {
                        ...
                        items: [
                            ...
                            },
                            {
                                ...
                                width: 500,
                                autoEl: {
                                    html: '<marquee onmouseover="this.stop()" onmouseout="this.start()"><font size=4 color=blue > <b>輸入資料，<font color=red> 要按[確認回傳]</font>，三總才會收到資料!</b></font></marquee >'
                                }
                            }
                        ]
                    },
                    {
                        ...
                        autoEl: {
                            ...
                            html: '※ 請貴公司惠予於交貨前一天，回覆每筆訂單交貨訊息，以利入庫接收時，可迅速清點及接收，感謝貴公司配合。<font color=red>要按[確認回傳]</font>，三總才會收到資料!'
                        }
                    }
                ]
            }
    ...
    var T1Tool = Ext.create('Ext.PagingToolbar', {
        ...
        buttons: [
            {
                itemId: 'export', text: '匯出訂單資料', disabled: true, id:'export',
                handler: function () {
                    var p = new Array();
    ...
    var T1Grid = Ext.create('Ext.grid.Panel', {
        ...
        listeners: {
            itemclick: function (self, record, item, index, e, eOpts) {
                ...
                if (isCR == 'Y') {
                    Ext.getCmp('btnDeliveryAdd').disable();
                    Ext.getCmp('btnAdd').disable();
                    Ext.getCmp('btnDelete').disable();
                    Ext.getCmp('btnSend').disable();
        ...
        items: [
            ...
            //},
            {
                ...
                validator: function (value) {
                    //一般物品不用批號,效期
                    if (T1Form.getForm().findField('PO_NO').getValue().indexOf('GEN') < 0 && T1Form.getForm().findField('INQTY').getValue() != 0 && value != null && value == '') {
                        return '[批號]不可空白';
                    }
                    return true;
                }
                                            ...
                                            Ext.Ajax.request({
                                                ...
                                                success: function (response) {
                                                    ...
                                                    if (data.success) {
                                                        ...
                                                        debugger
                                                        if (msg[1] == "")
                                                            Ext.MessageBox.alert("提示", "匯入完成，[交貨批次]=" + msg[0]);
                                                        else
                                                            Ext.MessageBox.alert("提示", "[交貨批次] =" + msg[0] + "</br>匯入失敗資料:</br>" + msg[1]);
                                                        T2Query.getForm().findField("P2").setValue(msg[0]);
                                                        Ext.getCmp('btnSearch').setDisabled(false);
                                                        T2Grid.columns[1].setVisible(false);
                                                        T2Grid.columns[2].setVisible(false);
                                                        T2Grid.getSelectionModel().deselectAll(true);
    ...
    function setFormT1(x, t) {
        ...
        if (isCR == 'Y') {
            f.findField('DELI_DT').setReadOnly(true);
            f.findField('LOT_NO').setReadOnly(true);
            f.findField('EXP_DATE').setReadOnly(true);
            f.findField('INQTY').setReadOnly(true);
            f.findField('MEMO').setReadOnly(true);
        }
    ...
    function showReport(IsList) {
        if (!win) {
            ...
            var winform = Ext.create('Ext.form.Panel', {
                ...
                layout: 'fit',
                closable: false,
                html: '<iframe src="' + reportUrl + '?p1=' + np.p1 + '&p2=' + np.p2 + '&p3=' + np.p3 + '" id="mainContent" name="mainContent" width="100%" height="100%" frameborder="0" style="background-color:#FFFFFF"></iframe>',
                buttons: [{
                    text: '關閉',
                    margin: '0 20 30 0',
                    handler: function () {
