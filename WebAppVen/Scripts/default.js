...
Ext.onReady(function () {
    ...
    var UrlHome = '../Form/Index/UR1013';
    var GetDbConnType = '../api/Acct/getDbConnType';
    var UrlLogo2 = '../Images/TSGH_logo_W.png';//正式環境
    var Title = '廠商作業管理系統';
    var IsPageLoad = true;

    var DefaultReqVal_g = "GET";
    var DefaultReqVal_p = "POST";
    ...
    var MsgForm = Ext.widget({
        ...
        items: [{
            ...
            items: [{
                ...
                xtype: 'label',
                itemId: 'msglabelIsTest',
                id: 'msglabelIsTest',
                hidden: true,
                width: 300,
                html: '<b><font color="red" size="4">這裡是測試區 這裡是測試區</font></b>'
            }]
        }]
    });
        ...
        items: [{
            ...
            items: [{
                ...
                items: [{
                    ...
                    padding: '8 0 0 0',
                    hidden: true,
                    html: '<b><font color="red" style="background-color:#9CF792;" size="4">這裡是廠商作業管理系統測試區</font></b>'
                }, {
                    id: 'n3',
                    itemId: 'nBarN3',
                    height: 24,
                    html: '<table border="0" style="nowrap:nowrap;height: 100%;color:#174F89">\
                                ...
                                <td id="flow2" style="width:200px;" nowrap></td>\
        ...
        listeners: {
            load: function (sender, records, successful, eOpts) {
                ...
                if (successful) {
                    ...
                    $('#userName')[0].innerHTML = userId+'-'+userName;
    ...
    // 若為測試機,顯示提示訊息及背景色
    function setDbConnType() {
        Ext.Ajax.request({
            url: GetDbConnType,
            method: DefaultReqVal_p,
            success: function (response) {
                var data = Ext.decode(response.responseText);
                if (data.success) {
                    if (data.msg == 'TEST') {
                        MsgForm.down('#msglabelIsTest').setVisible(true);
                        viewport.down('#nBarIsTest').setVisible(true);

                        var bgColor = '#9CF792';
                        menu.setBodyStyle('{background-color:' + bgColor + ';}');
                        MenuTree.setBodyStyle('{background-color:' + bgColor + ';}');
                        viewport.down('#nBarSub').setBodyStyle('{background-color:' + bgColor + ';}');
                        viewport.down('#nBarN1').setHtml('<image height="43px" width="129px" id="logoImage" style="background-color:' + bgColor + ';" src="' + UrlLogo2 + '" />');
                        viewport.down('#nBarN2').setBodyStyle('{background-color:' + bgColor + ';}');
                        viewport.down('#nBarN2').setHtml('<table border="0" style="background-color:' + bgColor + ';nowrap:nowrap;height: 100%;font-size:14px;color:#FF0000"><tr><td>' +
                            '</td></tr></table>');
                        viewport.down('#nBarIsTest').setHtml('<b><font color="red" style="background-color:' + bgColor + ';" size="4">這裡是廠商作業管理系統測試區</font></b>');

                        // 若為測試區則登入者名稱也填入背景色,但重新指定Html會有偶發登出鈕無作用的問題,先取消填背景色的動作
                        //// 若已取得使用者名稱,直接填入html內
                        //if (userName == '')
                        //{
                        //    viewport.down('#nBarN3').setHtml('<table border="0" style="background-color:' + bgColor + ';nowrap:nowrap;height: 100%;color:#174F89">\
                        //        <tr>\
                        //        <td id="flow" style="width:120px;" nowrap></td>\
                        //        <td id="flow2" style="width:200px;" nowrap></td>\
                        //        <td id="status" nowrap>讀取中...</td>\
                        ...
                        //        <td id="status" nowrap>讀取中...</td>\
                        //        <td nowrap>您以&nbsp;</td>\
                        //        <td id="userName" style="text-decoration:underline" nowrap>' + userId +'-'+ userName + '</td>\
                        //        <td nowrap>&nbsp;的身份登入|</td>\
                        //        <td nowrap><a href="#" id="loginStatus"></a>登出</td>\
                        //        </tr></table>');
                        //}
                        tabs.down('#tabMain').setBodyStyle('{background-color:' + bgColor + ';}');
                    }
                    else {
                        MsgForm.down('#msglabelIsTest').setVisible(false);
                        viewport.down('#nBarIsTest').setVisible(false);
                    }
                }
            },
            failure: function (response, options) {
