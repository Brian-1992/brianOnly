...
Ext.onReady(function () {
    ...
    {
        var r = T2Store.getAt(rowIndex);
        if (checked)
        {
            //全選
            if (!r.get('V')) r.set('V', 1);
            if (!r.get('U')) r.set('U', 1);
            if (!r.get('P')) r.set('P', 1);
            if (!r.get('R')) r.set('R', 1);
        }
        else
        {
            //取消全選
            if (r.get('V')) r.set('V', 0);
            if (r.get('U')) r.set('U', 0);
            if (r.get('P')) r.set('P', 0);
            if (r.get('R')) r.set('R', 0);
        }
    }

    var checkAll = function (column, recordIndex, checked, record) {
        if (record.data.leaf && checked) {
            record.bubble(function (n) {
                if(n.get('FG'))
                    n.set(column.dataIndex, checked);
            });
        } else {
    ...
    function T2Submit() {
        ...
        //var t2 = [];
        /*
        T2Tree.getRootNode().cascadeBy(function (n) {
            var fg = n.get("FG");
            var g = s = v = 0;
            if (fg.length > 0) {
                var eg = document.getElementById(fg + 'G');
                var es = document.getElementById(fg + 'S');
                var ev = document.getElementById(fg + 'V');
                if (eg != null) { g = eg.checked === true ? 1 : 0; }
                if (es != null) { s = es.checked === true ? 1 : 0; }
                if (ev != null) { v = ev.checked === true ? 1 : 0; }
                if (g === 1 || s === 1 || v === 1) {
                    t1.push({ "RLNO": T1RLNO, "FG": fg, "G": g, "S": s, "V": v });
                }
                //t2.push({ "RLNO": T1RLNO, "FG": fg, "G": g, "S": s, "V": v });
            }
        });
        */
        var activeTree = T2Tab.activeTab.tree;
        ...
        f.findField('SG').setValue(activeStore.SG);
        f.submit({
            url: T2Set,
