...
Ext.onReady(function () {
    var T1Get = '../../../api/File/GetByKey';
    var Ext1Get = '../../../api/File/GetEC';
    ...
    var lk = (Ext.getUrlParam('LK') == '1') ? true : false;
    ...
    var T1Store = Ext.create('Ext.data.Store', {
        ...
        listeners: {
            beforeload: function (store, options) {
                ...
                for (i = 0; i < len; i++)
                    files.push(store.data.get('FN'));
                parent.$('#args')[0].files = files;
                if (isFocus) {
                ...
                if (lk && !isLocked)
                {
                    var len = store.data.length;
                    for (i = 0; i < len; i++)
                        lk_arr.push(store.data.get(i).get('FG'));
                    isLocked = true;
                }
            }
        },
    ...
    var T1Tool = Ext.create('Ext.PagingToolbar', {
        ...
        buttons: [
            {
                cls: 'funBtn',
                itemId: 'T1Browse', text: '瀏覽檔案...', hidden: !ua, handler: function () {
                    $('#file')[0].click();
                }
            },
            {
    ...
    var T1Grid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{ xtype: 'rownumberer' },
        {
            ...
            renderer: function (val, meta, record) {
                return '<a href=javascript:DownloadFile("' + record.get('FG') + '");>' + val + '<img src="../../../Images/TRA/save.gif" align="absmiddle" /></a>';
    ...
    if (!ua) {
        T1Grid.on('beforeedit', function (editor, e) {
            if (e.colIdx == 2)
                return false;
        });
    }
    else if (lk)
    {
        T1Grid.on('beforeedit', function (editor, e) {
            if (e.colIdx == 2)
                if (lk_arr.indexOf(e.record.get('FG')) > -1) {
                    var w = Ext.Msg.show({
                        title: '欄位修改提示',
                        msg: e.record.get('FN') + ' <span style=\'color:red\'>檔案說明</span>已鎖定<br/>請重新上傳檔案，並輸入新的檔案說明。',
                        buttons: Ext.Msg.OK,
    ...
    parent.$('#CheckDesc')[0].callback = function ()
    {
        var isSomeFDempty = false;
        var st = T1Grid.getStore().data;
        var len = st.length;
        for (i = 0; i < len; i++)
            if (st.items[i].data['FD'] == "")
                isSomeFDempty = true;

        return isSomeFDempty;
    }
});