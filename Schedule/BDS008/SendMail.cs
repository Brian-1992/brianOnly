...
namespace Tsghmm.Utility
{
    class SendMail
    {
        ...
        public bool Send_Mail(string seq, string po_no, string hospName, string fromName, string fromMail, string toMail, string toMailCC, string mail_type, string toSubject, string toContent, string create_user, string toContentLog, List<string> attachmentNames)
        {
            bool rtn = true;
                ...
                try
                {
                    ...
                    if (serverName.Contains("AIDC")) //漢翔主機
                    {
                        ...
                        using (MailMessage mail = new MailMessage())
                        {
                            ...
                            mail.From = new MailAddress(emailFrom, hospName + ' ' + fromName);
                            mail.To.Add(toMail);
                            if (toMailCC.Trim() != "")
                    ...
                    else //三總主機
                    {
                        if (serverName.Contains("TEST")) //測試主機
                        {
                            ...
                            // 若要透過GMAIL當作mail server, smtp伺服器設為smtp.gmail.com, port可使用587, 指定EnableSsl = true
                            // 登入的gmail帳號(userName)需先啟用兩步驗證碼,再設定一組應用程式密碼當作password
                            if (smtpAddress == "smtp.gmail.com")
                                portNumber = 587;
                            string userName = config.AppSettings.Settings["userName"].Value.ToString(); //mail server 帳號
                            string password = config.AppSettings.Settings["password"].Value.ToString(); //mail server 密碼
                    ...
                    //增加處理mail_body不超過3800字
                    string truncatedContentLog = toContentLog.Length > 3800 ? toContentLog.Substring(0, 3800) : toContentLog;
                    //寫 log
                    MailLog(fromMail, toMail, toMailCC, toSubject, mail_type, truncatedContentLog, create_user);
                    UpdateSendMailLog(seq);
                }
                catch (Exception ex)
                {
                    rtn = false;
        ...
        public void UpdateSendMailLog(string seq)
        {
            try
            {
                CallDBtools_Oracle callDbtools_oralce = new CallDBtools_Oracle();
                string dbmsg = "error";
                int rowsAffected = -1;
                string cmdstrI = " update send_mail_log set is_send='Y', send_time=sysdate, update_time = sysdate, update_user='BATCH' where seq = :seq ";

                List<CallDBtools_Oracle.OracleParam> paraListI = new List<CallDBtools_Oracle.OracleParam>();
                paraListI.Add(new CallDBtools_Oracle.OracleParam(1, ":seq", "VARCHAR2", seq));
