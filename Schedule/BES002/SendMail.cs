...
namespace BES002
{
    ...
    {
        L l = new L("BES002");
        // Configuration config = ConfigurationManager.OpenExeConfiguration(Directory.GetCurrentDirectory() + "\\Schedule.config");

        //fromMail-寄件人；toMail-收件人；toMailCC-收件人副本；mail_type-信件類別；toSubject-主旨；toContent-信件內容；create_user-建立人員
        public bool Send_Mail(string crdocno, string fromMail, string toMail, string toMailCC, string mail_type, string toSubject, string toContent, string create_user, string toContentLog, List<string> attachmentNames)
        {
            //l.lg("getConfig", config.FilePath);
            ...
            if (sendMailFlag == "Y")
            {
                try
                {
                    ...
                    SaveMailLog(crdocno, toMail, toSubject, toContent);
                }
                catch (Exception ex)
                {
                    rtn = false;
                    l.le("BES002", "BES002-寄信失敗失敗:" + ex.Message);
                    SaveMailLog("失敗" + crdocno, toMail, toSubject, toContent);
        ...
        //以html檔保留mail資料
        public void SaveMailLog(string crdocno, string toMail, string toSubject, string toContent)
        {
            ExeConfigurationFileMap configMap = new ExeConfigurationFileMap();
            configMap.ExeConfigFilename = AppDomain.CurrentDomain.BaseDirectory + "\\Schedule.config.config";
            Configuration config = ConfigurationManager.OpenMappedExeConfiguration(configMap, ConfigurationUserLevel.None);

            string filePath = config.AppSettings.Settings["TsghmmLog"].Value.ToString() + "\\Schedule\\CR_DOC\\" + DateTime.Now.ToString("yyyyMMdd");
            string fileName = config.AppSettings.Settings["TsghmmLog"].Value.ToString() + "\\Schedule\\CR_DOC\\" + DateTime.Now.ToString("yyyyMMdd") + "\\" + crdocno + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".html";
