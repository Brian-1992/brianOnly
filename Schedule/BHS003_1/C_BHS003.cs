    ...
    class C_BHS003
    {
        ...
        private void Transaction01()
        {
            ...
            try
            {
                ...
                CallDBtools_Oracle callDbtools_oralce = new CallDBtools_Oracle();
                String s_conn_oracle = calldbtools.SelectDB("oracle");
                Console.WriteLine("連線oracle");
                OracleConnection conn_oracle = new OracleConnection(s_conn_oracle);
                if (conn_oracle.State == ConnectionState.Open) {
                    conn_oracle.Close();
                    Console.WriteLine("關閉現有oracle連線");
                }
                conn_oracle.Open();
                Console.WriteLine("連線oracle 成功");
                OracleTransaction transaction_oracle = conn_oracle.BeginTransaction();
                List<string> transcmd_oracle = new List<string>();
                List<CallDBtools_Oracle.OracleParam> listParam_oracle = new List<CallDBtools_Oracle.OracleParam>();
                int iTransOra = 0;
                ...
                CallDBtools_MSDb callDbtools_msdb = new CallDBtools_MSDb();
                String s_conn_msdb = calldbtools.SelectDB("msdb");
                Console.WriteLine("連線msdb");
                SqlConnection conn_msdb = new SqlConnection(s_conn_msdb);
                if (conn_msdb.State == ConnectionState.Open) {
                    conn_msdb.Close();
                    Console.WriteLine("關閉現有msdb連線");
                }
                    
                conn_msdb.Open();
                Console.WriteLine("連線msdb 成功");
                SqlTransaction transaction_msdb = conn_msdb.BeginTransaction();
                List<string> transcmd_msdb = new List<string>();
                List<CallDBtools_MSDb.MSDbParam> listParam_msdb = new List<CallDBtools_MSDb.MSDbParam>();
                int iTransMsdb = 0;
                try
                {
                    ...
                    dt_MSDB = callDBtools_msdb.CallOpenSQLReturnDT(sqlStr_MSDB, null, null, "msdb", "T1", ref msg_MSDB);
                    Console.WriteLine(string.Format("msg_MSDB: {0}", msg_MSDB));
                    //l.lg("Transaction01()", "sql=" + sqlStr_MSDB);
                    if (msg_MSDB == "") //取得 WB_PUT_D 無錯誤
                    {
                        Console.WriteLine(string.Format("dt_MSDB.Rows.Count: {0}", dt_MSDB.Rows.Count));
                        ...
                        if (dt_MSDB.Rows.Count > 0) //WB_PUT_D 有資料
                        {
                            ...
                            {
                                Console.WriteLine(string.Format(@"AGEN_NO: {0}, MMCODE: {1}, PO_NO: {2}, SEQ: {3}, FLAG: {4}, INQTY: {5}, INVOICE: {6}, LOT_NO: {7}, STATUS: {8}  "
                                    , dt_MSDB.Rows[i]["AGEN_NO"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["MMCODE"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["PO_NO"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["SEQ"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["FLAG"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["INQTY"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["INVOICE"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["LOT_NO"].ToString().Trim()
                                    , dt_MSDB.Rows[i]["STATUS"].ToString().Trim()
                                                    ));
                            ...
                            //        transaction_oracle,
                            //        conn_oracle);
                            //Console.WriteLine(string.Format("rowsAffected_oracle: {0}", rowsAffected_oracle));
                            ...
                            //                transaction_msdb,
                            //                conn_msdb);
                            //Console.WriteLine(string.Format("rowsAffected_msdb: {0}", rowsAffected_msdb));
                            ////l.lg("Transaction01()", "rowsAffected_msdb=" + rowsAffected_msdb);

                            Console.WriteLine(string.Format("commit"));
                            transaction_oracle.Commit();
                            conn_oracle.Close();
                            Console.WriteLine(string.Format("oracle close"));
                            //l.lg("Transaction01()", "transaction_oracle.Commit()");

                            transaction_msdb.Commit();
                            conn_msdb.Close();
                            Console.WriteLine(string.Format("msdb close"));
