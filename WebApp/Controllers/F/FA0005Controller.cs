...
{
    public class FA0005Controller : SiteBase.BaseApiController
    {
        public ApiResponse All(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new FA0005Repository(DBWork);
                    //session.Result.etts = repo.GetAll(chk_ym, storeId, matClass);

                    string hospCode = repo.GetHospCode();
                    IEnumerable<FA0005M> list = repo.GetAll(chk_ym, storeId, matClass, hospCode=="0");
                    list = GetGroup(list);
        ...
        public IEnumerable<FA0005M> GetGroup(IEnumerable<FA0005M> list)
        {
        ...
        public IEnumerable<FA0005M> GetCompleteData(IEnumerable<FA0005M> list, bool isExcel)
        {
            string lineBreak = isExcel ? "; " : "<br>";
            foreach (FA0005M item in list)
            {
                FA0005M temp = item.items.Select(x => x).First();
                ...
                foreach (FA0005M innerItem in item.items)
                {
                ...
                item.CHK_QTY = chk_qty.ToString();
                item.CHK_COST = (chk_qty * float.Parse(item.M_CONTPRICE)).ToString();
                item.STORE_COST = (float.Parse(item.STORE_QTY) * float.Parse(item.M_CONTPRICE)).ToString();
        ...
        public ApiResponse Excel(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    //session.Result.etts = repo.GetAll(chk_ym, storeId, matClass);
                    string hospCode = repo.GetHospCode();
                    IEnumerable<FA0005M> list = repo.GetExcel(chk_ym, storeId, matClass, hospCode=="0");
                    list = GetGroup(list);
        ...
        public DataTable GetDataTable(IEnumerable<FA0005M> list)
        {
            var props = typeof(FA0005M).GetProperties();
            var dt = new DataTable();

            List<string> columnList = new List<string>() {
                "MMCODE","MMNAME_C","MMNAME_E","STORE_QTY", "STORE_COST","CHK_QTY","CHK_COST","DIFF_COST","COMMENT"
            };
            List<string> columnListName = new List<string>() {
                "院內碼","中文品名","英文品名","庫存數量","庫存成本","盤點量", "盤點成本","差異成本","差異分析說明"
            };
