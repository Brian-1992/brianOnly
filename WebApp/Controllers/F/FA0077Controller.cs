...
{
    public class FA0077Controller : SiteBase.BaseApiController
    {
        #region temp class
        ...
        public ApiResponse GetPreym()
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new FA0077Repository(DBWork);
        ...
        public ApiResponse Columns(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    FA0077DataYmItem ymItem = repo.GetDataYmItem();

                    IEnumerable<FA0077> items = repo.GetPastData(ymItem.PreMinDataym, ymItem.PreMaxDataym, grp_no, inid, rate);
        ...
        public ApiResponse Data(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    IEnumerable<FA0077> items = repo.GetPastData(ymItem.PreMinDataym, ymItem.PreMaxDataym, grp_no, inid, rate);
                    IEnumerable<MM_ABS_FEE> CurrentFees = repo.GetData(ymItem.MinDataym, ymItem.MaxDataym, grp_no, inid);
                    items = GetFa0077(items, CurrentFees, ymItem, data_ym);
                    // Transform
        ...
        public IEnumerable<FA0077> GetFa0077(IEnumerable<FA0077> items, IEnumerable<MM_ABS_FEE> fees, FA0077DataYmItem ymItem, string data_ym)
        {

            IEnumerable<string> currentYms = GetCurrentYms(fees);

            foreach (FA0077 item in items)
            {
        ...
        public IEnumerable<FA0077Item> GetFees(IEnumerable<MM_ABS_FEE> fees, string inid, string target, string avg)
        {
            var o = from a in fees
                    where a.INID == inid
                    orderby a.DATA_YM ascending
                    select new FA0077Item()
                    {
                        ABS_YM = a.DATA_YM,
            ...
            List<FA0077Item> temps = o.ToList<FA0077Item>();
            return temps;
        }
        ...
        public IEnumerable<FA0077ColumnItem> GetColumns(FA0077DataYmItem ymItem, IEnumerable<string> currentYms)
        {
            List<FA0077ColumnItem> columns = new List<FA0077ColumnItem>();
        ...
        public FA0077ColumnItem getColumnItem(string text, string dataIndex)
        {
            return new FA0077ColumnItem()
            {
                TEXT = text,
                DATAINDEX = dataIndex
            };
        }
        public IEnumerable<FA0077> Transform(IEnumerable<FA0077> items)
        {
            int i = 0;
            foreach (FA0077 item in items)
            {
                i = 0;
                foreach (FA0077Item subItem in item.ITEMS)
                {
                    i++;
        ...
        public ApiResponse T1Excel(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    FA0077Repository repo = new FA0077Repository(DBWork);
        ...
        public ApiResponse GetT3Columns(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    FA0077DataYmItem ymItem = repo.GetT3DataYmItem(year, q);
                    ...
                    IEnumerable<FA0077ColumnItem> columns = GetT3Columns(ymItem, currentYms, "C");
                    IEnumerable<FA0077ColumnItem> pyColumns = GetT3Columns(ymItem, pyYms, "PRE_Y");
                    IEnumerable<FA0077ColumnItem> psColumns = GetT3Columns(ymItem, psYms, "PRE_S");

                    List<FA0077ColumnItem> list = new List<FA0077ColumnItem>();
        ...
        public IEnumerable<FA0077ColumnItem> GetT3Columns(FA0077DataYmItem ymItem, IEnumerable<string> currentYms, string ymNameCompare)
        {
            List<FA0077ColumnItem> columns = new List<FA0077ColumnItem>();
        ...
        public ApiResponse GetT3Data(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    FA0077DataYmItem ymItem = repo.GetT3DataYmItem(year, q);
                    List<FA0077> list = new List<FA0077>();
                    // C
                    IEnumerable<FA0077> items = repo.GetT3PastData(ymItem.C_MIN_QYM, ymItem.C_MAX_QYM, grp_no, inid, true);
                    IEnumerable<MM_ABS_FEE> CurrentFees = repo.GetData(ymItem.C_MIN_QYM, ymItem.C_MAX_QYM, grp_no, inid);
                    list.AddRange(GetT3Fa0077(items, CurrentFees, ymItem, "C", list));
                    ...
                    list = GetT3Fa0077(items, CurrentFees, ymItem, "PRE_Y", list).ToList();
                    // PRE_S
                    ...
                    list = GetT3Fa0077(items, CurrentFees, ymItem, "PRE_S", list).ToList();
        ...
        public IEnumerable<FA0077> GetT3Fa0077(IEnumerable<FA0077> items, IEnumerable<MM_ABS_FEE> fees, FA0077DataYmItem ymItem, string columnPrefix, List<FA0077> targetList)
        {

            IEnumerable<string> currentYms = GetCurrentYms(fees);
            List<FA0077> list = new List<FA0077>();
            FA0077 newItem = new FA0077();
            ...
            foreach (FA0077 item in items)
            {
                ...
                newItem = GetObject<FA0077>(dictionary);

                bool isTempNull = false;
                if (columnPrefix != "C")
                {
                    FA0077 temp = targetList.Where(x => x.INID == newItem.INID).Select(x => x).FirstOrDefault<FA0077>();
        ...
        public ApiResponse T3Excel(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    IEnumerable<FA0077ColumnItem> columns = GetT3ExcelColumns(DBWork, year, q, grp_no, inid);

                    IEnumerable<FA0077> datas = GetT3ExcelData(DBWork, year, q, grp_no, inid);

                    DataTable data_dataTable = ConvertToDataTable<FA0077>(datas.ToList());
                    ...
                    foreach (FA0077ColumnItem column in columns)
                    {
        ...
        public IEnumerable<FA0077ColumnItem> GetT3ExcelColumns(UnitOfWork DBWork, string year, string q, string grp_no, string inid)
        {
            try
            {
                var repo = new FA0077Repository(DBWork);
        ...
        public IEnumerable<FA0077> GetT3ExcelData(UnitOfWork DBWork, string year, string q, string grp_no, string inid)
        {
            try
            {
                var repo = new FA0077Repository(DBWork);
                ...
                List<FA0077> list = new List<FA0077>();
                // C
                IEnumerable<FA0077> items = repo.GetT3PastData(ymItem.C_MIN_QYM, ymItem.C_MAX_QYM, grp_no, inid, false);
                IEnumerable<MM_ABS_FEE> CurrentFees = repo.GetData(ymItem.C_MIN_QYM, ymItem.C_MAX_QYM, grp_no, inid);
                list.AddRange(GetT3Fa0077(items, CurrentFees, ymItem, "C", list));
