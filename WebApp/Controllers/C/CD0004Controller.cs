...
{
    public class CD0004Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        public ApiResponse All(FormDataCollection form)
        {
            ...
            var p7 = form.Get("p7"); // false則只回傳查詢結果的第一筆,true則回傳全部; 設定只回傳一筆資料以增進執行效率
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new CD0004Repository(DBWork);
                    ...
                    session.Result.afrs = repo.insertNewWhpick(p0, p6); // 若AA0015有取消揀貨,再於PDA揀貨時,需先恢復已被刪除的資料
                    session.Result.etts = repo.GetAll(p0, p2, p3, p4, getMmcode, p6, Convert.ToBoolean(p7), page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse PickAll(FormDataCollection form)
        {
            ...
            var p2 = form.Get("p2");
            var p3 = form.Get("p3");
            var p4 = form.Get("p4") == null ? string.Empty : form.Get("p4");
            var page = int.Parse(form.Get("page"));
            var start = int.Parse(form.Get("start"));
            var limit = int.Parse(form.Get("limit"));
            var sorters = form.Get("sort");
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    var repo = new CD0004Repository(DBWork);
                    session.Result.etts = repo.GetPickAll(p0, p1, p2, p3,p4, page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse SetActPickQty(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new CD0004Repository(DBWork);
                    //// 檢查BC_WHPICK是否存在，不存在為取消揀貨，需重新進入功能載入資料
                    //if (repo.CheckBcWhpickExists(docno, mmcode) == false) {
                    //    session.Result.success = false;
                    //    session.Result.msg = "已取消揀貨，請重新進入功能以載入此資料";
                    //    return session.Result;
                    //}
                    // 檢查ME_DOCD狀態，3C4D不可核撥
                    if (repo.CheckDocdPostID(docno, mmcode) == false) {
                        // 更新BC_WHPICK資料
                        session.Result.afrs = repo.UpdateBcWhpickActPickInfo(docno, mmcode);

                        DBWork.Commit();
                        session.Result.success = false;
                        session.Result.msg = "已核撥，請輸入其他院內碼";
                        return session.Result;
                    }
                    // 檢查BC_WHPICKDOC是否存在，不存在新增
                    if (repo.CheckBcWhpickDocExistsByDocno(wh_no, docno) == false) {
                        repo.insertNewWhpickDocByPc(docno);
                    }
                    // 檢查BC_WHPICK是否存在，不存在新增
                    if (repo.CheckBcWhpickExists(docno, mmcode) == false)
                    {
                        repo.InsertWhpickDocByDocnoMmcode(wh_no, docno, mmcode);
                    }
                    ...
                    // 若品項有做取消揀貨,會有已分配揀貨批次但在BC_WHPICK存在沒有PICK_USERID的資料,不論是否已分配批次都嘗試更新PICK_USERID
                    ...
                    if (wexp_id == "Y")
                    {
                        ...
                        {
                            // 判斷核撥量是否等於各批號效期數量和
                            double validQtySum = 0;
                            foreach (string strData in submitT)
                            {
                                string act_pick_qty_valid = strData.Split('^')[2];
                                validQtySum += Convert.ToDouble(act_pick_qty_valid);
                            }
                            if (Convert.ToDouble(act_pick_qty) != validQtySum)
                            {
                                chkWexp = "N";
                                session.Result.msg = "各效期數量合計與撥發量不符";
                            }
                    ...
                    if (chkWexp == "Y")
                    {
                        ...
                        repo.deleteDocexp(docno, seq);
                        repo.insertDocexp(wh_no, pick_date, docno, seq, mmcode, User.Identity.Name, DBWork.ProcIP); // 新增效期資料到點收品項效期資料中
        ...
        public ApiResponse SetPickAll(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new CD0004Repository(DBWork);
                    session.Result.afrs = repo.updatePickAll(p0, p2, p3, p4, p5, p6);
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    ...
                    if (repo.ChkMmcode(p0, p2, p3, p4, p5, p6) > 0) // 有找到院內碼
                        session.Result.msg = "1";
                    else
                    {
                        // 找不到院內碼,則改找是否有此單號
                        if (repo.ChkDocClose(p3, p5) > 0) // 先看是否已結案
                        {
                            // 若單據已結案
                            session.Result.etts = repo.GetPick(p5);
                            session.Result.msg = "C";
                        }
                        else if (repo.ChkDocno(p3, p5) > 0) // 尚未結案(只看單號,以及揀貨人員是否為自己或未分配)
                        {
                            session.Result.etts = repo.GetPick(p5);
        ...
        public ApiResponse chkPickCnt(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    var repo = new CD0004Repository(DBWork);
                    if (repo.chkPickCnt(p0, p2, p6) > 0)
                        session.Result.msg = "T"; // 申請單所有品項均揀貨完成但尚未出貨
                    else
        ...
        public ApiResponse SetShipoutAll(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    var repo = new CD0004Repository(DBWork);
                    session.Result.afrs += repo.updateShipoutAll(p0, p2, p6);
                    session.Result.afrs += repo.insertWhpickShipoutAll(p0, p2, p6);
                    session.Result.afrs += repo.deleteDocexpAll(p6);
                    session.Result.afrs += repo.insertDocexpAll(p0, p2, p6);
                    session.Result.afrs += repo.UpdateMeDocdAll(p6);
                    session.Result.afrs += repo.UpdateMeDocmShipout(p6);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetWH_NoCombo(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    CD0004Repository repo = new CD0004Repository(DBWork);
        ...
        public ApiResponse GetPickUserCombo(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    string whno_me1 = repo.GetWhnoMe1();
        ...
        [HttpPost]
        public ApiResponse GetDocnoInfo(FormDataCollection form) {
            var wh_no = form.Get("wh_no");
            var pick_date = form.Get("pick_date");
            var pick_user = form.Get("pick_user");
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    CD0004Repository repo = new CD0004Repository(DBWork);
                    session.Result.etts = repo.GetDocnoInfo(wh_no,  docno);
                }
                catch
                {
                    throw;
