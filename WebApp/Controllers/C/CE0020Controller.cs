...
namespace WebApp.Controllers.C
{
    ...
    public class CE0020Controller : SiteBase.BaseApiController
    {
        public ApiResponse MasterAll(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new CE0020Repository(DBWork);
        ...
        public ApiResponse ModifiedSheet(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    repo.Update_CHK_G2_DETAIL_Status(CHK_NO);
                    repo.Update_CHK_MAST_Status(CHK_NO);
                    session.Result.afrs = repo.Insert_CHK_G2_DETAILTOT(CHK_NO, UserName, UserIP, start_date, end_date);
        ...
        [HttpPost]
        public ApiResponse Random(FormDataCollection form) {
            var chk_no = form.Get("chk_no");
            ...
            using (WorkSession session = new WorkSession(this)) {
                ...
                try
                {
                    ...
                    IEnumerable<CHK_G2_WHINV> invs = repo.GetAllG2Whinv(chk_no);

                    Random random = new Random();

                    foreach (CHK_G2_WHINV inv in invs) {
                        inv.STORE_QTY = random.Next(1, 1000).ToString();

                        session.Result.afrs = repo.UpdateStoreQtycRandom(inv);
                    }
        ...
        [HttpPost]
        public ApiResponse IsStoreqtyEmpty(FormDataCollection form) {
            ...
            using (WorkSession session = new WorkSession(this)) {
                ...
                try
                {
                    IEnumerable<CHK_G2_WHINV> list = repo.GetAllG2Whinv(chk_no);
                    if (list.Any()) {
