...
{
    public class CE0005Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        ...
        public ApiResponse InsertMaster(CHK_MAST masterInput)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    foreach (CHK_MAST master in masters)
                    {
                        string pre_chk_no = master.CHK_NO;
                        ...
                        session.Result.afrs = repo.InsertChkMast(master);
                        var preChkno = repo.GetPreChkno(master.CHK_NO1, "1");
                        session.Result.afrs = repo.UpdateChkno1(preChkno, master.UPDATE_USER, master.UPDATE_IP);


                        #region 中央庫房產生複盤單自動將數量不符之品項加入盤點 2020-01-30新增
                        // 若為中央庫房 wh_grade ='1' & wh_kind = '1'
                        if (master.CHK_WH_GRADE == "1" && master.CHK_WH_KIND == "1") {
                            IEnumerable<CHK_DETAILTOT> tots = repo.GetNotMatchDetails(pre_chk_no);
        ...
        public ApiResponse GetDetailInclude(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new CE0005Repository(DBWork);
                    var repo1 = new CE0002Repository(DBWork);
                    ...
                    if (windowNewOpen == "Y")
                    {
                        repo1.DeleteDetailTempAll(chk_no);
                        ...
                        foreach (CHK_DETAIL include in includes)
                        {
                            ...
                            temp.CHK_NO = chk_no;
                            session.Result.afrs = repo1.InsertDetailTemp(temp);
                        }
                    }

                    session.Result.etts = repo1.DetailTempAll(chk_no, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetDetailExclude(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    ...
                    //    if (exclude.CHK_QTY != "0") {
                    //        exclude.DIFF_P = Math.Round(exclude.QTY_DIFF / float.Parse(exclude.CHK_QTY), 5);
                    //    }
                    //}
        ...
        public ApiResponse DetailSave(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    string wh_no = repoCE0005.GetChkWhNo(chk_no);
                    CHK_MAST mast = repoCE0005.GetChkMast(chk_no);
                    ...
                    if (mast.CHK_WH_KIND == "E" || mast.CHK_WH_KIND == "C")
                    {
                        foreach (CHK_DETAIL_TEMP temp in temps)
                        {
                            ...
                            session.Result.afrs = repoCE0005.InsertChkdetailCE(detail);
                        }
                    }
                    else
                    if (mast.CHK_WH_GRADE == "1" && mast.CHK_WH_KIND != "E" && mast.CHK_WH_KIND != "C")
                    {
                        if (wh_no == "PH1S")
                        {
                            foreach (CHK_DETAIL_TEMP temp in temps)
                            {
                                ...
                                session.Result.afrs = repoCE0005.InsertChkdetailPH1S(detail, false);
                            }
                        }
                        else {
                            foreach (CHK_DETAIL_TEMP temp in temps)
                    ...
                    else
                    {
                        foreach (CHK_DETAIL_TEMP temp in temps)
                        {
                            ...
                            if (repoCE0005.CheckDetailExists(mast.CHK_NO1, detail.MMCODE) == false)
                            {
                                session.Result.afrs = repoCE0005.InsertChkDetailFromTemp(detail);
                            }
                            else {
                                session.Result.afrs = repoCE0005.InsertChkdetail(detail, mast.CHK_WH_GRADE);
                            }
                ...
                try
                {
                        ...
                        if (chk_wh_kind == "0")
                        {
                            if (rechoose)
                            {
                                ...
                                foreach (CHK_DETAIL  detail in details)
                                {
                                    ...
                                    session.Result.afrs = repoCE0005.InsertChkdetailPH1S(detail, true);
                                }
                            }
                            else {
                                foreach (CHK_DETAIL_TEMP temp in temps)
                            ...
                            session.Result.afrs = repoCE0005.UpdateChkPH1SWhinv(chk_no, DBWork.UserInfo.UserId, DBWork.ProcIP);
                        }
                        else {
                            foreach (CHK_DETAIL_TEMP temp in temps)
                            {
                    ...
                    else
                    {
                        foreach (CHK_DETAIL_TEMP temp in temps)
                        {
                            ...
                            session.Result.afrs = repoCE0005.InsertChkdetail(detail, chk_wh_grade);
                        }
                    }
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    foreach (CHK_DETAIL_TEMP temp in temps)
                    {
                        ...
                        if (repoCE0005.CheckDetailExists(chk_no1, detail.MMCODE) == false)
                        {
                            session.Result.afrs = repoCE0005.InsertChkDetailFromTemp(detail);
                        }
                        else
        ...
        public ApiResponse GetAddItemList(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    session.Result.etts = repo.GetAddItemList(chk_no, wh_no, chk_wh_grade, chk_wh_kind, chk_type, chk_class, chk_period, mmcode, chk_no1);
                }
                catch
                {
                    throw;
