...
{
    public class CE0007Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse All(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new CE0007Repository(DBWork);
                    CE0007Repository.CHK_MAST_QUERY_PARAMS query = new CE0007Repository.CHK_MAST_QUERY_PARAMS();
                    //if (form.Get("p1").Trim() != "")
                    //{
        ...
        public ApiResponse DetailUpdate(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    CE0007Repository repo = new CE0007Repository(DBWork);
        ...
        public ApiResponse Finish(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    string chk_wh_kind = repo.GetChkWhKind(form.Get("CHK_NO"));

                    CE0007Repository.CHK_MAST_QUERY_PARAMS query = new CE0007Repository.CHK_MAST_QUERY_PARAMS();
                    query.CHK_NO = form.Get("CHK_NO") == null ? "" : form.Get("CHK_NO");    // 複盤點單號
                    ...
                    //{
                    //    //session.Result.afrs = SetDetailtotWard(DBWork, mast, myEnum, tempCHK_YM);
                    ...
                    var count = repoCE04.GetUndoneDetailCount(query.CHK_NO);
                    if (count != "0") //沒盤完
                    {
                        session.Result.success = false;
                    ...
                    foreach (var item in myEnum)
                    {
                        ...
                        CE0007Repository.UPDATE_CHK_DETAIL_PARAMS myParams = new CE0007Repository.UPDATE_CHK_DETAIL_PARAMS();
        ...
        public CE0007Repository.UPDATE_CHK_DETAIL_PARAMS GetUpdateParamsGrade1(IUnitOfWork DBWork, CHK_MAST mast, CHK_DETAIL detail) {
            CE0007Repository.UPDATE_CHK_DETAIL_PARAMS myParams = new CE0007Repository.UPDATE_CHK_DETAIL_PARAMS();
            CE0007Repository repo = new CE0007Repository(DBWork);
            ...
            if (mast.CHK_WH_GRADE == "1")
            {
                if (mast.CHK_WH_KIND == "0" || mast.CHK_WH_KIND == "1")   // 0-藥品 & 1-衛材
                {
                    ...
                    else
                    {
                        ...
                        //myParams.STORE_QTYC = repo.GetSTORE_QTYC(mast.CHK_NO, detail.MMCODE, detail.MMCODE, spec_wh_no, mast.CHK_WH_GRADE);
        ...
        public CE0007Repository.UPDATE_CHK_DETAIL_PARAMS GetUpdateParamsGrade23(IUnitOfWork DBWork, CHK_MAST mast, CHK_DETAIL detail)
        {
            CE0007Repository.UPDATE_CHK_DETAIL_PARAMS myParams = new CE0007Repository.UPDATE_CHK_DETAIL_PARAMS();
            CE0007Repository repo = new CE0007Repository(DBWork);
