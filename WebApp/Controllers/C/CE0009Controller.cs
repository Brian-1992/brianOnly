...
{
    public class ﻿﻿CE0009Controller : SiteBase.BaseApiController
    {

        //修改
        [HttpPost]
        public ApiResponse Update﻿﻿CE0009_INI(﻿﻿CE0009 ce0009)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new ﻿﻿CE0009Repository(DBWork);
                    ce0009.UPDATE_USER = User.Identity.Name;
                    ce0009.UPDATE_IP = DBWork.ProcIP;
                    session.Result.afrs = repo.Update﻿﻿CE0009_INI(ce0009);
                    session.Result.etts = repo.Get(ce0009.CHK_NO, ce0009.MMCODE, ce0009.STORE_LOC); //可讓前端收到資料後關閉mask
        ...
        [HttpPost]
        public ApiResponse GetCE0009_INI(FormDataCollection form) //﻿﻿CE0009
        {
            var chk_no = form.Get("chk_no");
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ﻿﻿CE0009Repository repo = new ﻿﻿CE0009Repository(DBWork);
                    session.Result.etts = repo.Get﻿﻿CE0009_INI(chk_no);
                }
                catch
                {
                    throw;
        ...
        [HttpPost] //已盤
        public ApiResponse SelectCountSign(FormDataCollection form) //﻿﻿CE0009
        {
            var chk_no = form.Get("chk_no");
            var chk_uid = form.Get("chk_uid");
        ...
        [HttpPost]//未盤
        public ApiResponse SelectCountNoSign(FormDataCollection form) //﻿﻿CE0009
        {
            var chk_no = form.Get("chk_no");
            var chk_uid = form.Get("chk_uid");
        ...
        public ApiResponse UpdateAll(FormDataCollection form)
        {
            ...
            IEnumerable<﻿﻿CE0009> cE0009 = JsonConvert.DeserializeObject<IEnumerable<﻿﻿CE0009>>(ITEM_STRING);
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new ﻿﻿CE0009Repository(DBWork);
                    foreach (var onecE0009 in cE0009)
                    {
                        if (onecE0009.CHK_QTY == "")
                        {
                            session.Result.afrs = repo.UpdateCE0009_INI_NOTIME_PC(onecE0009.CHK_QTY, onecE0009.CHK_REMARK, User.Identity.Name, DBWork.ProcIP, onecE0009.CHK_NO, onecE0009.MMCODE, onecE0009.STORE_LOC);
                        }
                        else
                        {
                            session.Result.afrs = repo.UpdateCE0009_INIPC(onecE0009.CHK_QTY, onecE0009.CHK_REMARK, User.Identity.Name, DBWork.ProcIP, onecE0009.CHK_NO, onecE0009.MMCODE, onecE0009.STORE_LOC);
                        }
                    }
