...
{
    public class CE0019Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        public ApiResponse All(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new CE0019Repository(DBWork);
                    var repoCE0002 = new CE0002Repository(DBWork);

                    IEnumerable<CE0019> items = repo.GetAll(wh_no, d0, User.Identity.Name, page, limit, sorters);

                    foreach (CE0019 item in items)
                    {
        ...
        public ApiResponse AllINI(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new CE0019Repository(DBWork);
                    IEnumerable<CHK_CE0019VM> items = repo.GetAllINI(chk_no, User.Identity.Name, page, limit, sorters);
        ...
        public ApiResponse Excel(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    CE0019Repository repo = new CE0019Repository(DBWork);
        ...
        //修改
        [HttpPost]
        public ApiResponse Save(FormDataCollection form) //CE0019
        {
            var CHK_NO = form.Get("CHK_NO");
            var ITEM_STRING = form.Get("ITEM_STRING");

            IEnumerable<CHK_CE0019VM> cE0019 = JsonConvert.DeserializeObject<IEnumerable<CHK_CE0019VM>>(ITEM_STRING);
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new CE0019Repository(DBWork);
                    foreach (var onecE0019 in cE0019)
                    {
                        session.Result.afrs = repo.UpdateCHK_G2_DETAIL_Save(onecE0019.CHK_QTY, CHK_NO , onecE0019.MMCODE, User.Identity.Name, DBWork.ProcIP);
                    }
