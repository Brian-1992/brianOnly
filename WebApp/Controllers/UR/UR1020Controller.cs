...
{
    public class UR1020Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        ...
        public ApiResponse GetRolesInUser(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new UR_UIRRepository(DBWork);
                    session.Result.etts = repo.GetRolesInUser(p0, DBWork.UserInfo.UserId);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetRolesNotInUser(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new UR_UIRRepository(DBWork);
                    session.Result.etts = repo.GetRolesNotInUser(p0, p1, p2, DBWork.UserInfo.UserId);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetUsersNotInRole(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new UR_UIRRepository(DBWork);
                    session.Result.etts = repo.GetUsersNotInRole(p0, p1, p2, p3, p4, DBWork.UserInfo.Inid.Substring(0, 2));
                }
                catch
                {
                    throw;
