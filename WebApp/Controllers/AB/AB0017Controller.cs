...
{
    public class AB0017Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AB0017Repository(DBWork);
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AB0017Repository repo = new AB0017Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0017Repository repo = new AB0017Repository(DBWork);
                    AB0017Repository.MI_MAST_QUERY_PARAMS query = new AB0017Repository.MI_MAST_QUERY_PARAMS();
        ...
        public ApiResponse GetMatclassCombo()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    ...
                    session.Result.etts = repo.GetMatclassCombo("5");
                }
                catch
                {
                    throw;
