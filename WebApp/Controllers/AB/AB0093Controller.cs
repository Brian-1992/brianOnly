...
{
    public class AB0093Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AB0093Repository(DBWork);
                    var v_inid = repo.GetUridInid(User.Identity.Name);
                    session.Result.etts = repo.GetAllM(p0, d0, d1, arr_p2, v_inid, page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AB0093Repository repo = new AB0093Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0093Repository repo = new AB0093Repository(DBWork);
                    AB0093Repository.MI_MAST_QUERY_PARAMS query = new AB0093Repository.MI_MAST_QUERY_PARAMS();
