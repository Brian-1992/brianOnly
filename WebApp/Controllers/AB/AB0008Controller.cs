...
{
    public class AB0008Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AB0008Repository(DBWork);
                    var v_inid = repo.GetUridInid(User.Identity.Name);
                    session.Result.etts = repo.GetAllM(p0, d0, d1, arr_p2, p3, p4, p5, v_inid, page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse CreateD(ME_DOCD_EC ME_DOCD_EC)
        {
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    if (!repo.CheckMmcode(ME_DOCD_EC.MMCODE,ME_DOCD_EC.DOCNO))
                    {
                        session.Result.afrs = 0;
                        session.Result.success = false;
                        session.Result.msg = "<span style='color:red'>此院內碼「" + ME_DOCD_EC.MMCODE + "」不正確</span>，請確認。";
                    }
                    else
                    {
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AB0008Repository repo = new AB0008Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            ...
                            if (repo.CheckExistsM(tmp[i]))
                            {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = "<span style='color:red'>此申購單狀態非申購中</span>不得核撥。";
                            }
                            else
                            {
                                if (repo.CheckExistsD(tmp[i])) // 傳入DOCNO檢查申購單是否有院內碼項次
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            if (repo.CheckExistsM2(tmp[i]))
                            {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = "<span style='color:red'>此申購單狀態非已核撥</span>不得取消核撥。";
                            }
                            else
                            {
                                if (repo.CheckExistsD(tmp[i])) // 傳入DOCNO檢查申購單是否有院內碼項次
        ...
        [HttpPost]
        public ApiResponse GetJcnCombo()
        {
            using (WorkSession session = new WorkSession())
            {
                UnitOfWork DBWork = session.UnitOfWork;
                try
                {
                    AB0008Repository repo = new AB0008Repository(DBWork);
                    var v_inid = repo.GetUridInid(User.Identity.Name);
                    session.Result.etts = repo.GetJcnCombo(v_inid);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0008Repository repo = new AB0008Repository(DBWork);
                    AB0008Repository.MI_MAST_QUERY_PARAMS query = new AB0008Repository.MI_MAST_QUERY_PARAMS();
