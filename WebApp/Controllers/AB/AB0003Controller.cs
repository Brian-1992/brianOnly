...
{
    public class AB0003Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AB0003Repository(DBWork);
                    var v_inid = User.Identity.Name; //repo.GetUridInid(User.Identity.Name);
                    session.Result.etts = repo.GetAllM(p0, d0, d1, arr_p2, p3, p4, v_inid, page, limit, sorters);
                }
                catch
                {
                    throw;
            ...
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    if (!repo.CheckExists(ME_DOCM.DOCNO)) // 新增前檢查主鍵是否已存在
                    {
                        ...
                        var v_applywhno = repo.GetApplyWhno(v_inid);
                        ...
                        //{
                            if (v_applykind == "1")
                            {
                                if (v_num > 0) // 1月1次
                                {
                                    ...
                                    session.Result.afrs = 0;
                                    session.Result.success = false;
                                    session.Result.msg = "<span style='color:red'>常態申請</span>次數超過，請改為臨時申請。";
                                }
                                else
                                {
                                    v_create = true;
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AB0003Repository repo = new AB0003Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            ...
                            if (repo.CheckDuplicateMmcode(tmp[i])) {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = string.Format("申請單號「{0}」<span style='color:red'>品項院內碼重複</span>，請修改後再送核撥", tmp[i]);
                                return session.Result;
                            }
                            ...
                            if (whmm_docds.Any())
                            {
                                ...
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = string.Format("申請單號「{0}」<span style='color:red'>有本庫房不可申領品項</span>，請修改後再送核撥，品項如下：<br>{1}",
                                                                    tmp[i], error_mmcodes);
                                return session.Result;
                            }
                            // 2022-11-16 新增：檢查是否所有品項皆為庫備
                            IEnumerable<ME_DOCD> mstoreid_docds = repo.CheckMstoreid(tmp[i], "1");
                            if (mstoreid_docds.Any())
                            {
                                ...
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = string.Format("申請單號「{0}」<span style='color:red'>有非庫備品項</span>，請修改後再送核撥，品項如下：<br>{1}",
                                                                    tmp[i], error_mmcodes);
                                return session.Result;
                            }
                            ...
                            var v_apply = false;
                            var v_applykind = repo.GetDocMApplyKind(tmp[i]);
                            if (v_applykind == "1")
                            {
                                var v_towh = repo.GetDocMTowh(tmp[i]);
                                if (repo.CheckApplyKind(v_towh))
                                {
                                    v_apply = true;
                                }
                                else
                                {
                                    v_apply = false;
                                    if (repo.CheckWhapldt(v_towh))
                                    {
                                        session.Result.afrs = 0;
                                        session.Result.success = false;
                                        session.Result.msg = "此入庫庫房未設定核撥日期<br/><span style='color:red'>請洽中央庫房管理人員!</span>";
                                    }
                                    else
                                    {
                                        var apply_dates = repo.GetApplyDateS(v_towh);
                                        var apply_datee = repo.GetApplyDateE(v_towh);
                                        session.Result.afrs = 0;
                                        session.Result.success = false;
                                        session.Result.msg = "<span style='color:red'>無法送核撥!</span><br/><br/>" +
                                            "目前設定常態申請可送核撥期間為<span style='color:red'>" + apply_datee +" 12:00 前"+ "</span>。";
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0003Repository repo = new AB0003Repository(DBWork);
                    AB0003Repository.MI_MAST_QUERY_PARAMS query = new AB0003Repository.MI_MAST_QUERY_PARAMS();
        ...
        public ApiResponse Excel(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0003Repository repo = new AB0003Repository(DBWork);
                    JCLib.Excel.Export("AB0003.xls", repo.GetExcel());
                }
                catch
                {
                    throw;
