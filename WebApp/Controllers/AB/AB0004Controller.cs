...
{
    public class AB0004Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AB0004Repository(DBWork);
            ...
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    if (!repo.CheckExists(ME_DOCM.DOCNO)) // 新增前檢查主鍵是否已存在
                    {
                        ...
                        var v_num = repo.CheckApplyKind(v_matclass, ME_DOCM.TOWH);
                        var v_num_daily = repo.CheckApplyKindDaily(v_matclass, ME_DOCM.TOWH);
                        ...
                        //else
                        //{
                        if (v_applykind == "1" && v_num_daily >= 1) {    //每天最多一張常態申請單
                            session.Result.afrs = 0;
                            session.Result.success = false;
                            session.Result.msg = "<span style='color:red'>常態申請</span>每日次數超過，請改為臨時申請。";
                            return session.Result;
                        }
                            if (v_applykind == "1" && v_num > 1) //每週可申請二次
                            {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = "<span style='color:red'>常態申請</span>每週次數超過，請改為臨時申請。";
                            }
                            else
                            {
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AB0004Repository repo = new AB0004Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
                ...
                try
                {
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            ...
                            IEnumerable<ME_DOCD> e_docds = repoAB0003.CheckMApplyidE(tmp[i], false);
                            if (e_docds.Any())
                            {
                                string error_mmcodes = string.Empty;
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0004Repository repo = new AB0004Repository(DBWork);
                    AB0004Repository.MI_MAST_QUERY_PARAMS query = new AB0004Repository.MI_MAST_QUERY_PARAMS();
        ...
        public ApiResponse Excel(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AB0004Repository repo = new AB0004Repository(DBWork);
                    JCLib.Excel.Export("AB0004.xls", repo.GetExcel());
                }
                catch
                {
                    throw;
