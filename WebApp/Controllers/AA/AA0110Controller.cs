...
{
    public class AA0110Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AA0110Repository(DBWork);
                    var v_user = User.Identity.Name;
                    session.Result.etts = repo.GetAllM(p0, d0, d1, p2, p3, v_user, page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse UpdateMeDocd()
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AA0110Repository repo = new AA0110Repository(DBWork);
        ...
        public ApiResponse GetMmCodeCombo(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0110Repository repo = new AA0110Repository(DBWork);
                    var greaterThanInv = true;
                    //如果是813新竹，庫存條件會改成大於等於
                    if (repo.GetHospCode() == "813")
                    {
                        greaterThanInv = false;
                    }
                    session.Result.etts = repo.GetMmCodeCombo(p0, p1, greaterThanInv, page, limit, "");
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0110Repository repo = new AA0110Repository(DBWork);
                    AA0110Repository.MI_MAST_QUERY_PARAMS query = new AA0110Repository.MI_MAST_QUERY_PARAMS();
                    ...
                    session.Result.etts = repo.GetMmcode(query, greaterThanInv, page, limit, sorters);
                }
                catch
                {
                    throw;
