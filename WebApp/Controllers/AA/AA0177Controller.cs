...
{
    public class AA0177Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse All(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new AA0177Repository(DBWork);
        ...
        public ApiResponse GetExcel(FormDataCollection form)
        {
            ...
            string fileName = string.Format("一級庫基準量維護_{0}.xlsx", DateTime.Now.ToString("yyyyMMddHHmmss"));
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AA0177Repository repo = new AA0177Repository(DBWork);
        ...
        public ApiResponse Update(FormDataCollection form)
        {
            ...
            var SUPPLY_MAX_APPQTY = form.Get("SUPPLY_MAX_APPQTY");
            var PHR_MAX_APPQTY = form.Get("PHR_MAX_APPQTY");
            var G34_MAX_APPQTY = form.Get("G34_MAX_APPQTY");
            var WAR_QTY = form.Get("WAR_QTY");
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                {
                    var repo = new AA0177Repository(DBWork);
                    AA0177 AA0177 = new AA0177();
                    AA0177.MMCODE = MMCODE;
                    AA0177.NOW_RO = NOW_RO;
                    AA0177.DIFF_PERC = DIFF_PERC;
                    AA0177.DAY_RO = DAY_RO;
                    AA0177.SAFE_QTY = SAFE_QTY;
                    AA0177.NORMAL_QTY = NORMAL_QTY;
                    AA0177.SUPPLY_MAX_APPQTY = SUPPLY_MAX_APPQTY;
                    AA0177.PHR_MAX_APPQTY = PHR_MAX_APPQTY;
                    AA0177.G34_MAX_APPQTY = G34_MAX_APPQTY;
                    AA0177.WAR_QTY = WAR_QTY;
                    AA0177.RO_TYPE = RO_TYPE;
                    AA0177.WH_NO = WH_NO;

                    AA0177.UPDATE_USER = DBWork.UserInfo.UserId;
                    AA0177.UPDATE_IP = DBWork.ProcIP;

                    session.Result.afrs = repo.Update(AA0177);
                    DBWork.Commit();
                }
                catch
                {
        ...
        public ApiResponse UpdatePerc(FormDataCollection form)
        {
            ...
            var SUPPLY_PERC = form.Get("SUPPLY_PERC");
            var PHR_PERC = form.Get("PHR_PERC");
            var G34_PERC = form.Get("G34_PERC");
            var WAR_PERC = form.Get("WAR_PERC");
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new AA0177Repository(DBWork);
                    AA0177 AA0177 = new AA0177();
                    AA0177.SAFE_PERC = SAFE_PERC;
                    AA0177.NORMAL_PERC = NORMAL_PERC;
                    AA0177.SUPPLY_PERC = SUPPLY_PERC;
                    AA0177.PHR_PERC = PHR_PERC;
                    AA0177.G34_PERC = G34_PERC;
                    AA0177.WAR_PERC = WAR_PERC;
                    AA0177.SAFE_QTY = SAFE_QTY;
                    AA0177.NORMAL_QTY = NORMAL_QTY;
                    AA0177.SUPPLY_MAX_APPQTY = SUPPLY_MAX_APPQTY;
                    AA0177.PHR_MAX_APPQTY = PHR_MAX_APPQTY;
                    ...
                    session.Result.afrs = repo.UpdatePerc(AA0177);
                    DBWork.Commit();
                }
                catch
                {
        ...
        public ApiResponse GetExcelExample(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    string fileName = string.Format("匯入一級庫基準量維護_{0}.xlsx", DateTime.Now.ToString("yyyyMMddHHmmss"));
                    AA0177Repository repo = new AA0177Repository(DBWork);
                    ...
                    for (i = 0; i < dtTable.Rows.Count; i++)
                    {
                        ...
                        if (dtTable.Rows[i]["檢核結果"].ToString() == "OK")
                        {
                            ...
                            string[] colNames = {
                                ...
                                //"六個月平均消耗量",
                                "護理病房最大請領量", "供應中心最大請領量", "藥局請領最大量", "戰備存量",
                                "安全庫存量", "正常庫存量", "誤差百分比", "安全存量比值百分比",
                                //"日基準量", "月基準量"
                ...
                try
                {
                    ...
                    foreach (MI_BASERO_14 mi_basero_14 in mi_basero_14List)
                    {
                        ...
                        if (repo.CheckExistsCompundKey(mi_basero_14.MMCODE, mi_basero_14.WH_NO))
                        {
                            ...
                            if (double.TryParse(oldMI_BASE_RO.SUPPLY_PERC, out double dSUPPLY_PERC) && dSUPPLY_PERC > 0)
                            {
                                oldMI_BASE_RO.SUPPLY_MAX_APPQTY = (double.Parse(mi_basero_14.NOW_RO) * dSUPPLY_PERC).ToString();
                            }
                            else
                            {
                            ...
                            if (double.TryParse(oldMI_BASE_RO.PHR_PERC, out double dPHR_PERC) && dPHR_PERC > 0)
                            {
                                oldMI_BASE_RO.PHR_MAX_APPQTY = (double.Parse(mi_basero_14.NOW_RO) * dPHR_PERC).ToString();
                            }
                            else
                            {
                            ...
                            if (double.TryParse(oldMI_BASE_RO.WAR_PERC, out double dWAR_PERC) && dWAR_PERC > 0)
                            {
                                oldMI_BASE_RO.WAR_QTY = (double.Parse(mi_basero_14.NOW_RO) * dWAR_PERC).ToString();
                            }
                            else
                            {
