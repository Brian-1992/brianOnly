...
{
    public class AA0150Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AA0150Repository(DBWork);

                    session.Result.etts = repo.GetAllM(p0, p1, arr_p2, arr_p3, arr_p4, pp, tuser, page, limit, sorters);
                }
                catch
                {
                    throw;
            ...
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        session.Result.msg = "<span style='color:red'>換貨單號</span>重複，請重新輸入。";
                    }
        ...
        public ApiResponse CreateD(ME_DOCEXP docexp)
        {
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    if (!repo.CheckExistsMMCODE(docexp.MMCODE))
                    {
                    ...
                    if (repo.CheckExpExisted(docexp, false))
                    {
                        ...
                        if (docexp.INOUT == "1")
                            INOUT_N = "換入";
                        else if (docexp.INOUT == "2")
                            INOUT_N = "換出";
                        session.Result.msg = "<span style='color:red'>此單明細已有重複的「" + INOUT_N + "」、院內碼「" + docexp.MMCODE + "」、批號、效期</span>，請確認。";
                    }
                    else
                    {
        ...
        public ApiResponse DeleteM(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AA0150Repository repo = new AA0150Repository(DBWork);
                    if (form.Get("DOCNO") != "")
                    {
                                ...
                                if (repo.CheckExistsE(tmp[i])) 
                                {
                                    ...
                                    { //等量換貨才做
                                        foreach (ME_DOCEXP docexp in docexps)
                                        {
                                            ...
                                            AA0151Repository repo1 = new AA0151Repository(DBWork);
                                            repo1.UpdateWloc(docm_ori.TOWH, docexp.MMCODE, docexp.APVQTY, DBWork.UserInfo.UserId, DBWork.ProcIP);
                                        }
                                    }
        ...
        public ApiResponse GetLotNoCombo(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0150Repository repo = new AA0150Repository(DBWork);
                    session.Result.etts = repo.GetLotNoCombo(p0,p1);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0150Repository repo = new AA0150Repository(DBWork);
                    AA0150Repository.MI_MAST_QUERY_PARAMS query = new AA0150Repository.MI_MAST_QUERY_PARAMS();
