...
{
    public class AA0024Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AA0024Repository(DBWork);

                    var taskid = repo.GetTaskid(User.Identity.Name);
                    session.Result.etts = repo.GetAllM(p0, p1, arr_p2, arr_p3, taskid, page, limit, sorters);
                }
                catch
                {
                    throw;
        ...
        public ApiResponse UpdateMeDocd()
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    AA0024Repository repo = new AA0024Repository(DBWork);
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0024Repository repo = new AA0024Repository(DBWork);
                    AA0024Repository.MI_MAST_QUERY_PARAMS query = new AA0024Repository.MI_MAST_QUERY_PARAMS();
        ...
        public ApiResponse GetAgenCombo()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    AA0026Repository repo = new AA0026Repository(DBWork);
