...
{
    public class AA0163Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        public ApiResponse All(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    dt.Rows.Add(SetRow(dt, "當月醫療總收入金額(A)", form.Get("money1"), form.Get("money2"), ""));

                    IEnumerable<AA0163M> myEnum = repo.GetAll(form.Get("DATA_YM"));
                    ...
                    foreach (var item in myEnum)
                    {
                        for (int i = 1; i < 10; i++)
                        {
                            ...
                            else if (i == 4)
                            {
                                ...
                                dt.Rows.Add(SetRow(dt, "當月平均庫儲金額<br>(D)=(B+C)/2", a.ToString("f2"), b.ToString("f2"), ""));
                            }
                            else if (i == 5)
                            ...
                            else if (i == 6)
                            {
                                ...
                                dt.Rows.Add(SetRow(dt, "當月期末比值<br>(F)=(C/E)", a.ToString("f2"), b.ToString("f2"), ""));
                            }
                            else if (i == 7)
                            {
                                double a = 0, b = 0;
                                ...
                                dt.Rows.Add(SetRow(dt, "當月存貨週轉率<br>(G)=(E/D)", a.ToString("f2"), b.ToString("f2"), ""));
                            }
                            else if (i == 8)
                            {
                                double a = 0, b = 0;
                                ...
                                dt.Rows.Add(SetRow(dt, "當月消耗金額佔醫療收入之百分比<br>(H)=(E/A)*100%", a.ToString("f2") + "%", b.ToString("f2") + "%", ""));
                            }
                            else if (i == 9)
                            ...
                            else if (i == 10)
                            {
                                ...
                                dt.Rows.Add(SetRow(dt, "當月低週轉率品項之百分比<br>(J)=(I/E)*100%", a.ToString("f2") + "%", b.ToString("f2") + "%", ""));

                            }
                        }
                    }

                    session.Result.etts = ConvertToAA0163Model(dt);
                }
                catch
                {
                    throw;
        ...
        public IEnumerable<AA0163M> ConvertToAA0163Model(DataTable dataTable)
        {
            foreach (DataRow row in dataTable.Rows)
            {
                yield return new AA0163M
                {
