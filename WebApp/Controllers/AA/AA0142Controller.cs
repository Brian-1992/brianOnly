...
{
    public class AA0142Controller : SiteBase.BaseApiController
    {
        [HttpPost]
        public ApiResponse MasterAll(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    var repo = new AA0142Repository(DBWork);
        ...
        public ApiResponse LoadDetailItems(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    IEnumerable<MI_WINVMON> invs = repo.GetInvs(frwh.WH_NO, frwh.MAT_CLASS);
                    ...
                    foreach (ME_DOCM master in masters)
                    {
                        ...
                        foreach (ME_DOCD docd in docds)
                        {
                            ...
                            MI_WHTRNS trns = new MI_WHTRNS()
                            {
                                WH_NO = docd.WH_NO,
                                MMCODE = docd.MMCODE,
                                TR_INV_QTY = (double.Parse(docd.APPQTY) ).ToString(),
                                TR_ONWAY_QTY = "0",
                                TR_DOCNO = master.DOCNO,
                                TR_DOCSEQ = docd.SEQ,
                                TR_FLOWID = docm.FLOWID,
                                TR_DOCTYPE = docm.DOCTYPE,
                                TR_IO = double.Parse(docd.APPQTY) >= 0 ? "O" : "I",
                                TR_MCODE = double.Parse(docd.APPQTY) >= 0 ? "ADJO" : "ADJI",
