...
{
    public class AA0151Controller : SiteBase.BaseApiController
    {
        // 查詢
        [HttpPost]
        public ApiResponse AllM(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    var repo = new AA0151Repository(DBWork);
        ...
        public ApiResponse CreateD(ME_DOCEXP docexp)
        {
            using (WorkSession session = new WorkSession(this)) // 如果要取得DBWork資訊(如ProcIP),WorkSession需代入this
            {
                ...
                try
                {
                    ...
                    if (repo.CheckExpExisted(docexp,false))
                    {
                        session.Result.afrs = 0;
                        session.Result.success = false;
        ...
        public ApiResponse UpdateD(ME_DOCEXP docexp)
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    if (!repo.CheckExpExisted(docexp, true))
                    {
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            if (repo.CheckExistsM(tmp[i]))
                            {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = "<span style='color:red'>此單據狀態非申請中</span>不得執行過帳。";
                            }
                            else
                            {
                                if (repo.CheckExistsE(tmp[i])) 
                                {
                                    ...
                                    // 2022-06-07: 非批號效期品項修改MI_WEXPINV
                                    ...
                                    { //等量換貨才做
                                        foreach (ME_DOCEXP docexp in docexps)
                                        {
                                            ...
                                            repo.UpdateWloc(docm_ori.TOWH, docexp.MMCODE, docexp.APVQTY, DBWork.UserInfo.UserId, DBWork.ProcIP);
                                        }
                                    }
                                    ...
                                    //2023-10-07: 退貨增加寫入發票檔
                                    session.Result.afrs = repo.InsertPhInvoice(me_docm.DOCNO, DBWork.UserInfo.UserId, DBWork.ProcIP);
        ...
        [HttpPost]
        public ApiResponse CheckMFlow(FormDataCollection form)
        {
            using (WorkSession session = new WorkSession(this))
            {
                UnitOfWork DBWork = session.UnitOfWork;
                ...
                try
                {
                    ...
                    if (form.Get("DOCNO") != "")
                    {
                        ...
                        for (int i = 0; i < tmp.Length; i++)
                        {
                            if (!repo.CheckMFlow(tmp[i]))
                            {
                                session.Result.afrs = 0;
                                session.Result.success = false;
                                session.Result.msg = "<span style='color:red'>此單據非退貨狀態</span>不得列印。";
                            }
                            else
                            {
                                v_success += 1;
        ...
        public ApiResponse GetMmcode(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    AA0151Repository repo = new AA0151Repository(DBWork);
                    AA0151Repository.MI_MAST_QUERY_PARAMS query = new AA0151Repository.MI_MAST_QUERY_PARAMS();
        ...
        public ApiResponse GetMatclassQCombo()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    ...
                    session.Result.etts = repo.GetMatclassQCombo(id);
                }
                catch
                {
                    throw;
        ...
        [HttpPost]
        public ApiResponse GetReturnReasonCombo()
        {
            using (WorkSession session = new WorkSession())
            {
                UnitOfWork DBWork = session.UnitOfWork;
                ...
                {
                    AA0151Repository repo = new AA0151Repository(DBWork);
                    session.Result.etts = repo.GetReturnReasonCombo();
                }
                catch
                {
                    throw;
        ...
        public string GetINV_QTY(FormDataCollection form)
        {
            ...
            using (WorkSession session = new WorkSession())
            {
                ...
                {
                    var repo = new AA0151Repository(DBWork);
                    var invNum = repo.GetINV_QTY(FRWH, MMCODE, LOT_NO);
                    return invNum != null ? invNum : "0";
                }
                catch
                {
                    throw;
