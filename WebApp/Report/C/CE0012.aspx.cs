...
{
    public partial class CE0012 : Page
    {
        string chk_no;
        string chk_level;
        string wh_kind;
        ...
        protected void report1Bind()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    CE0012Repository repo = new CE0012Repository(DBWork);
                    ReportViewer1.EnableTelemetry = false;
                    IEnumerable<CE0012M> temp = repo.Print(chk_no, chk_level, wh_kind, condition);
                    foreach (CE0012M item in temp) {
                        item.CHK_TYPE_NAME = repo.GetChkWhkindName(item.CHK_WH_KIND, item.CHK_TYPE);
                    }
                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("CE0012", temp));
