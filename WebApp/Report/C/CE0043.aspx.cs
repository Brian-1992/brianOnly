...
{
    public partial class CE0043 : Page
    {
        string chk_no;
        string chk_level;
        string wh_kind;
        ...
        protected void report1Bind()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    CE0043Repository repo = new CE0043Repository(DBWork);
                    ReportViewer1.EnableTelemetry = false;

                    IEnumerable<CE0043Data> temp = repo.PrintData(chk_no, chk_level, wh_kind, condition);
                    foreach (CE0043Data item in temp)
                    {
                        item.CHK_TYPE_NAME = repo.GetChkWhkindName(item.CHK_WH_KIND, item.CHK_TYPE);
                    }
                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("CE0043Data", temp));
                    ...
                    IEnumerable<CE0043Count> count = GetCountItem(countAll, countP, countN);

                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("CE0043Count", count));
