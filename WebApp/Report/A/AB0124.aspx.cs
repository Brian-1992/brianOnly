...
{
    public partial class AB0124 : SiteBase.BasePage
    {
        FL fl = new FL("WebApp.Report.A.AB0124");
        string P0;
        string YYYMM;
        ...
        protected void report1Bind()
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    AB0124Repository repo = new AB0124Repository(DBWork);
                    ...
                    IEnumerable<AB0124ReportMODEL> lstTime = repo.GetMiMnsetData(P0);  // 從資料庫讀取開始結束日期(SET_BTIME, SET_ETIME)
                    foreach (AB0124ReportMODEL t in lstTime)
                    {
                    ...
                    IEnumerable<AB0124ReportMODEL> lst;
                    // 從資料庫讀取資料
                    lst = repo.GetPrintData(P0);
                    if (
                        ((List<AB0124ReportMODEL>)lst).Count == 0
                    )
                    {
                        // 模擬給值
                        List<AB0124ReportMODEL> lstAll = new List<AB0124ReportMODEL>();
                        AB0124ReportMODEL v = new AB0124ReportMODEL();
                        v.MMCODE = " "; // 00.院內碼
                        ...
                        v.TPN_A = "0"; // 16.製劑室_消耗
                        v.TPN_I = "0"; // 17.製劑室_結存 
                        v.DISC_CPRICE = "0"; // 優惠平均單價
                        lstAll.Add(v);
                        lst = (IEnumerable<AB0124ReportMODEL>)lstAll;
                    } // 

                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("AB0124DataSet", lst));
