...
{
    public partial class AB0077 : SiteBase.BasePage
    {
        string P0;  //庫別
        string P1;  //表單類別
        string P2;  //出入庫
        string P3;  //異動日期(起)
        string P4;  //異動日期(迄)
        string P5;  //合約、非合約 or 優惠百分比
        string P6;  //合約品項 or 非合約品項 or 全部品項
        string P7;  //院內碼(起)
        string P8;  //院內碼(迄)
        string P9;  //庫存異動
        string P10; //表單號碼
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                P8 = (Request.QueryString["p8"] is null) ? "" : Request.QueryString["p8"].ToString().Replace("null", "");
                P9 = (Request.QueryString["p9"] is null) ? "" : Request.QueryString["p9"].ToString().Replace("null", "");
                P10 = (Request.QueryString["p10"] is null) ? "" : Request.QueryString["p10"].ToString().Replace("null", "");
        ...
        protected void report1Bind()
        {
            using (WorkSession session = new WorkSession(this))
            {
                ...
                try
                {
                    ...
                    //先清空報表的DataSet，再將讀到的AB0077 DataTable放到DataSources(對應到AB0077.xsd)
                    ReportViewer1.LocalReport.DataSources.Clear();

                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("AB0077", repo.GetReport(P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10)));
