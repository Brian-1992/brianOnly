...
namespace WebApp.Report.AA
{
    public partial class AA0147 : System.Web.UI.Page
    {
        /// <summary>
        /// 申請單號
        /// </summary>
        ...
        protected void report1Bind()
        {
            using (WorkSession session = new WorkSession())
            {
                ...
                try
                {
                    ...
                    DataTable dt = LinqQueryToDataTable<AA0147Report_MODEL>(repo.GetPrintData(arr_docno, User.Identity.Name, hospCode, Order));

                    //判斷資料筆數，如果沒有大於0則將s_IsAnyData填入無資料(報表顯示用)
                    string s_IsAnyData = dt.Rows.Count > 0 ? " " : "查無資料";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        string apptime_str = apptime.Substring(0, 3) + "年" + apptime.Substring(3, 2) + "月" + apptime.Substring(5, 2) + "日";

                        string posttime = dt.Rows[0]["POST_TIME_T"].ToString();
                        string posttime_str = "";
                        if (posttime != "")
                        {
                            posttime_str = posttime.Substring(0, 3) + "年" + posttime.Substring(3, 2) + "月" + posttime.Substring(5, 2) + "日";
                        }
                        ...
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("POSTTIME", posttime_str) });
                        ...
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("APPID_N", dt.Rows[0]["APPID_N"].ToString().Trim()) });
                    }
                    else {
                        //1130313 查詢結果count=0需將參數值設定空白，否則報表會整個空白誤解為還在執行中
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("DOCNO", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("APPTIME", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("POSTTIME", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("FRWH_N", "")});
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("ISARMY_N", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("APPDEPT_N", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("APPLY_NOTE", "") });
                        ReportViewer1.LocalReport.SetParameters(new ReportParameter[] { new ReportParameter("APPID_N", "") });
                    }
                    //先清空報表的DataSet，再將讀到的AA0147 DataTable放到DataSources(對應到AA0147.xsd)
                    ReportViewer1.LocalReport.DataSources.Clear();
                    ReportViewer1.LocalReport.DataSources.Add(new ReportDataSource("AA0147", dt));
