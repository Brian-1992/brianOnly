...
{
    public class FA0032Repository : JCLib.Mvc.BaseRepository
    {
        public FA0032Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        public IEnumerable<FA0032> GetAll_A(string MAT_CLASS, string APPTIME_B, string APPTIME_E, string P4, string P5, string P6,  int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"select a.appdept
                        ...
                        ,SUM(b.appqty*(select avg_price from MI_WHCOST where data_ym=substr(TWN_DATE(a.apptime),1,5)
                        ...
                        *(select avg_price from MI_WHCOST where data_ym=substr(TWN_DATE(b.apvtime),1,5)
            ...
            if (P5 == "1")
            {
                sql += @"  and ((a.doctype='MR' and a.flowid>='0103')
                                ...
                                or(a.doctype in ('MR1', 'MR2', 'MR3', 'MR4') and a.flowid >= '3')) ";
            }
            else
            {
                sql += @"  and ((a.doctype='MR' and a.flowid<'0103')
                                or(a.doctype = 'MS' and a.flowid < '0602')
                                or(a.doctype in ('MR1', 'MR2', 'MR3', 'MR4') and a.flowid = '2')) ";
            }
            ...
            return DBWork.Connection.Query<FA0032>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }

        public IEnumerable<FA0032> GetAll_M(string MAT_CLASS, string APPTIME_B, string APPTIME_E, string P4, string P5, string P6, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();
        ...
        //public DataTable Report(string MAT_CLASS, string APVTIME_B, string APVTIME_E, string MMCODE)
        //{
        //    var p = new DynamicParameters();

        //    var sql = @"select b.mmcode,(select mmname_c from MI_MAST where mmcode=b.mmcode) as mmname_c,
        //                (select mmname_e from MI_MAST where mmcode=b.mmcode) as mmname_e,
        //                (select base_unit from MI_MAST where mmcode=b.mmcode) as base_unit,
        //                (select m_contprice from MI_MAST where mmcode=b.mmcode) as m_contprice,
        //                (select TWN_TIME(b.apvtime) from dual where a.flowid='6')||
        //                (select TWN_TIME(b.dis_time) from dual where a.flowid<>'6') apvtime,
        //                (select b.apvqty from dual where a.flowid='6')||
        //                (select b.expt_distqty from dual where flowid<>'6') as apvqty
        //                ,a.docno,a.appdept,(select inid_name from UR_INID where inid=a.appdept) as appdeptname
        //                        from ME_DOCM a,ME_DOCD b
        //                       where a.docno=b.docno 
        ...
        //    sql += @" order by b.mmcode,(select TWN_TIME(b.apvtime) from dual where a.flowid='6')||(select TWN_TIME(b.dis_time) from dual where a.flowid<>'6')";
