...
namespace WebApp.Repository.F
{
    ...
    public class FA0074_MODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class FA0074Repository : JCLib.Mvc.BaseRepository
    {

        public FA0074Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<FA0074_MODEL> GetAll(string mysysdate, int page_index, int page_size)
        {
...
null as avginv_ratio,null as avgdays from dual 
	...
	ROUND(365/(select sum(use_amt)/sum((p_inv_amt+inv_amt)/2) from TMP_INVCTL_YR_RP where to_char(rep_time,'yyyymmddhh24mmss')=:rep_time),2) as avgdays from dual";
            ...
            return DBWork.Connection.Query<FA0074_MODEL>(GetPagingStatement(sql), p, DBWork.Transaction);
        }

        public IEnumerable<FA0074_MODEL> GetPrintData(string mysysdate)
        {
            var p = new DynamicParameters();
...
null as avginv_ratio,null as avgdays from dual 
	...
	ROUND(365/(select sum(use_amt)/sum((p_inv_amt+inv_amt)/2) from TMP_INVCTL_YR_RP where to_char(rep_time,'yyyymmddhh24mmss')=:rep_time),2) as avgdays from dual";
            ...
            return DBWork.Connection.Query<FA0074_MODEL>(sql, p, DBWork.Transaction);
        }
     ...
     select To_Date(:RAWSYSDATE,'YYYYMMDD HH24:MI:SS') as rep_time, SUBSTR(A.DATA_YM,1,3) as rep_yr,
        B.MAT_CLASS, SUBSTR(A.DATA_YM,4,2) MON, 
  ...
  GROUP BY SUBSTR(A.DATA_YM,1,3),B.MAT_CLASS,SUBSTR(A.DATA_YM,4,2)";
