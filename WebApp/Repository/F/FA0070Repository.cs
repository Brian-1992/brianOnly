...
namespace WebApp.Repository.F
{
    ...
    public class FA0070ReportMODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class FA0070Repository : JCLib.Mvc.BaseRepository
    {
        public FA0070Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }


        public IEnumerable<FA0070> GetAllM(string rep_time, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"select p.seq ,p.mat_class ,p.skey ,p.mat_clsname ,p.inv_type ,
                        ...
                        p.check_amt ,p.inv_amt , :rep_time rep_time , p.d_amt  
                     from (
                        select seq,c.mat_class,c.skey,c.mat_clsname,inv_type, 
                         nvl(pmn_cost,0) as p_amt,
                         nvl(in_cost,0) as in_amt,
                         nvl(out_cost,0) as out_amt,
                    ...
                    from (
                          select mat_class||skey as seq,skey,mat_class,(select mat_clsname from dual where skey=1) as mat_clsname,
                           ...
                           where mat_class in ('30','31','32','33','34','35','36','37'))
                           order by seq
                  ...
                  union select '999' as seq,'' as mat_class,9 as skey,'總計' as mat_clsname,'全部總計' as inv_type,
                               ...
                               nvl((select sum(d_cost) from TMP_INVCOST_ADJ_RP where rep_time=to_date(:rep_time,'yyyymmddhh24miss')),0) as d_amt
                          from dual
                ) p where 1 = 1
             ";
            ...
            return DBWork.Connection.Query<FA0070>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }

        public IEnumerable<FA0070ReportMODEL> GetPrintData(string rep_time)
        {
            var p = new DynamicParameters();
            var sql = @" select p.seq F1,p.mat_class F2,p.skey F3,p.mat_clsname F4,inv_type F5,
                     ...
                     from (
                             select seq,c.mat_class,c.skey,c.mat_clsname,inv_type, nvl(pmn_cost,0) as p_amt,
                             nvl(in_cost,0) as in_amt,
                             nvl(out_cost,0) as out_amt,
                             nvl((select invt_cost from dual where invt_cost>=0),0) as check_p_amt,
                             nvl((select invt_cost from dual where invt_cost<=0),0) as check_m_amt,
                             nvl((select invt_cost from dual),0) as check_amt,
                             nvl(mn_cost,0) as inv_amt,
                             nvl(d_cost,0) as d_amt
                        from (
                              select mat_class||skey as seq,skey,mat_class,(select mat_clsname from dual where skey=1) as mat_clsname,
                                     (select '醫院軍存量' from dual where skey=1)||
            ...
            return DBWork.Connection.Query<FA0070ReportMODEL>(sql, p, DBWork.Transaction);
        }
        ...
        public DataTable GetExcel4(string apptime1, string apptime2)
        {
            ...
            var sql = @"SELECT A.MMCODE 院內碼,B.MAT_CLASS 類別,B.MMNAME_C 中文品名,B.BASE_UNIT 計量單位, SUM(APVQTY+M_APVQTY+S_APVQTY) 消耗量
                        ...
                        WHERE A.MMCODE=B.MMCODE AND EC_ID='C' ";
            if (apptime1 != "" & apptime2 != "")
            {
            ...
            var sql = @"insert into TMP_INVCOST_ADJ_RP 
                    (rep_time,data_ym,mat_class,skey,
                     ...
                     select to_date(:rep_time,'yyyymmddhh24miss') as rep_time,data_ym,mat_class,skey,
                        pmn_cost, in_cost,out_cost,invt_cost,mn_cost,d_cost  
                       from V_COST_CMM where data_ym=:data_ym order by mat_class,skey
            ";
