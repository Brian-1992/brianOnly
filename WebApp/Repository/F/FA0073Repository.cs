...
namespace WebApp.Repository.F
{
    ...
    public class FA0073_MODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class FA0073Repository : JCLib.Mvc.BaseRepository
    {

        public FA0073Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<FA0073_MODEL> GetAll(string APPTIME1, string APPTIME2, string mat_class, string towh, string showopt, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"select a.TOWH, (select WH_NAME from MI_WHMAST where WH_NO=a.TOWH) as WH_NAME,
                        ...
                        ROUND((select DISC_CPRICE from MI_WHCOST where DATA_YM=substr(TWN_DATE(case when a.MAT_CLASS = '09' then b.apvtime else b.DIS_TIME end),1,5) and MMCODE=b.mmcode),2) as disc_cprice, 
                        ROUND(SUM(b.apvqty)*(select DISC_CPRICE from MI_WHCOST where DATA_YM=substr(TWN_DATE(case when a.MAT_CLASS = '09' then b.apvtime else b.DIS_TIME end),1,5) and MMCODE=b.mmcode),2) as M_ALLPRICE,
            ...
            return DBWork.Connection.Query<FA0073_MODEL>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }
        ...
        public IEnumerable<FA0073_MODEL> GetPrintData(string userId, string APPTIME1, string APPTIME2, string mat_class, string towh, string showopt)
        {
            //轉成需求格式 EX:10804
            ...
            var sql = @"select INID_NAME(USER_INID(:INID)) INID_NAME_USER, a.TOWH as APPDEPT, (select WH_NAME from MI_WHMAST where WH_NO=a.TOWH) as INID_NAME,b.mmcode,(select mmname_c from MI_MAST where mmcode=b.mmcode) as mmname_c, 
                        ...
                        substr(TWN_DATE(case when a.MAT_CLASS = '09' then b.apvtime else b.DIS_TIME end),1,5) as APPTIME_YM,SUM(b.apvqty) as APVQTYN,ROUND((select DISC_CPRICE from MI_WHCOST where DATA_YM=substr(TWN_DATE(case when a.MAT_CLASS = '09' then b.apvtime else b.DIS_TIME end),1,5) and MMCODE=b.mmcode),2) as disc_cprice, 
                        ROUND(SUM(b.apvqty)*(select DISC_CPRICE from MI_WHCOST where DATA_YM=substr(TWN_DATE(case when a.MAT_CLASS = '09' then b.apvtime else b.DIS_TIME end),1,5) and MMCODE=b.mmcode),2) as M_ALLPRICE,COUNT(distinct a.docno) as APP_CNT 
                        from ME_DOCM a,ME_DOCD b where a.docno=b.docno  ";
            ...
            return DBWork.Connection.Query<FA0073_MODEL>(sql, p, DBWork.Transaction);
        }
    }
}