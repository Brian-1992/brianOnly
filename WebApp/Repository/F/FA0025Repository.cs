...
namespace WebApp.Repository.FA
{
    public class FA0025Repository : JCLib.Mvc.BaseRepository
    {
        public FA0025Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<MI_WHMAST> GetWH_NOComboOne(int page_index, int page_size, string sorters)
        {
            ...
            //////////string sql = @"SELECT WH_NO, WH_NAME from MI_WHMAST WHERE WH_GRADE='2' AND WH_KIND = '1'  ORDER BY WH_NO ";
            ////////string sql = @"SELECT WH_NO, WH_NAME from MI_WHMAST where WH_KIND = '1' ORDER BY WH_NO ";
        ...
        public IEnumerable<MI_WHMAST> GetMMCODECombo(string mmcode, int page_index, int page_size, string sorters)
        {
        ...
        public IEnumerable<COMBO_MODEL> GetMatClassCombo(string str_UserID)
        {
            string sql = @"SELECT MAT_CLASS AS VALUE, MAT_CLSNAME AS TEXT, 
                        ...
                        FROM MI_MATCLASS ORDER BY MAT_CLASS";

             // FROM MI_MATCLASS WHERE MAT_CLSID = WHM1_TASK(:UserID)";
