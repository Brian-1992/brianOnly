...
{
    public class CE0012Repository : JCLib.Mvc.BaseRepository
    {
        public CE0012Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<CE0012M> GetDetails(string chk_no, string chk_level, string wh_kind, string condition, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();

            #region old sql
            /*
            string sql = string.Format(@"select mmcode,
                                                (TRIM(NVL(mmname_c, ' ')) || '<br>' || TRIM(NVL(mmname_e, ' '))) as mmname,
                                                store_qty, 
                                                ( store_qty * m_contprice ) as store_cost, 
                                                (CASE
                                                  ...
                                                  END)    AS CHK_QTY,
                                                (CASE
                                                   WHEN STATUS_TOT = '1' THEN CHK_QTY1 * M_CONTPRICE
                                                   WHEN STATUS_TOT = '2' THEN CHK_QTY2 * M_CONTPRICE
                                                   WHEN STATUS_TOT = '3' THEN CHK_QTY3 * M_CONTPRICE
                                                   ELSE 0
                                                   END)    as CHK_COST, 
                                                ...
                                                (case
                                                    when apl_outqty = 0
                                                        then (pro_los_qty)
                                                    else TRUNC(((pro_los_qty) / apl_outqty * 100),5)
                                                  end) as diff_p,
                                                (pro_los_qty * M_CONTPRICE) as diff_cost,
            ...
            string sql = string.Format(@"
              ...
              select mmcode,
                     mmname,
                     store_qty, 
                     round(( store_qty * (case when is_current_ym = 'Y' then m_contprice else avg_price end) ), 2) as store_cost, 
                     CHK_QTY,
                     round(CHK_QTY * (case when is_current_ym = 'Y' then m_contprice else avg_price end), 2)   as CHK_COST, 
                     pro_los_qty as diff_qty,
                    ...
                    round( (pro_los_qty * (case when is_current_ym = 'Y' then m_contprice else avg_price end)), 2) as diff_cost,
                     diff_remark  
                from cost_data
               where 1=1
            ...
            return DBWork.Connection.Query<CE0012M>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }

        public IEnumerable<CE0012M> Print(string chk_no, string chk_level, string wh_kind, string condition)
        {
            var p = new DynamicParameters();
            ...
            return DBWork.Connection.Query<CE0012M>(sql, p, DBWork.Transaction);
        }
