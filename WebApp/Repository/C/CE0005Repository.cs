...
{
    public class CE0005Repository : JCLib.Mvc.BaseRepository
    {
        public CE0005Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<CHK_MAST> GetChkMasts(string wh_no, string chk_ym, string keeper, string chk_status, string chk_level, int page_index, int page_size, string sorters)
        {
            ...
            string sql = @"select a.CHK_NO,
                                  ...
                                  (case
                                        when (select create_user from chk_mast where chk_no1 = a.chk_no1 and chk_level = '1') = 'BATCH'
                                            then 'Y'
                                        else 'N'
                                  end) as is_batch,
                                  (case
        ...
        public int UpdateChkno1(string chk_no, string update_user, string update_ip) {
            string sql = @"update CHK_MAST
                              set chk_status = 'C', update_user = :update_user, 
                                  update_time = sysdate, update_ip = :update_ip
                            where chk_no = :chk_no";
            return DBWork.Connection.Execute(sql, 
                new { chk_no = chk_no, update_user= update_user, update_ip = update_ip}, 
                DBWork.Transaction);
        }
        ...
        public bool CheckExists(string chk_no)
        {
            string sql = @"select 1 from CHK_MAST
                            WHERE chk_no1 = :chk_no 
                              and chk_level = '2'"; // TRUNC(a.EXP_DATE, 'DD') TWN_TODATE(:exp_date)
        ...
        public IEnumerable<CHK_DETAIL> GetExcludeDetails(string chk_no, string chk_no1, string wh_no, float f_u_price, float f_number, float f_amount, string f_mmcode, int page_index, int page_size, string sorters)
        {
            ...
            string sql = @"select distinct 
                                  a.mmcode, a.mmname_c, a.mmname_e, a.base_unit,a.m_contprice,	
                                  a.wh_no, a.mat_class,	a.m_storeid, b.e_takekind, b.e_restrictcode,
                                  a.store_qty1 as STORE_QTYC, ";
            if (wh_no == "PH1S")
            {
                sql += @"          (select inv_qty
                                      from mi_whinv
                ...
                sql += @"          a.store_qty1 - nvl(his_consume_qty_t1, 0) as NEW_STORE_QTYC, ";
            }

            sql += string.Format(@"a.chk_qty1 as CHK_QTY,
                                  a.miss_per, a.gap_t,
                                  ( a.chk_qty1 - (a.store_qty{0} - nvl(his_consume_qty_t1, 0))) as QTY_DIFF,
                                  (case
                                      when (a.store_qty{0} = 0 or a.store_qty{0} is null)
                                          then (a.chk_qty1 - (a.store_qty{0} - nvl(his_consume_qty_t1, 0)))
                                      else TRUNC(((a.chk_qty1 - (a.store_qty{0} - nvl(his_consume_qty_t1, 0))) / a.store_qty{0} * 100),5)
                                    end) as diff_p,
        ...
        private string GetFilter(float f_u_price, float f_number, float f_amount, string f_mmcode)
        {
            ...
            if (f_number > 0)
            {
                filter += @"   and ABS(a.store_qtyc + a.store_qtym + a.store_qtys - a.chk_qty1 ) >= ABS(:f_number)";
            }
            if (f_amount > 0)
            {
                filter += @"   and ABS((a.store_qtyc + a.store_qtym + a.store_qtys - a.chk_qty1) * a.M_CONTPRICE) > ABS(:f_amount)";
            }
            if (f_mmcode != "")
            {
        ...
        public IEnumerable<BC_WHCHKID> GetChknoids(string chk_no)
        {
            string sql = @"select chk_uid as WH_CHKUID  from CHK_NOUID where chk_no = :chk_no";
            return DBWork.Connection.Query<BC_WHCHKID>(sql, new { chk_no = chk_no }, DBWork.Transaction);

        }

        public string GetPreChkno(string chk_no1, string chk_level) {
            string sql = @"select chk_no from CHK_MAST where chk_no1 = :chk_no1 and chk_level = :chk_level";
            return DBWork.Connection.QueryFirst<string>(sql, new { chk_no1 = chk_no1, chk_level = chk_level }, DBWork.Transaction);
        }
...
";
        ...
        {
            string sql = @"select * from CHK_DETAILTOT where chk_no = :chk_no and ABS(gap_t) >= 1";
            return DBWork.Connection.Query<CHK_DETAILTOT>(sql, new { chk_no = chk_no }, DBWork.Transaction);
        }
        #endregion

        #region 2020-09-18 新增: 藥庫盤點單需將當下申請量核撥量更新CHK_PH1S_WHINV
        public int UpdateChkPH1SWhinv(string chk_no, string userid, string update_ip)
        {
            string sql = @"update CHK_PH1S_WHINV a
                              set apl_inqty = (select apl_inqty from MI_WHINV where wh_no = 'PH1S' and mmcode = a.mmcode),
                                  apl_outqty = (select apl_outqty from MI_WHINV where wh_no = 'PH1S' and mmcode = a.mmcode),
                                  exg_inqty = (select exg_inqty from MI_WHINV where wh_no = 'PH1S' and mmcode = a.mmcode),
                                  exg_outqty = (select exg_outqty from MI_WHINV where wh_no = 'PH1S' and mmcode = a.mmcode),
                                  update_time = sysdate,
                                  update_user = :userid,
                                  update_ip = :update_ip
                            where a.chk_no =  (select chk_no1 from CHK_MAST where chk_no = :chk_no) 
