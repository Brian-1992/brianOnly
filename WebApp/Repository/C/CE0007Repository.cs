...
{
    public class CE0007Repository : JCLib.Mvc.BaseRepository
    {
        public CE0007Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<CHK_MAST> GetAll(CHK_MAST_QUERY_PARAMS query)
        {
            var p = new DynamicParameters();
            //var sql = @"Select CHK_NO, CHK_YM, CHK_WH_NO
            //            , CHK_KEEPER || ' ' || USER_NAME(CHK_KEEPER) AS CHK_KEEPER
            //            , WH_NAME(CHK_WH_NO) AS WH_NAME
            //            , CHK_NUM || '/' || CHK_TOTAL AS QTY
            //            , CHK_LEVEL || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_LEVEL' AND DATA_VALUE=CHK_LEVEL) AS CHK_LEVEL
            //            , CHK_WH_GRADE || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_WH_GRADE' AND DATA_VALUE=CHK_WH_GRADE) AS CHK_WH_GRADE
            //            , CHK_WH_KIND || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_WH_KIND' AND DATA_VALUE=CHK_WH_KIND) AS CHK_WH_KIND
            //            , CHK_PERIOD || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_PERIOD' AND DATA_VALUE=CHK_PERIOD) AS CHK_PERIOD
            //            , CASE WHEN CHK_WH_KIND='0' THEN CHK_TYPE || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_WH_KIND_0' AND DATA_VALUE=CHK_TYPE)
            //            ELSE CHK_TYPE || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_WH_KIND_1' AND DATA_VALUE=CHK_TYPE) END AS CHK_TYPE
            //            , CHK_STATUS || ' ' || (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='CHK_MAST' AND DATA_NAME='CHK_STATUS' AND DATA_VALUE=CHK_STATUS) AS CHK_STATUS
            //            , CHK_NO1
            //            from CHK_MAST a, MI_WHID b 
            //            where 1=1 and chk_status='2' and CHK_LEVEL='2'
            //              and b.WH_USERID = :CHK_KEEPER
            //              and a.CHK_WH_NO = b.WH_NO";
            //p.Add("CHK_KEEPER", query.CHK_KEEPER);
            var sql = @"Select CHK_NO, CHK_YM, CHK_WH_NO, chk_level, chk_wh_grade, chk_wh_kind, chk_period, chk_type, chk_status, create_date
                        ...
                        , CHK_NO1
                        from CHK_MAST
                        where 1=1 and chk_status>='1' and CHK_LEVEL='2'";
            p.Add("CHK_KEEPER", query.CHK_KEEPER);
        ...
        {
            var sql = string.Format(@"SELECT (STORE_QTYC 
        ...
        public int UpdateDetailTot(UPDATE_CHK_DETAIL_PARAMS myParams)
        {
            ...
            var sql = @"UPDATE CHK_DETAILTOT 
                           SET STATUS_TOT ='2', CHK_QTY2=:CHK_QTY, GAP_T=:GAP_T, GAP_C=:GAP_C, 
                               ...
                               store_qty2 = :STORE_QTY_N,
                               store_qty_time2 = (select store_qty_update_time from CHK_DETAIL 
                                  where chk_no = :CHK_NO and mmcode = :MMCODE and rownum = 1),
                               consume_qty = :CONSUME_QTY, consume_amount = :CONSUME_AMOUNT,
                               his_consume_qty_t2 = :HIS_CONSUME_QTY_T, 
                               his_consume_datatime2 = :HIS_CONSUME_DATATIME
        ...
        public string GetStoreQty(string chk_no, string mmcode) {
            string sql = @"select store_qty from CHK_DETAILTOT where chk_no = :chk_no and mmcode = :mmcode";
            return DBWork.Connection.QueryFirstOrDefault<string>(sql, new { CHK_NO = chk_no, MMCODE = mmcode }, DBWork.Transaction);
        }
        ...
        public int InsertDetailTot(INSERT_CHK_DETAIL_PARAMS myParams)
        {
            ...
            var sql = @"INSERT INTO CHK_DETAILTOT (chk_no, mmcode, mmname_c, mmname_e, base_unit, m_contprice, wh_no,
                                                   ...
                                                   store_qty1, store_qty2, store_qty3, store_qty_time2, consume_qty, consume_amount,
                                                   his_consume_qty_t2, his_consume_datatime2)
                        (select distinct
                                :CHK_NO1 ,
                                a.MMCODE  ,
                                b.MMNAME_C ,
                                ...
                                :UPDATE_USER ,
                                :UPDATE_IP  ,
                                0 as store_qty1, :STORE_QTY_N as store_qty2, 0 as store_qty3,
                                (select store_qty_update_time from CHK_DETAIL 
                                  where chk_no = a.chk_no and mmcode = b.mmcode and rownum = 1) as store_qty_time2,
                                :CONSUME_QTY, :CONSUME_AMOUNT,
                                :HIS_CONSUME_QTY_T as his_consume_qty_t2, 
                                :HIS_CONSUME_DATATIME as his_consume_datatime2
