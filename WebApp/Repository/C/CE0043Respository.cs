...
{
    public class CE0043Repository : JCLib.Mvc.BaseRepository
    {
        public CE0043Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<CE0043Data> GetDetails(string chk_no, string chk_level, string wh_kind, string condition, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();
            ...
            string sql = string.Format(@"
                ...
                select a.mmcode, a.chk_no, a.mmname,a.store_qty, a.CHK_QTY,  
                       ...
                       round((a.pro_los_qty) *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 2) as diff_amount,
                       (case when
            ...
            return DBWork.Connection.Query<CE0043Data>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }

        public CE0043Count GetCountAll(string chk_no, string chk_level, string wh_kind)
        {
            var p = new DynamicParameters();
            ...
            string sql = string.Format(@"
                ...
                cost_data as (
                    select a.mmcode, a.last_qty, b.set_ym, b.DISC_CPRICE,
                           (a.store_qty) as store_qty,
                           (select uprice from MI_MAST where mmcode = a.mmcode) as m_contprice,
                           (case
                              when status_tot = '1' then chk_qty1
                ...
                select round(sum(chk_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end) ), 2) as tot1,
                       round(sum(store_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end)), 2) as tot2,
                       TRUNC ( SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)) / 
                               decode (SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)),
                                        0, 1 ,
                                        SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)))
                                * 100, 5) AS tot3,
                       round(SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS tot4,
                       round(SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS tot5
                  from cost_data
            ", wh_kind == "E" || wh_kind == "C" ? "_EC" : string.Empty
            , wh_kind == "E" || wh_kind == "C" ? "   and b.mc_id = '1'" : string.Empty);
            ...
            return DBWork.Connection.QueryFirst<CE0043Count>(sql, p, DBWork.Transaction);
        }

        public CE0043Count GetCountP(string chk_no, string chk_level, string wh_kind)
        {
            var p = new DynamicParameters();
            ...
            string sql = string.Format(@"
                ...
                select round(sum(chk_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end) ), 2) as p_tot1,
                       round(sum(store_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end)), 2) as p_tot2,
                       TRUNC ( SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)) / 
                               decode (SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)),
                                        0, 1 ,
                                        SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)))
                                * 100, 5) AS p_tot3,
                       round(SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS p_tot4,
                       round(SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS p_tot5
                  from cost_data
            ", wh_kind == "E" || wh_kind == "C" ? "_EC" : string.Empty
            , wh_kind == "E" || wh_kind == "C" ? "   and b.mc_id = '1'" : string.Empty);
            ...
            IEnumerable<CE0043Count> results = DBWork.Connection.Query<CE0043Count>(sql, p, DBWork.Transaction);
            if (results.Any() == false)
            {
                return new CE0043Count();
            }

            return results.First();
        }
        public CE0043Count GetCountN(string chk_no, string chk_level, string wh_kind)
        {
            var p = new DynamicParameters();
            ...
            string sql = string.Format(@"
                ...
                select round(sum(chk_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end) ), 2) as n_tot1,
                       round(sum(store_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end)), 2) as n_tot2,
                       TRUNC ( SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)) / 
                               decode (SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)),
                                        0, 1 ,
                                        SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 1)))
                                * 100, 5) AS n_tot3,
                       round(SUM (nvl(pro_los_qty *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS n_tot4,
                       round(SUM (nvl(apl_outqty  *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end), 0)), 2) AS n_tot5
                  from cost_data
            ", wh_kind == "E" || wh_kind == "C" ? "_EC" : string.Empty
            , wh_kind == "E" || wh_kind == "C" ? "   and b.mc_id = '1'" : string.Empty);
        ...
        public IEnumerable<CE0043Data> PrintData(string chk_no, string chk_level, string wh_kind, string condition)
        {
            var p = new DynamicParameters();
            ...
            string sql = string.Format(@"
                ...
                select a.mmcode, a.chk_no, a.mmname,a.store_qty, a.CHK_QTY,  
                       (pro_los_qty) as diff_qty,
                        (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end) as m_contprice,
                       round((a.pro_los_qty) *  (case when is_current_ym = 'Y' then m_contprice else DISC_CPRICE end)) as diff_amount,
                       (case when
                               decode(apl_outqty, 0 ,   null,apl_outqty) is null
                               then a.pro_los_qty
                               else 
            ...
            return DBWork.Connection.Query<CE0043Data>(sql, p, DBWork.Transaction);
        }
