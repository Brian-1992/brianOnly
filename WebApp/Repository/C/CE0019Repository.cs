...
namespace WebApp.Repository.C
{
    ...
    public class CE0019Repository : JCLib.Mvc.BaseRepository
    {
        public CE0019Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<CE0019> GetAll(string wh_no, string date, string userId, int page_index, int page_size, string sorters)
        {
            ...
            sql += "       AND CHK_LEVEL = '1' AND a.chk_no =b.chk_no AND a.CHK_YM =b.CHK_YM and b.chk_uid = :p2";
            ...
            return DBWork.Connection.Query<CE0019>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }

        //GET T2Grid
        public IEnumerable<CHK_CE0019VM> GetAllINI(string chk_no, string userId, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();
            ...
            var sql = @"select distinct  c.MMCODE, c.MMNAME_C,c.MMNAME_E,c.BASE_UNIT,a.CHK_QTY, 
                               ...
                               (select seq from CHK_G2_WHINV where chk_no = :chk_no and mmcode = a.mmcode) as seq,
                               (select una from UR_ID where tuser = a.chk_uid) as chk_uid_name, 
                               a.CHK_TIME,b.updn_status 
            ...
            return DBWork.Connection.Query<CHK_CE0019VM>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }
...
(select TWN_DATE(chk_pre_date) from CHK_G2_WHINV where chk_no = :chk_no and mmcode = a.mmcode) as 預計盤點日期,
        ...
        public IEnumerable<CE0019> UPDN_STATUSGet(string chk_no)
        {
            var p = new DynamicParameters();
            ...
            return DBWork.Connection.Query<CE0019>(sql, p, DBWork.Transaction);
        }


        public IEnumerable<CE0019> Get(string chk_no, string mmocde, string store_loc)
        {
            var sql = @"SELECT * FROM CHK_DETAIL WHERE chk_no = :CHK_NO and mmcode = :MMCODE and store_loc = :STORE_LOC";
            return DBWork.Connection.Query<CE0019>(sql, new { CHK_NO = chk_no, MMCODE = mmocde, STORE_LOC = store_loc }, DBWork.Transaction);
        }
        ...
        public IEnumerable<CE0019> SelectFinalPro(string chk_no, string userId)
        {
            var sql = @"Select * FROM CHK_G2_DETAIL WHERE chk_no = :CHK_NO and chk_uid = :CHK_UID and CHK_QTY is NULL";
            return DBWork.Connection.Query<CE0019>(sql, new { CHK_NO = chk_no, CHK_UID = userId }, DBWork.Transaction);
        }
