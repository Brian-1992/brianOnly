...
{
    public class BC0002Repository : JCLib.Mvc.BaseRepository
    {
        public class BOSS : JCLib.Mvc.BaseModel
        {
            public string BOSS_INID { get; set; }
            public string BOSS_NAME { get; set; }
            public string BOSS_ID { get; set; }
        }
        public BC0002Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<PH_SMALL_M> GetMasterAll(string dn, string apptime_bg, string apptime_ed, string status, string tuser, string qtype, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @" select A.DN, A.ACCEPT, A.AGEN_NAMEC, A.AGEN_NO, A.ALT, A.SIGNDATA,
                                ...
                                (case when CREATE_USER='緊急醫療出貨' then '緊急醫療結驗後轉單' else '' end) as memo, --備註
                                (case when CREATE_USER='緊急醫療出貨' then 'Y' else 'N' end) as is_CR
            ...
            if (qtype == "Audit2") // 小額採購消審會審核
            {
                //sql += " AND APP_INID = (select INID from UR_ID where TUSER = :TUSER)";
                //" AND STATUS <> 'A' AND STATUS <> 'B' ";
            }
            else if (qtype == "AuditNRS") // 小額採購申請單位護理部審核
            {
                sql += @" AND (substr(APP_INID,1,2) = (select substr(INID,1,2) from UR_ID where TUSER = :TUSER)
                                or (substr(APP_INID, 1, 2) = '35' and(select substr(INID, 1, 2) from UR_ID where TUSER = :TUSER) = '33' and STATUS = :p3)
                               ) ";
            }
            else if (qtype == "Audit") // 小額採購申請單位審核, 抓 PH_SMALL_M[NEXT_USER]
            {
                //sql += " AND (APP_INID = (select INID from UR_ID where TUSER = :TUSER)" +
                //    "OR INID = (select INID from UR_ID where TUSER = :TUSER))";
                //" AND STATUS <> 'A' ";
                //sql += " AND APP_INID = (select INID from UR_ID where TUSER = :TUSER) ";
                sql += " AND NEXT_USER = :TUSER ";
            }
            else if (qtype == "New") // 小額採購申請
            {
                sql += " AND APP_INID = (select INID from UR_ID where TUSER = :TUSER) ";
            }
            p.Add(":TUSER", tuser);
        ...
        {
            var sql = @" select A.DN, A.ACCEPT, A.AGEN_NAMEC, A.AGEN_NO, A.ALT, '' NRSFLAG, A.SIGNDATA, 
                        (select INID_NAME from UR_INID where INID = A.APP_INID) as APP_INID, 
                        (select UNA from UR_ID where TUSER = A.APP_USER) as APP_USER, 
                        (select UNA from UR_ID where TUSER = A.APP_USER1) as APP_USER1, 
                        A.APPTIME, A.APPTIME1, 
        ...
        public IEnumerable<BOSS> GetBOSS(string bossname, string qry, string app_inid, int page_index, int page_size, string sorters)
        {
            var sql = "";
            if (qry == "Y") // 按[查詢] 抓 ur_id
                sql = @" select tuser as BOSS_ID, una as BOSS_NAME, inid as BOSS_INID
                        FROM ur_id WHERE FL=1 and una like :bossname 
                        order by una ";
            else
                sql = @" select distinct tuser as BOSS_ID, una as BOSS_NAME, inid as BOSS_INID
                         FROM ph_small_m a, ur_id b
                         WHERE a.next_user=b.tuser and b.FL=1 and a.app_inid=:app_inid 
                         order by una ";
            var p = new DynamicParameters();
            p.Add("bossname", bossname + "%");
            p.Add("app_inid", app_inid);
            p.Add("OFFSET", (page_index - 1) * page_size);
            p.Add("PAGE_SIZE", page_size);
            return DBWork.Connection.Query<BOSS>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }
        ...
        public int MasterAudit(PH_SMALL_M ph_small_m)
        {
            ...
            {
                sign = " 呈核業務督導";
                ph_small_m.NEXT_USER = null;
            }
            var sql = @"UPDATE PH_SMALL_M SET STATUS='B', NEXT_USER=:NEXT_USER,
                                SIGNDATA = TWN_TIME_FORMAT(sysdate) ||' '||(select UNA from UR_ID where TUSER = :UPDATE_USER) || '" + sign + "' || chr(13) || chr(10) || SIGNDATA ,";
            sql += " UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP WHERE DN = :DN AND (STATUS = 'A' OR STATUS = 'D') ";
            return DBWork.Connection.Execute(sql, ph_small_m, DBWork.Transaction);
        }
        public string getMAIL_ADDRESS(string urid)
        {
            var sql = @"select email from UR_ID where tuser =:tuser";
            return DBWork.Connection.QueryFirst<string>(sql, new { tuser = urid }, DBWork.Transaction);
        }
        public string getNRSMAIL_ADDRESS(string sendto)
        {
            var sql = @"select MAIL_ADD from PH_SMALL_MAIL where SEND_TO =:SEND_TO";
            return DBWork.Connection.QueryFirst<string>(sql, new { SEND_TO = sendto }, DBWork.Transaction);
        }
        public string getUser_MAIL_ADDRESS(string dn)
        {
            var sql = @"select email from PH_SMALL_M a, UR_ID b 
                        where a.app_user=b.tuser and dn =:dn";
        ...
        public IEnumerable<PH_SMALL_D> GetDetailAll(string dn, string p1, int page_index, int page_size, string sorters)
        {
            ...
            if (p1 == "0")
            {
                sql = @"SELECT A.SEQ, A.DN, A.MEMO, A.MMCODE, A.MMCODE as OLD_MMCODE,
                        (select DATA_VALUE || ' ' || DATA_DESC from PARAM_D 
                        where GRP_CODE='PH_SMALL_M' and DATA_NAME='CHARGE' and DATA_VALUE=A.CHARGE ) as CHARGE, 
                        (select INID || ' ' || INID_NAME from UR_INID where INID = A.INID) as INID, A.INID as OLD_INID, 
                        (select INID_NAME from UR_INID where INID = A.INID) as INIDNAME, 
                        A.NMSPEC, A.NMSPEC as OLD_NMSPEC, A.PRICE, A.QTY, A.UNIT, A.QTY * A.PRICE as TOTAL_PRICE,
        ...
        {
            var sql = @" SELECT A.SEQ, A.DN, A.MEMO, A.MMCODE, A.MMCODE as OLD_MMCODE,
                        (select DATA_VALUE || ' ' || DATA_DESC from PARAM_D 
                        where GRP_CODE='PH_SMALL_M' and DATA_NAME='CHARGE' and DATA_VALUE=A.CHARGE ) as CHARGE, 
                        (select INID || ' ' || INID_NAME from UR_INID where INID = A.INID) as INID, A.INID as OLD_INID,  
                        (select INID_NAME from UR_INID where INID = A.INID) as INIDNAME, 
        ...
        public DataTable GetMmcodeData(string mmcode)
        {
            var sql = @"select MMNAME_C, BASE_UNIT, nvl(UPRICE,0) UPRICE, M_PAYKIND from MI_MAST
                            where MMCODE=:MMCODE";
            DataTable dt = new DataTable();
            using (var rdr = DBWork.Connection.ExecuteReader(sql, new { MMCODE = mmcode }, DBWork.Transaction))
                dt.Load(rdr);
            return dt;
        }
        ...
        public int DetailFileDelete(string dn)
        {
            var sql = @"delete from PH_ATTFILE   where DN = :DN ";
            return DBWork.Connection.Execute(sql, new { DN = dn }, DBWork.Transaction);
        }

        public DataTable getATTFileUK(string dn)
        {
            var sql = @"select FTP_NAME as UK from PH_ATTFILE   where dn =:dn";
            DataTable dt = new DataTable();
            using (var rdr = DBWork.Connection.ExecuteReader(sql, new { dn = dn }, DBWork.Transaction))
                dt.Load(rdr);
            return dt;
        }
        // 檢查申請單號是否存在
        ...
        // 檢查申請單號+NMPESC+需求單位是否存在 for 院內碼空白
        public bool CheckDetailExist1(string dn, string inid, string nmspec)
        {
        ...
        // 檢查申請單號+院內碼+需求單位是否存在
        public bool CheckDetailExist2(string dn, string inid, string mmcode)
        {
            string sql = @"SELECT 1 FROM PH_SMALL_D 
                           WHERE DN=:DN and INID=:INID and MMCODE=:MMCODE ";
            return !(DBWork.Connection.ExecuteScalar(sql, new { DN = dn, INID = inid, MMCODE = mmcode }, DBWork.Transaction) == null);
        }
        ...
        public IEnumerable<MI_MAST> GetMmcodeCombo(string p0, int page_index, int page_size, string sorters)
        {
            ...
            string sql = @"select {0} MMCODE, MMNAME_C, MMNAME_E,
                            NVL( ( SELECT MIN_ORDQTY FROM MI_WINVCTL WHERE MMCODE = A.MMCODE AND WH_NO ='560000' AND ROWNUM=1 ) ,1) AS TOT_DISTUN
                            from MI_MAST a where mat_class='02' and (CANCEL_ID <> 'Y' or CANCEL_ID is null) and M_CONTID = '3' ";
        ...
        public bool CheckMmcodeExists(string MMCODE)
        {
            ...
            else
            {
                ...
                ret = !(DBWork.Connection.ExecuteScalar(sql, new { MMCODE = MMCODE }, DBWork.Transaction) == null);
            }
            return ret;
        }
        // 檢查[需求單位]是否正確
        ...
        #region 2020-11-03: 無明細不可送審
        public int GetDetailCount(string dn) {
            string sql = @"select count(*) from PH_SMALL_D where dn = :dn";
            return DBWork.Connection.QueryFirstOrDefault<int>(sql, new { dn = dn }, DBWork.Transaction);
        }
        #endregion

        #region 2023-09-07 取得最小撥補量
        public string GetMinOrqty(string mmcode) {
            string sql = @"
               select  NVL( ( SELECT MIN_ORDQTY FROM MI_WINVCTL WHERE MMCODE = :mmcode AND WH_NO ='560000' AND ROWNUM=1 ) ,1) as min_ordqty from dual
            ";
