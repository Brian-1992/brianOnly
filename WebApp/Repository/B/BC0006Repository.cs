...
{
    public class BC0006Repository : JCLib.Mvc.BaseRepository
    {
        public BC0006Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        {
            var sql = @"UPDATE PH_SMALL_M SET STATUS = 'H', 
                                SIGNDATA = TWN_TIME_FORMAT(sysdate) ||' '||(select UNA from UR_ID where TUSER = :UPDATE_USER) ||  ' 陳核護理部業務副主任。' || chr(13) || chr(10) || SIGNDATA,
                                UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP
                                WHERE DN = :DN AND STATUS = 'G' ";
            return DBWork.Connection.Execute(sql, ph_small_m, DBWork.Transaction);
        }
