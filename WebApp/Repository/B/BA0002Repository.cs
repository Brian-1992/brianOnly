...
{
    public class BA0002Repository : JCLib.Mvc.BaseRepository
    {
        public BA0002Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<MM_PR_M> GetMST(string MAT_CLASS, string PR_TIME_B, string PR_TIME_E, string M_STOREID, string XACTION, string PR_DEPT, int page_index, int page_size, string sorters)
        {

            var p = new DynamicParameters();

            string sql = @"Select a.pr_no, a.pr_dept, a.pr_time, a.pr_user, a.mat_class,
                                  a.m_storeid, pr_status, create_time, create_user, update_time, update_user, update_ip,
                                  a.xaction, a.sdn, 
                                  (case when pr_user='緊急醫療出貨' then '緊急醫療出貨' else '' end) as memo,
                                  (case when pr_user='緊急醫療出貨' then 'Y' else 'N' end) as is_cr
                             from mmsadm.MM_PR_M a
                            where PR_DEPT= :PR_DEPT  and XACTION=:XACTION ";
        ...
        public IEnumerable<MM_PR_D> GetD(string PR_NO, string MMCODE, string WH_NO, int page_index, int page_size, string sorters)
        {
            ...
            string sql = @"select D.mmcode, nvl(D.M_CONTPRICE,0) as M_CONTPRICE, D.unit_swap, PR_PRICE, D.agen_no, D.agen_name, D.agen_fax ,  D.PR_NO, D.PR_QTY, D.REQ_QTY_T,
                            A.mmname_c, A.mmname_e,A.M_CONTID,A.M_STOREID, A.BASE_UNIT, D.M_PURUN , floor(nvl(D.M_CONTPRICE,0)*D.REQ_QTY_T) as TOTAL_PRICE, 0 as  TOT,
                            ...
                            from MM_PR_M M, MM_PR_D D , V_MI_MAST A
        ...
        public string MasterCreate(MM_PR_M mm_pr_m)
        {
            ...
            //If (物料類別 == ’02’)
            if (mm_pr_m.MAT_CLASS == "02")
            {
                //  If (申購日期起(迄)在 22~31)
                if (DateTime.Today.Day >= TXTDAY["02_BDAY"] && DateTime.Today.Day <= TXTDAY["02_EDAY"])
                {
                    sql = @" select count(1) from mmsadm.MM_PR_M where SUBSTR(PR_NO, 20, 2) ='02'
                    ...
                    if (int.Parse(DBWork.Connection.ExecuteScalar(sql, mm_pr_m, DBWork.Transaction).ToString()) == 0)
                    {
                        ...
                        sql = @" insert into mmsadm.MM_PR_M(PR_NO         , PR_DEPT, PR_TIME,  PR_USER , MAT_CLASS, M_STOREID, PR_STATUS, CREATE_TIME, CREATE_USER, UPDATE_IP,XACTION) 
                                            values(" + v_pr_no + ", :PR_DEPT, sysdate, :PR_USER,:MAT_CLASS,:M_STOREID, '35'     , sysdate,    :CREATE_USER, :UPDATE_IP,'1')";
                ...
                {
                    //提示 {衛材每月22日起開放下月常態申購!! }
                    return "衛材每月" + TXTDAY["02_BDAY"] + " - " + TXTDAY["02_EDAY"]+ "日開放下月常態申購!!";
                }
            }
            else // 非 ’02’
            {
                //  If (申購日期起(迄)在 13~19)
                if (DateTime.Today.Day >= TXTDAY["0308_BDAY"] && DateTime.Today.Day <= TXTDAY["0308_EDAY"])
                {
                    sql = @" select count(1) from MM_PR_M where SUBSTR(PR_NO, 20, 2) =:MAT_CLASS
                    and substr(PR_NO,7,7) >= TWN_DATE(sysdate)
                    and substr(PR_NO,7,7) <= TWN_DATE(sysdate)
                    and PR_DEPT = :PR_DEPT
                    and M_STOREID = :M_STOREID
                ...
                {
                    // 提示 { 一般物品每月13 - 19日開放下月常態申購!! }
                    return "一般物品每月"+ TXTDAY["0308_BDAY"] + " - "+ TXTDAY["0308_EDAY"] + "日開放下月常態申購!!";
                }
        ...
        public IEnumerable<MM_PR_D> GetSelectMmcodeDetail(string MMCODE, string MAT_CLASS, string WH_NO, string M_STOREID)
        {
            ...
            string sql = @"select M.mmcode, M.mat_class,M.M_STOREID, M.mmname_c, M.mmname_e, 0 as  TOTAL_PRICE, 0 as  TOT,
                            V.agen_no, V.agen_namec, V.agen_fax, M.M_DISCPERC, (select inv_qty from MI_WHINV where wh_no=:WH_NO and mmcode=m.mmcode and ROWNUM <= 1) inv_qty , 
                            M.M_CONTID, '' as PR_PRICE, M.base_unit,M.M_PURUN, M.UPRICE,
                            nvl(EXCH_RATIO,1) as unit_swap , nvl(M.M_CONTPRICE,0) as M_CONTPRICE
                            from V_MI_MAST M, PH_VENDER V
        ...
        public IEnumerable<MI_MAST> GetMmcodeCombo(string MAT_CLASS, string m_storeid, string p0, int page_index, int page_size, string sorters)
        {
            ...
            string sql = @"select {0} MMCODE, MMNAME_C, MMNAME_E
                            from V_MI_MAST where mat_class=:mat_class and M_CONTID<>'3'
                             and m_storeid = :m_storeid";
