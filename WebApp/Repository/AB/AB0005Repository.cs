...
{
    public class AB0005Repository : JCLib.Mvc.BaseRepository
    {
        public AB0005Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<ME_DOCM> GetAllM(string DOCNO, string APPTIME1, string APPTIME2, string[] str_FLOWID, string APPLY_KIND, string MAT_CLASS, string INID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE = 'MR3' ";
        ...
        public IEnumerable<ME_DOCD> GetAllD(string DOCNO, string MMCODE, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.DOCNO, A.SEQ, A.MMCODE , A.APPQTY , A.APLYITEM_NOTE, 
                        ...
                        NVL(( SELECT AVG_APLQTY FROM V_MM_AVGAPL WHERE MMCODE = A.MMCODE AND WH_NO=C.TOWH ),0) AS AVG_APLQTY,
                        ...
                        NVL( b.min_ordqty ,1) AS TOT_DISTUN,
                        (select PFILE_ID from MI_MAST where mmcode = a.mmcode) as PFILE_ID  
                        , A.CREATE_USER, USER_NAME(A.CREATE_USER) as create_user_name
                        FROM ME_DOCD A 
                        INNER JOIN V_MI_MAST B ON B.MMCODE=A.MMCODE 
        ...
        public IEnumerable<ME_DOCD> GetD(string id,string seq)
        {
            var sql = @"SELECT A.DOCNO, A.SEQ, A.MMCODE , A.APPQTY , A.APLYITEM_NOTE, 
                        ...
                        (case when A.AVG_PRICE is null then NVL(( SELECT AVG_PRICE FROM MI_WHCOST WHERE MMCODE = A.MMCODE AND DATA_YM=CUR_SETYM AND ROWNUM=1 ),0) else A.AVG_PRICE end) AS AVG_PRICE,
                        NVL(( SELECT INV_QTY FROM MI_WHINV WHERE MMCODE = A.MMCODE AND WH_NO=C.FRWH ),0) AS INV_QTY,
                        NVL(( SELECT AVG_APLQTY FROM V_MM_AVGAPL WHERE MMCODE = A.MMCODE AND WH_NO=C.FRWH ),0) AS AVG_APLQTY,
                        NVL(( SELECT SAFE_QTY FROM MI_WINVCTL WHERE MMCODE = A.MMCODE AND WH_NO=C.TOWH ),0) AS SAFE_QTY,
                        NVL(( SELECT HIGH_QTY FROM MI_WINVCTL WHERE MMCODE = A.MMCODE AND WH_NO=C.TOWH ),0) AS HIGH_QTY,
                        NVL(E.TOT_APVQTY,0) TOT_APVQTY,
                        NVL( b.min_ordqty ,1) AS TOT_DISTUN,
        ...
        {
            var sql = @"UPDATE ME_DOCD SET EXPT_DISTQTY = APPQTY,
        ...
        public IEnumerable<ME_DOCD> GetPackD(string DOCNO, string DOCNO2, string MAT_CLASS, string APPDEPT, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.MMCODE ,A.APPQTY,
                          ...
                          FROM MM_PACK_D A,V_MI_MAST B, MM_PACK_M C 
        ...
        public IEnumerable<ME_DOCD> GetSaveD(string MAT_CLASS, string APPDEPT, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT P.*,
                        ...
                        FROM (
                        SELECT A.MMCODE, 
                        CEIL((B.SAFE_QTY+B.OPER_QTY+B.SHIP_QTY-A.INV_QTY)/NVL( c.min_ordqty ,1))*NVL( c.min_ordqty ,1) APPQTY,
                        ...
                        FROM MI_WHINV A,MI_WINVCTL B, V_MI_MAST C
        ...
        {
            string sql = @"SELECT 1 FROM V_MI_MAST WHERE MMCODE=:MMCODE
                            AND M_STOREID = '0'
                            AND M_CONTID <> '3'
                            AND M_APPLYID <> 'E'
                            AND EXISTS ( SELECT 1 FROM ME_DOCM WHERE MAT_CLASS=V_MI_MAST.MAT_CLASS AND DOCNO=:DOCNO)
                        ";
        ...
        public string CheckApplyKind()
        {
            string sql = @"SELECT NVL((SELECT 'Y' FROM DUAL WHERE (SELECT EXTRACT(DAY FROM SYSDATE) FROM DUAL) 
                            BETWEEN (SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR3_BDAY')
                            AND (SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR3_EDAY')),'N') MR3
                            FROM DUAL ";
        ...
        public int CheckApplyKindNum(string id, string inid)
        {
            string sql = @"SELECT COUNT(*) AS CNT FROM ME_DOCM 
                            WHERE DOCTYPE='MR3' AND APPLY_KIND='1' 
                            AND twn_date(APPTIME) BETWEEN twn_yyymm(sysdate)||LPAD((SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR3_BDAY'), 2, '0') 
                                                      AND twn_yyymm(sysdate)||LPAD((SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR3_EDAY') , 2, '0')
        ...
        public string GetApplyKindDocno(string id, string inid)
        {
            string sql = @"SELECT DOCNO FROM ME_DOCM 
                            WHERE DOCTYPE='MR3' AND APPLY_KIND='1' 
                            AND APPTIME BETWEEN trunc((sysdate),'month') AND last_day(sysdate)  
                            AND MAT_CLASS = :MAT_CLASS  AND TOWH = :INID AND ROWNUM=1";
            string rtn = DBWork.Connection.ExecuteScalar(sql, new { MAT_CLASS = id, INID = inid }, DBWork.Transaction).ToString();
            return rtn;
        }
        ...
        public IEnumerable<MI_MAST> GetMmCodeCombo(string p0, string p1, string p2, string docno, string p4, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT {0} A.MMCODE, A.MMNAME_C, A.MMNAME_E, A.MAT_CLASS, A.BASE_UNIT, A.DISC_UPRICE as M_CONTPRICE,
                               ...
                               NVL(E.TOT_APVQTY,0) TOT_APVQTY ,
                               NVL( a.min_ordqty ,1) AS TOT_DISTUN,
                               (select PFILE_ID from MI_MAST where mmcode = a.mmcode) as PFILE_ID,
                               (case
                                    when ((select 1 from dual where not exists (select 1 from MI_WHMM where mmcode = a.mmcode)
                                              union
        ...
        public IEnumerable<MI_MAST> GetMMCodeDocd(string p0, string p1, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT {0} A.MMCODE, A.MMNAME_C, A.MMNAME_E  
                        FROM V_MI_MAST A, ME_DOCD B WHERE A.MMCODE=B.MMCODE AND B.DOCNO = :DOCNO  ";
            if (p0 != "")
            {
        ...
        public IEnumerable<COMBO_MODEL> GetMatclassCombo()
        {
            string sql = @"select MAT_CLASS AS VALUE, MAT_CLSNAME AS TEXT, 
                        ...
                        (CASE WHEN MAT_CLASS = '08' THEN '2' ELSE '1' END) EXTRA1  
        ...
        {
            string sql = @" SELECT 1 FROM V_MI_MAST WHERE 1=1 
                          AND MMCODE = :MMCODE";
        ...
        {
            string sql = @" SELECT 1 FROM V_MI_MAST A 
                            WHERE 1=1 AND A.MMCODE=:MMCODE 
                            AND mod(:APPQTY , NVL( min_ordqty ,1)) > 0";
        ...
        #region 2022-06-02 新增: 送核撥檢查是否為本月可申請期間
        public bool CheckApplyable() {
            string sql = @"
                  with bday as (
                    select to_date(to_char(sysdate,'yyyymm')||data_value,'yyyymmdd') as bday 
                      from PARAM_D
                     where data_name = 'MR3_BDAY'
                  ), eday as (
                    select to_date(to_char(sysdate,'yyyymm')||data_value,'yyyymmdd') as eday 
                      from PARAM_D
                     where data_name = 'MR3_EDAY'
                  )
                  select 1 
                    from bday a, eday b
                   where trunc(sysdate) between a.bday and b.eday
            ";

            return DBWork.Connection.ExecuteScalar(sql, DBWork.Transaction) != null;
        }

        public bool CheckApplyable(string docno)
        {
            string sql = @"
                  with bday as (
                    select to_date(to_char(sysdate,'yyyymm')||data_value,'yyyymmdd') as bday 
                  ...
                  select 1 
                    from ME_DOCM c, bday a, eday b
                   where c.docno = :docno
                     and (   (trunc(sysdate) between a.bday and b.eday
                              and trunc(c.apptime) between a.bday and b.eday
                              and c.apply_kind ='1')
                          or (c.apply_kind = '2')
                         )
            ";
        ...
        public IEnumerable<ME_DOCD> CheckMstoreid(string docno, string m_storeid)
        {
            string sql = @"
                select * from ME_DOCD a
                 where docno = :docno
                   and exists (select 1 from V_MI_MAST 
        ...
        public IEnumerable<ME_DOCD> CheckMinOrderQty(string docno)
        {
            string sql = @"
                with min_ordqtys as (
                    select a.mmcode, 
                           nvl( b.min_ordqty ,1) as min_ordqty
                     from ME_DOCD a, V_MI_MAST b
                    where a.docno = :docno
