...
{
    public class AB0008Repository : JCLib.Mvc.BaseRepository
    {
        public AB0008Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_DOCM> GetAllM(string DOCNO, string APPTIME1, string APPTIME2, string[] str_FLOWID, string MAT_CLASS, string jcn, string towh, string INID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='FLOWID_CM1' AND DATA_VALUE=A.FLOWID) FLOWID_N,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE = 'CM1' ";
        ...
        public IEnumerable<ME_DOCD_EC> GetAllD(string DOCNO, string MMCODE, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        ( SELECT INV_QTY FROM MI_WHINV WHERE MMCODE = A.MMCODE AND WH_NO IN (
                          SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='C' AND WH_GRADE='1') ) A_INV_QTY,
                        ( SELECT INV_QTY FROM MI_WHINV WHERE MMCODE = A.MMCODE AND WH_NO IN (
                          SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='C' AND WH_GRADE='M') ) M_INV_QTY,
                        ( SELECT INV_QTY FROM MI_WHINV WHERE MMCODE = A.MMCODE AND WH_NO IN (
                          SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='C' AND WH_GRADE='S') ) S_INV_QTY,
                        F.APVQTY A_APV_QTY,
                        ...
                        LEFT OUTER JOIN MM_ECUSE F ON F.MMCODE=A.MMCODE AND F.USE_DEPT=E.TOWH AND EC_ID='C' AND USE_YM=SUBSTR(TWN_DATE(E.APPTIME),0,5)
                        WHERE 1 = 1 ";
        ...
        public IEnumerable<COMBO_MODEL> GetFlowidCombo()
        {
            string sql = @"SELECT DISTINCT DATA_VALUE as VALUE, DATA_DESC as TEXT ,
                        ...
                        FROM PARAM_D
                        WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='FLOWID_CM1' 
                        ORDER BY DATA_VALUE";
        ...
        public IEnumerable<COMBO_MODEL> GetMatclassCombo()
        {
            string sql = @"SELECT MAT_CLASS AS VALUE, MAT_CLSNAME AS TEXT, 
                        ...
                        FROM MI_MATCLASS WHERE MAT_CLSID='5'   
                        ORDER BY MAT_CLASS";
