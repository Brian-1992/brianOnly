...
{
    public class AB0017Repository : JCLib.Mvc.BaseRepository
    {
        public AB0017Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_DOCM> GetAllM(string DOCNO, string APPTIME1, string APPTIME2, string[] str_FLOWID, string MAT_CLASS, string INID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        (SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='FLOWID_CM2' AND DATA_VALUE=A.FLOWID) FLOWID_N,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE = 'CM2' ";
        ...
        public string GetTaskid(string id)
        {
            string sql = @"SELECT TASK_ID FROM MI_WHID WHERE WH_USERID=:WH_USERID 
                            AND WH_NO IN (SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='C')";
        ...
        public IEnumerable<COMBO_MODEL> GetFlowidCombo()
        {
            string sql = @"SELECT DISTINCT DATA_VALUE as VALUE, DATA_DESC as TEXT ,
                        ...
                        FROM PARAM_D
                        WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='FLOWID_CM2' 
                        ORDER BY DATA_VALUE";
