...
{
    public class AB0023Repository : JCLib.Mvc.BaseRepository
    {

        public AB0023Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_BACK> GetMeBack(string docno, string rseq, string ordersort)
        {
            ...
            string sql = @"SELECT
                                ...
                                and DATA_VALUE=a.ORDERSORT) as ORDERSORT
                            FROM 
                                ME_BACK a, ME_DOCD b
                            WHERE 1 = 1 
            ...
            if (ordersort != null && ordersort != "")
            {
                sql += " AND a.ORDERSORT = :ORDERSORT ";
                p.Add(":ORDERSORT", ordersort);
            }
            

            return DBWork.Connection.Query<ME_BACK>(sql, p, DBWork.Transaction);
        }

        public IEnumerable<MI_WHMAST> GetTowh(string id)
        {
            var sql = @"SELECT WH_NO,WH_NO || ' ' || WH_NAME(WH_NO) AS WH_NAME FROM MI_WHID a
                        WHERE WH_USERID=:TUSER
                        and exists (select 1 from MI_WHMAST where WH_KIND='0' and WH_NO=a.WH_NO)
                        order by WH_NO ";
            return DBWork.Connection.Query<MI_WHMAST>(sql, new { TUSER = id }, DBWork.Transaction);
        }
        public IEnumerable<MI_WHMAST> GetFrwh()
        {
            var sql = @"SELECT WH_NO,WH_NO || ' ' || WH_NAME(WH_NO) AS WH_NAME,WH_GRADE FROM MI_WHMAST WHERE WH_KIND='0' AND WH_GRADE  IN ('3','4') order by WH_GRADE,WH_NO";
        ...
        public int CheckDocdStatB(string docno) {
