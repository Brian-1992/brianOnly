...
{
    public class AB0006Repository : JCLib.Mvc.BaseRepository
    {
        public AB0006Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_DOCM> GetAllM(string DOCNO, string APPTIME1, string APPTIME2, string[] str_FLOWID, string APPLY_KIND, string INID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND DOCTYPE = 'MR4' ";
            if (INID != "")
            {
        ...
        public IEnumerable<ME_DOCD> GetAllD(string DOCNO, string MMCODE, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.DOCNO, A.SEQ, A.MMCODE , A.APPQTY , A.APLYITEM_NOTE, 
                        ...
                        (select PFILE_ID from MI_MAST where mmcode = b.mmcode) as PFILE_ID    
        ...
        public int CheckApplyKindNum(string id, string inid)
        {
            string sql = @"SELECT COUNT(*) AS CNT FROM ME_DOCM 
                            WHERE DOCTYPE='MR4' AND APPLY_KIND='1' 
                            AND twn_date(APPTIME) BETWEEN twn_yyymm(sysdate)||LPAD((SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR4_BDAY'), 2, '0') 
                                                      AND twn_yyymm(sysdate)||LPAD((SELECT DATA_VALUE FROM PARAM_D WHERE DATA_NAME = 'MR4_EDAY') , 2, '0')
        ...
        public string GetApplyKindDocno(string id, string inid)
        {
            string sql = @"SELECT DOCNO FROM ME_DOCM 
                            ...
                            and CREATE_USER<>'緊急醫療出貨'";
            string rtn = Convert.ToString(DBWork.Connection.ExecuteScalar(sql, new { MAT_CLASS = id, INID = inid }, DBWork.Transaction));
            return rtn;
        }
