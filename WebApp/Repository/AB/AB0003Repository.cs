...
{
    public class AB0003Repository : JCLib.Mvc.BaseRepository
    {
        public AB0003Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_DOCM> GetAllM(string DOCNO,string APPTIME1, string APPTIME2, string[] str_FLOWID, string APPLY_KIND, string MAT_CLASS, string INID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE = 'MR1' ";
            if (INID != "")
            {
        ...
        public IEnumerable<ME_DOCD> GetPackD(string DOCNO, string DOCNO2, string MAT_CLASS, string APPDEPT, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.MMCODE ,A.APPQTY,
                               ...
                               NVL( ( SELECT MIN_ORDQTY FROM MI_WINVCTL 
                                       WHERE MMCODE = A.MMCODE AND WH_NO ='560000' AND ROWNUM=1 ) ,1) AS TOT_DISTUN,
                               (case
        ...
        public bool CheckApplyKind(string id)
        {
            string sql = @"SELECT 1 FROM MM_WHAPLDT 
                            WHERE (SYSDATE <= 
                            (TO_DATE(TO_CHAR(APPLY_DATE-1,'YYYYMMDD')||'120000','YYYYMMDDHH24MISS')))
                            AND WH_NO=:WH_NO";
            return !(DBWork.Connection.ExecuteScalar(sql, new { WH_NO = id }, DBWork.Transaction) == null);
        }
        ...
        public int CheckApplyKindNum(string id, string inid)
        {
            string sql = @"SELECT COUNT(*) AS CNT FROM ME_DOCM 
                            WHERE DOCTYPE='MR1' AND APPLY_KIND='1' 
        ...
        public string GetApplyWhno(string id)
        {
            string sql = @"SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='1' AND INID=:INID ";
        ...
        public IEnumerable<ME_DOCD> CheckMApplyidE(string docno, bool isView) {
            string sql = string.Format(@"
                           select * from ME_DOCD a
                            where docno = :docno
                              and exists (select 1 from {0}MI_MAST 
                                            where mmcode = a.mmcode
                                              and (m_applyid = 'E' or M_CONTID = '3' or cancel_id = 'Y'))"
