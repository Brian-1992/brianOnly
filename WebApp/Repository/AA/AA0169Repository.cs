...
namespace WebApp.Repository.AA
{
    ...
    public class AA0169Report_MODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class AA0169Repository : JCLib.Mvc.BaseRepository
    {
        public AA0169Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<AA0169Report_MODEL> GetReport(string P0, string P1, string P2, string P4)
        {
            var p = new DynamicParameters();
            string sql = @"SELECT A.WH_NO,A.MMCODE,B.MMNAME_E,B.BASE_UNIT,C.DISC_CPRICE,SUM(A.APL_OUTQTY) APL_OUTQTY,
                           SUM(A.APL_OUTQTY * C.DISC_CPRICE) OUT_AMT,SUM(A.INV_QTY) SQTY,SUM(A.INV_QTY * C.DISC_CPRICE) INV_AMT, 
                           CASE WHEN SUM(A.APL_OUTQTY) = 0 THEN NULL ELSE ROUND(SUM(A.APL_INQTY) / SUM(A.APL_OUTQTY),0) END  RATIO
                           FROM MI_WINVMON A, MI_MAST B, MI_WHCOST C
            ...
            sql += " GROUP BY A.WH_NO,A.MMCODE,B.MMNAME_E,B.BASE_UNIT,C.DISC_CPRICE ";
            return DBWork.Connection.Query<AA0169Report_MODEL>(sql, p, DBWork.Transaction);
        }
