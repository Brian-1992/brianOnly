...
{
    public class AA0128Repository : JCLib.Mvc.BaseRepository
    {
        public AA0128Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<ME_DOCM> GetAllM(string apptime1, string apptime2, string towh, string[] str_FLOWID, string applykind, string matclass, string doctype, string task, int page_index, int page_size, string sorters)
        {
        ...
        public IEnumerable<ME_DOCD> GetAllD(string DOCNO, string MMCODE, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.DOCNO, A.SEQ, A.MMCODE , A.APPQTY , A.APLYITEM_NOTE, 
                        ...
                        E.TOT_APVQTY + A.APPQTY AS TOT_APVQTY,
                        ...
                        ( SELECT HIGH_QTY FROM MI_WINVCTL WHERE MMCODE = A.MMCODE AND WH_NO  IN (SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='1' AND WH_GRADE='1') AND ROWNUM=1) AS HIGH_QTY,
                        ...
                        (CASE WHEN (E.TOT_APVQTY + A.APPQTY) > ( SELECT HIGH_QTY FROM MI_WINVCTL WHERE MMCODE = A.MMCODE AND WH_NO  IN (SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='1' AND WH_GRADE='1') AND ROWNUM=1) THEN '1' ELSE '0' END) SGN,
        ...
        public int UpdateD(ME_DOCD ME_DOCD)
        {
            var sql = @"UPDATE ME_DOCD SET 
                        EXPT_DISTQTY = :EXPT_DISTQTY, BW_MQTY = :BW_MQTY, APVQTY = :APVQTY,
                        UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP,
        ...
        {
            var sql = @"UPDATE ME_DOCD SET EXPT_DISTQTY = :EXPT_DISTQTY, BW_MQTY = :BW_MQTY, APVQTY = :APVQTY,
                        APVTIME = SYSDATE,ACKTIME = SYSDATE, APVID = :UPDATE_USER,
                        UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP
                        WHERE DOCNO = :DOCNO AND SEQ = :SEQ";
            return DBWork.Connection.Execute(sql, ME_DOCD, DBWork.Transaction);
        }
        ...
        {
            string sql = @"SELECT 1 FROM ME_DOCM WHERE DOCNO=:DOCNO AND FLOWID <> '3'";
            return !(DBWork.Connection.ExecuteScalar(sql, new { DOCNO = id }, DBWork.Transaction) == null);
        }
        ...
        public string CallProc(string id, string code, string upuser, string upip)
        {
            ...
            DBWork.Connection.Query("POST_DOC_AIR", p, commandType: CommandType.StoredProcedure);
        ...
        {
            string sql = @" select
                            case when (SELECT count(*) FROM MI_WHID WHERE WH_USERID=:WH_USERID AND TASK_ID = '6'
                            AND WH_NO IN (SELECT WH_NO FROM MI_WHMAST WHERE WH_KIND='1' AND WH_GRADE='1')) > 0 then 
                            (SELECT TASK_ID FROM MI_WHID WHERE WH_USERID=:WH_USERID  AND TASK_ID = '6'
        ...
        public IEnumerable<COMBO_MODEL> GetDocnoCombo(string task)
        {
            string sql = @"SELECT DISTINCT A.DOCNO as VALUE, A.DOCNO as TEXT,
                        ...
                        WHERE A.MAT_CLASS=B.MAT_CLASS AND DOCTYPE = 'AIR'
        ...
        public IEnumerable<COMBO_MODEL> GetMatclassCombo()
        {
            string sql = @"select MAT_CLASS AS VALUE, MAT_CLSNAME AS TEXT, 
                        ...
                        FROM MI_MATCLASS WHERE MAT_CLSID IN( '6' )   
                        ORDER BY MAT_CLASS";
