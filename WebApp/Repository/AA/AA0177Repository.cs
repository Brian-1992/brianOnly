...
{
    public class AA0177Repository : JCLib.Mvc.BaseRepository
    {
        public AA0177Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        //查詢
        public IEnumerable<AA0177> GetAll(string MatClass, string MMCode, string p2, int page_index, int page_size, string sorters)
        {
            ...
            {
                if (MatClass == "01" || MatClass == "02")
                {
                    sql += " AND B.MAT_CLASS = :MAT_CLASS_SUB_TEXT ";
            ...
            return DBWork.Connection.Query<AA0177>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }
        ...
        public DataTable GetExcel(string MatClass, string MMCode, string p2)
        {
            ...
            var sql = @" SELECT A.MMCODE 院內碼, 
                                ...
                                A.SUPPLY_MAX_APPQTY 供應中心最大請領量,      
                                A.PHR_MAX_APPQTY 藥局請領最大量,      
                                A.WAR_QTY 戰備存量,      
                                A.SAFE_QTY 安全庫存量,      
                                A.NORMAL_QTY 正常庫存量,      
                                A.DIFF_PERC 誤差百分比,       
            ...
            if (!string.IsNullOrWhiteSpace(MatClass))
            {
                if (MatClass == "all01" || MatClass == "all02")
                {
                    sql += " AND B.MAT_CLASS = :MAT_CLASS_SUB_TEXT ";
                    p.Add(":MAT_CLASS_SUB_TEXT", MatClass.Replace("all", ""));
                }
                else
                {
                    sql += " AND B.MAT_CLASS_SUB = :MAT_CLASS_SUB_TEXT ";
        ...
        {
            string sql = @"SELECT 
                    data_value AS value, data_desc AS text, data_value || ' ' || data_desc as combitem, 99 as sort_num 
                FROM
                    param_d
                WHERE
                    grp_code = 'MI_MAST'
                ...
                UNION
                     SELECT ' ' AS value, '全部' AS text, '全部' AS combitem, 1 as sort_num FROM dual
                UNION 
                     SELECT '02' AS value, '全部衛材' AS text, '全部衛材' AS combitem, 2 as sort_num FROM dual
                UNION
                     SELECT '01' AS value, '全部藥品' AS text, '全部藥品' AS combitem, 3 as sort_num FROM dual
                    order by sort_num";
        ...
        public IEnumerable<MI_MAST> GetMMCodeCombo(string p0, string p1, int page_index, int page_size, string sorters)
        {
            ...
            if (p1.Trim() != "")
            {
                if (p1 == "01" || p1 == "02")
                {
                    sql += "     AND MAT_CLASS = TRIM(:MAT_CLASS_TEXT) ";
                }
                else
                {
        ...
        //修改
        public int Update(AA0177 AA0177)
        {
            var sql = @"UPDATE MI_BASERO_14
                           SET RO_TYPE = :RO_TYPE,
                               ...
                               UPDATE_IP= :UPDATE_IP
                         WHERE MMCODE = :MMCODE AND WH_NO = :WH_NO AND  RO_WHTYPE = '1' ";
            return DBWork.Connection.Execute(sql, AA0177, DBWork.Transaction);
        }

        public int UpdatePerc(AA0177 AA0177)
        {
            var sql = @"update MI_BASERO_14 
