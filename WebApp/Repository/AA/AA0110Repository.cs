...
{
    public class AA0110Repository : JCLib.Mvc.BaseRepository
    {
        public AA0110Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<ME_DOCM> GetAllM(string docno, string apptime1, string apptime2, string flowid, string matclass, string user, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE LIKE 'AJ%' AND A.MAT_CLASS IN 
                        (SELECT MAT_CLASS FROM MI_MATCLASS WHERE MAT_CLSID  IN ('2','3'))
            ...
            {
                sql += " AND A.APPID = :p11  ";
                p.Add(":p11", string.Format("{0}", user));
            }
        ...
        public IEnumerable<COMBO_MODEL> GetDocnoCombo(string id)
        {
            ...
            return DBWork.Connection.Query<COMBO_MODEL>(sql, new { p1 = id });
        }
        ...
        public IEnumerable<MI_MAST> GetMmCodeCombo(string p0, string p1, bool greaterThanInv, int page_index, int page_size, string sorters)
        {

            var p = new DynamicParameters();

            var sql = @"SELECT {0} A.MMCODE, A.MMNAME_C, A.MMNAME_E, A.MAT_CLASS, A.BASE_UNIT,A.M_CONTPRICE,
                        ( SELECT INV_QTY FROM MI_WHINV WHERE MMCODE = A.MMCODE AND WH_NO =WHNO_MM1 AND ROWNUM =1 ) AS INV_QTY 
            ...
            if (greaterThanInv)
            {
                sql += @" AND EXISTS ( SELECT 1 FROM MI_WHINV WHERE MMCODE = A.MMCODE  AND INV_QTY > 0 
                                        AND WH_NO = WHNO_MM1 ) ";
            }
            else
            {
        ...
        public IEnumerable<MI_MAST> GetMmcode(MI_MAST_QUERY_PARAMS query, bool greaterThanInv, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();
        ...
        #region 2021-11-26 檢查flowId = 2
        public bool ChceckFlowId02(string docno)
        {
            string sql = @"
                select 1 from ME_DOCM
                 where docno = :docno
                   and (substr(flowId, length(flowId)-1 , 2) = '02'
                       or substr(flowId, length(flowId)-1 , 2) = '2')
            ";
            return DBWork.Connection.ExecuteScalar(sql, new { docno }, DBWork.Transaction) != null;
        }
        #endregion
