...
{
    public class AA0150ReportMODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class AA0150ReportBMODEL : JCLib.Mvc.BaseModel
    {
        public string F1 { get; set; }
        public string F2 { get; set; }
        public string F3 { get; set; }
    ...
    public class AA0150Repository : JCLib.Mvc.BaseRepository
    {
        public AA0150Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<ME_DOCM> GetAllM(string apptime1, string apptime2, string[] str_flowid, string[] str_matclass, string[] str_stktrankind, string docno, string tuser, int page_index, int page_size, string sorters)
        {
            var p = new DynamicParameters();

            var sql = @"SELECT A.* ,
                        (case when mat_class = '01' and a.flowId <> 'X'
                        then (select distinct flowId || ' ' || flowName from ME_FLOW where doctype in ('EX') and flowId = a.flowId) 
                        when mat_class <> '01' and a.flowId <> 'X'
                        then (select distinct flowId || ' 衛材' || flowName from ME_FLOW where doctype in ('EX1') and flowId = a.flowId) 
                         else (select distinct flowId ||' ' || flowName from ME_FLOW where doctype in ('EX') and flowId = a.flowId)
                        end) AS FLOWID_N,
                        ...
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE IN  ('EX','EX1')  
            ...
            if (str_stktrankind.Length > 0) //P4
            {
                string sql_stktrankind = "";
                sql += @"AND (";
                foreach (string tmp_stktrankind in str_stktrankind)
                {
                    if (string.IsNullOrEmpty(sql_stktrankind))
                    {
                        sql_stktrankind = @"A.STKTRANSKIND = '" + tmp_stktrankind + "'";
                    }
                    else
                    {
                        sql_stktrankind += @" OR A.STKTRANSKIND = '" + tmp_stktrankind + "'";
                    }
                }
                sql += sql_stktrankind + ") ";
            }
        ...
        public int CreateM(ME_DOCM ME_DOCM)
        {
            var sql = @"INSERT INTO ME_DOCM (
                        ...
                        APPLY_KIND ,APPLY_NOTE ,MAT_CLASS, STKTRANSKIND, 
        ...
        {
            var sql = @"UPDATE ME_DOCEXP SET MMCODE = :MMCODE, C_TYPE = :C_TYPE,
        ...
        {
            var sql = @"UPDATE ME_DOCM SET FLOWID = (case when doctype = 'EX' then '0899' when doctype = 'EX1' then '3' end),
                        UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP
                      WHERE DOCNO = :DOCNO";
            return DBWork.Connection.Execute(sql, me_docm, DBWork.Transaction);
        }
        ...
        public IEnumerable<AA0150ReportMODEL> GetPrintData(string apptime1, string apptime2)
        {
            var p = new DynamicParameters();
            ...
            return DBWork.Connection.Query<AA0150ReportMODEL>(sql, p, DBWork.Transaction);
        }

        public IEnumerable<AA0150ReportBMODEL> GetPrintDataB(string apptime1, string apptime2)
        {
            var p = new DynamicParameters();
            ...
            string sql = @"SELECT A.DOCNO F1,(SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='STKTRANSKIND2' AND DATA_VALUE=A.STKTRANSKIND and rownum=1) F2,
                            ...
                            AND A.DOCTYPE = 'EX' 
                            AND A.FRWH = ( select frwh from ME_DOCM where docno = A.DOCNO )  AND A.MAT_CLASS = '01' AND A.FLOWID = '0899'
                             ";
            ...
            return DBWork.Connection.Query<AA0150ReportBMODEL>(sql, p, DBWork.Transaction);
        }
        ...
        {
            string sql = @"SELECT 1 FROM ME_DOCM WHERE DOCNO=:DOCNO AND FLOWID NOT IN ('0801','1') ";
            return !(DBWork.Connection.ExecuteScalar(sql, new { DOCNO = id }, DBWork.Transaction) == null);
        }
        ...
        {
            string sql = @"SELECT substr(:pDate,1,4) - 1911 ||substr(:pDate,6,2)||substr(:pDate,9,2) GETDATE FROM DUAL ";
        ...
        public IEnumerable<COMBO_MODEL> GetFlowidCombo(string id)
        {
            string sql = @"with temp_whkinds as (
                        ...
                        from temp_mat_class a, ME_FLOW b
                        where a.value = '01' 
                        and b.doctype in ('EX1')
                        and b.flowId <> 'X'
                        union
                        select b.flowid as value, b.flowName as text,flowId || ' ' || flowName as COMBITEM
                        from temp_mat_class a, ME_FLOW b
                        where a.value = '01' 
                        and b.doctype in ('EX', 'EX1')
                        and b.flowId = 'X'
                        union 
        ...
        public IEnumerable<COMBO_MODEL> GetLotNoCombo(string id,string docno)
        {
            ...
            return DBWork.Connection.Query<COMBO_MODEL>(sql, new { DOCNO = docno, MMCODE = id });
        }
