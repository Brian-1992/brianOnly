...
{
    public class AA0141Repository : JCLib.Mvc.BaseRepository
    {
        public AA0141Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<MI_WINVMON> GetInvs(string wh_no, string wh_kind) {
            string sql = string.Format(@"
                select wh_no, mmcode, inv_qty
                  from MI_WHINV a
                 where a.wh_no = :wh_no
                   and exists (select 1 from MI_MAST where mmcode = a.mmcode and mat_class = '{0}')
                   and inv_qty <> 0
            ", wh_kind == "1" ? "01" : "02"); //衛材庫找藥品，藥品庫找衛材

            return DBWork.Connection.Query<MI_WINVMON>(sql, new { wh_no, wh_kind }, DBWork.Transaction);
        }
        ...
        public IEnumerable<COMBO_MODEL> GetWhnoCombo() {
            string sql = @"
                select wh_no as value,
                  ...
                  from MI_WHMAST
                 where 1=1
                   and ( (wh_kind = '1' and wh_grade = '2')
                        or 
                         (wh_kind = '0' and wh_grade in ('3', '4'))
                       )
                 order by wh_no  
            ";
