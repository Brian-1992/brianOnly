...
{
    public class AA0168ReportMODEL : JCLib.Mvc.BaseModel
    {
        ...
        public float DISC_CPRICE { get; set; }
        public float LUMP_SUM { get; set; }
    }

    public class AA0168ApplyMODEL : JCLib.Mvc.BaseModel
    {
        public string CLASS_WH { get; set; }
        public float APL_CNT { get; set; }
    }

    public class AA0168Repository : JCLib.Mvc.BaseRepository
    {
        public AA0168Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<AA0168ReportMODEL> GetQueryData(string pFromWhere, string pClass, string pFromYM, string pToYM, string pWhNo, string pDocType, int page_index, int page_size, string sorters, string pformpgm)
        {
            DynamicParameters sqlParam = new DynamicParameters();
            string sqlStr = GetSqlstr(pFromWhere, pClass, pFromYM, pToYM, pWhNo, pDocType, pformpgm, out sqlParam);

            sqlParam.Add("OFFSET", (page_index - 1) * page_size);
            sqlParam.Add("PAGE_SIZE", page_size);

            return DBWork.Connection.Query<AA0168ReportMODEL>((pFromWhere == "rdlc" ? sqlStr : GetPagingStatement(sqlStr, sorters)), sqlParam, DBWork.Transaction);
        }

        public DataTable GetExcelData(string fromWhere, string pClass, string pFromYM, string pToYM, string pWhNo, string pDocType, string pPrintTitle, string pUserID, string pfrompgm)
        {
            DynamicParameters sqlParam = new DynamicParameters();
            string sqlStr = GetSqlstr(fromWhere, pClass, pFromYM, pToYM, pWhNo, pDocType, pfrompgm, out sqlParam);
        ...
        public String GetSqlstr(string fromWhere, string pClass, string pFromYM, string pToYM, string pWhNo, string pDocType, string pFrompgm, out DynamicParameters pSqlParam)
        {
            ...
            string orderByStr = "ORDER BY 7, 1, 3, 9";
            if (fromWhere == "xls")
            {
                 orderByStr = "ORDER BY 1, 2, 5, 9";
            }
            ...
            sqlStrNow += @" C.DATA_YM, B.APL_INQTY as APL_OUTQTY, C.DISC_CPRICE, (B.APL_INQTY * C.DISC_CPRICE) as LUMP_SUM "; // 衛星庫房的撥發量相當於接收量，故 撥發總量(APL_OUTQTY)  改為讀取 入庫總量(APL_INQTY)
            ...
            if (fromWhere == "xls")
            {
                ...
                sqlStrNow += @" C.DATA_YM AS 撥發年月,B.APL_INQTY AS 核撥總量, C.DISC_CPRICE AS 撥發單價, (B.APL_INQTY * C.DISC_CPRICE) AS 撥發成本 "; // 衛星庫房的撥發量相當於接收量，故 撥發總量(APL_OUTQTY)  改為讀取 入庫總量(APL_INQTY)
        ...
        public IEnumerable<AA0168ApplyMODEL> GetApplyData(string pClass, string pFromYM, string pToYM, string pWhNo, string pDocType, string pFrompgm)
        {
            DynamicParameters sqlParam = new DynamicParameters();
            StringBuilder sqlStr = new StringBuilder();
            Int16 nowYM = Convert.ToInt16((DateTime.Now.Year - 1911).ToString() + DateTime.Now.ToString("MM")); //因 String 無法<、>、>=、<=等來比較, 故轉為
            ...
            return DBWork.Connection.Query<AA0168ApplyMODEL>(sqlStr.ToString(), sqlParam, DBWork.Transaction);
        }
