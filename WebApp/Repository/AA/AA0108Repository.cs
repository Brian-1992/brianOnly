...
{
    public class AA0108Repository : JCLib.Mvc.BaseRepository
    {
        public AA0108Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        // GET api/<controller>
        public IEnumerable<MI_WHMAST> GetWH_NOComboOne()
        {
            var p = new DynamicParameters();

            //string sql = @"SELECT wh_no as VALUE,wh_no ||' '|| wh_name as text from MI_WHMAST  ORDER BY VALUE ";
            ...
            //return DBWork.Connection.Query<MI_WHMAST>(GetPagingStatement(sql), p, DBWork.Transaction);
        ...
        public IEnumerable<AA0108> GetAll(string mclass, string whno, string chk_ym, string rb1, string rb2)
    {
        var p = new DynamicParameters();

            string sql = GetMainSql(mclass, whno, chk_ym, rb1, rb2);
       
        return DBWork.PagingQuery<AA0108>(sql, new { mclass, whno, chk_ym, rb1, rb2 }, DBWork.Transaction);
    }
        

        public IEnumerable<AA0108> GetReport(string mclass, string whno, string ym, string rb1, string rb2)
        {
            var p = new DynamicParameters();
            ...
            var sql = @"SELECT a.wh_no, 
                           ...
                           (a.chk_QTY * c.avg_price) as chk_amount 
            ...
            return DBWork.Connection.Query<AA0108>(sql, p, DBWork.Transaction);
        }
