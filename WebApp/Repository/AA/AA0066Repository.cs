...
namespace WebApp.Repository.AA
{
    ...
    public class AA0066_MODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class AA0066Repository : JCLib.Mvc.BaseRepository
    {

        public AA0066Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<AA0066_MODEL> GetAll(string APPTIME1, string APPTIME2, string MAT_CLASS, string DOCTYPE, string MMCODE, string USRID, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT MDM.DOCNO,
                               ...
                               (SELECT MWT.AVG_PRICE
                                FROM MI_WHCOST MWT
                                WHERE     MWT.MMCODE = MDP.MMCODE
                                AND MWT.DATA_YM = TO_CHAR (SYSDATE, 'YYYYMM') - 191100) AVG_PRICE
            ...
            return DBWork.Connection.Query<AA0066_MODEL>(GetPagingStatement(sql, sorters), p, DBWork.Transaction);
        }
        ...
        public DataTable GetExcel(string APPTIME1, string APPTIME2, string MAT_CLASS, string DOCTYPE, string MMCODE, string USRID)
        {
            ...
            var sql = @"SELECT MDM.DOCNO 單據號碼,
                               ...
                               TO_CHAR (MDM.APPTIME, 'YYYYMMDD') - 19110000 申請日期,
                               ...
                               (SELECT MWT.AVG_PRICE
                                FROM MI_WHCOST MWT
                                WHERE     MWT.MMCODE = MDP.MMCODE
                                AND MWT.DATA_YM = TO_CHAR (SYSDATE, 'YYYYMM') - 191100) 平均單價
                        FROM ME_DOCM MDM, ME_DOCEXP MDP
                        WHERE     MDM.DOCNO = MDP.DOCNO
                        AND MDM.FRWH = WHNO_MM1";
        ...
        public IEnumerable<AA0066_MODEL> GetPrintData(string APPTIME1, string APPTIME2, string MAT_CLASS, string DOCTYPE, string MMCODE, string USRID)
        {
            var p = new DynamicParameters();
            ...
            return DBWork.Connection.Query<AA0066_MODEL>(sql, p, DBWork.Transaction);
        }
