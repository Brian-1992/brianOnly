...
{
    public class AA0131Repository : JCLib.Mvc.BaseRepository
    {
        public AA0131Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }

        public IEnumerable<ME_DOCM> GetAllM(string DOCNO, string APPTIME1, string APPTIME2, string[] str_FLOWID, int page_index, int page_size, string sorters)
        {
        ...
        public IEnumerable<COMBO_MODEL> GetAppDeptCombo()
        {
            string sql = @"SELECT DISTINCT A.INID as VALUE, B.INID_NAME as TEXT ,
                        ...
                        WHERE A.WH_KIND = '1' AND A.WH_NO=A.INID 
                        ORDER BY VALUE";
