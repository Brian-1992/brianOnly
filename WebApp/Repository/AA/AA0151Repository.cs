...
{
    public class AA0151ReportMODEL : JCLib.Mvc.BaseModel
    {
    ...
    public class AA0151ReportBMODEL : JCLib.Mvc.BaseModel
    {
        public string F1 { get; set; }
        public string F2 { get; set; }
        public string F3 { get; set; }
    ...
    public class AA0151Repository : JCLib.Mvc.BaseRepository
    {
        public AA0151Repository(IUnitOfWork unitOfWork) : base(unitOfWork) { }
        ...
        public IEnumerable<ME_DOCM> GetAllM(string apptime1, string apptime2, string[] str_flowid, string[] str_matclass, string docno, string tuser, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT A.* ,
                        ...
                        then (select distinct flowId || ' ' || flowName from ME_FLOW where doctype in ('RJ') and flowId = a.flowId) 
                        when mat_class <> '01' and a.flowId <> 'X'
                        then (select distinct flowId || ' 衛材' || flowName from ME_FLOW where doctype in ('RJ1') and flowId = a.flowId) 
                         else (select distinct flowId ||' ' || flowName from ME_FLOW where doctype in ('RJ') and flowId = a.flowId)
                        end) AS FLOWID_N,
                        ...
                        (SELECT SUM(X.APVQTY * x.c_up * -1) FROM ME_DOCEXP X, MI_MAST Y WHERE X.DOCNO=A.DOCNO AND X.MMCODE=Y.MMCODE AND X.APVQTY < 0) SUM_EX,
                        (SELECT SUM(X.APVQTY * x.c_up) FROM ME_DOCEXP X, MI_MAST Y WHERE X.DOCNO=A.DOCNO AND X.MMCODE=Y.MMCODE AND X.APVQTY > 0) SUM_IN,
                        (SELECT SUM( X.APVQTY * x.c_up ) FROM ME_DOCEXP X,MI_MAST Y WHERE X.MMCODE = Y.MMCODE AND X.DOCNO = A.DOCNO ) BALANCE
                        FROM ME_DOCM A WHERE 1=1 AND A.DOCTYPE IN  ('RJ','RJ1')  
        ...
        public int CreateE(ME_DOCEXP docexp)
        {
            var sql = @"INSERT INTO ME_DOCEXP (
                        ...
                        C_UP, C_AMT, EXP_DATE, LOT_NO,C_TYPE, REASON,
                        UPDATE_TIME, UPDATE_USER, UPDATE_IP)  
                      VALUES (
                        :DOCNO, :SEQ, :MMCODE , :APVQTY , :ITEM_NOTE, 
                        :C_UP, :C_AMT, TO_DATE(:EXP_DATE,'YYYY/MM/DD'), :LOT_NO, :C_TYPE, :REASON,
        ...
        public int ApplyM(ME_DOCM me_docm)
        {
            var sql = @"UPDATE ME_DOCM 
                           SET --FLOWID = (case when doctype = 'RJ' then '0999' when doctype = 'RJ1' then '3' end),
                        UPDATE_TIME = SYSDATE, UPDATE_USER = :UPDATE_USER, UPDATE_IP = :UPDATE_IP
                      WHERE DOCNO = :DOCNO";
            return DBWork.Connection.Execute(sql, me_docm, DBWork.Transaction);
        }
        ...
        public IEnumerable<AA0151ReportMODEL> GetPrintData(string apptime1, string apptime2)
        {
            var p = new DynamicParameters();
            ...
            string sql = @"SELECT TWN_DATE(B.APPTIME) F1,
                            ...
                            AND B.DOCTYPE = 'EX'  AND B.FRWH = ( select frwh from ME_DOCM where docno = A.DOCNO )  AND B.MAT_CLASS = '01'
                            and b.flowId = '0999'        
            ";
            ...
            return DBWork.Connection.Query<AA0151ReportMODEL>(sql, p, DBWork.Transaction);
        }

        public IEnumerable<AA0151ReportBMODEL> GetPrintDataB(string apptime1, string apptime2)
        {
            var p = new DynamicParameters();
            ...
            string sql = @"SELECT A.DOCNO F1,(SELECT DATA_DESC FROM PARAM_D WHERE GRP_CODE='ME_DOCM' AND DATA_NAME='STKTRANSKIND2' AND DATA_VALUE=A.STKTRANSKIND and rownum=1) F2,
                            ...
                            ITEM_NOTE F13,
                            NVL((SELECT SUM(X.APVQTY * Y.M_CONTPRICE * -1) FROM ME_DOCEXP X, MI_MAST Y WHERE X.DOCNO=A.DOCNO AND X.MMCODE=Y.MMCODE AND X.APVQTY < 0),0) F14,
                            NVL((SELECT SUM(X.APVQTY * Y.M_CONTPRICE) FROM ME_DOCEXP X, MI_MAST Y WHERE X.DOCNO=A.DOCNO AND X.MMCODE=Y.MMCODE AND X.APVQTY > 0),0) F15,
                            ( SELECT SUM( X.APVQTY * Y.M_CONTPRICE ) FROM ME_DOCEXP X,MI_MAST Y WHERE X.MMCODE = Y.MMCODE AND X.DOCNO = A.DOCNO ) F16 
                            FROM ME_DOCM A,ME_DOCEXP B,MI_MAST C 
                            WHERE A.DOCNO = B.DOCNO AND B.MMCODE=C.MMCODE 
                            AND A.DOCTYPE = 'EX' 
                            AND A.FRWH = ( select frwh from ME_DOCM where docno = A.DOCNO )  AND A.MAT_CLASS = '01' AND A.FLOWID = '0999'
                             ";
            ...
            return DBWork.Connection.Query<AA0151ReportBMODEL>(sql, p, DBWork.Transaction);
        }
        ...
        {
            string sql = @"SELECT 1 FROM ME_DOCM WHERE DOCNO=:DOCNO AND FLOWID NOT IN ('0901','1') ";
            return !(DBWork.Connection.ExecuteScalar(sql, new { DOCNO = id }, DBWork.Transaction) == null);
        }
        public bool CheckMFlow(string id)
        {
            string sql = @"SELECT 1 FROM ME_DOCM WHERE DOCNO=:DOCNO 
                            AND FLOWID = ( CASE WHEN DOCTYPE = 'RJ' THEN '0999' WHEN DOCTYPE = 'RJ1' THEN '3' END) ";
            return !(DBWork.Connection.ExecuteScalar(sql, new { DOCNO = id }, DBWork.Transaction) == null);
        }
        ...
        public IEnumerable<COMBO_MODEL> GetFlowidCombo(string id)
        {
            string sql = @"with temp_whkinds as (
                            ...
                            select b.flowid as value, b.flowName as text,flowId ||' '|| flowName as COMBITEM
                            from temp_mat_class a, ME_FLOW b
                            where a.value = '01' 
                            and b.doctype in ('RJ')
                            and b.flowId <> 'X'
                            union
                            select b.flowid as value, '衛材'||b.flowName as text,flowId || ' 衛材' || flowName as COMBITEM
                            from temp_mat_class a, ME_FLOW b
                            where a.value = '01' 
                            and b.doctype in ('RJ1')
                            and b.flowId <> 'X'
                            union
                            select b.flowid as value, b.flowName as text,flowId || ' ' || flowName as COMBITEM
                            from temp_mat_class a, ME_FLOW b
                            where a.value = '01' 
                            and b.doctype in ('RJ', 'RJ1')
                            and b.flowId = 'X'
                            union 
                            select ' ' as value, '全部' as text, '全部' as COMBITEM from dual
                        ";
        ...
        public IEnumerable<MI_MAST> GetMmCodeCombo(string p0, string p1, string docno, int page_index, int page_size, string sorters)
        {
            ...
            var sql = @"SELECT {0} A.MMCODE, A.MMNAME_C, A.MMNAME_E, A.MAT_CLASS, A.BASE_UNIT, A.disc_cprice as M_CONTPRICE, 
        ...
        {
            var p = new DynamicParameters();
            var sql = @"SELECT A.MMCODE, A.MMNAME_C, A.MMNAME_E, A.MAT_CLASS, A.BASE_UNIT,A.disc_cprice as M_CONTPRICE,A.DISC_UPRICE, 
        ...
        public IEnumerable<COMBO_MODEL> GetMatclassQCombo(string id)
        {
            string sql = @"with temp_whkinds as (
                            select b.wh_no, b.wh_kind, nvl((case when a.task_id = '3' then '2' else a.task_id end), '2') as task_id
                            from MI_WHID a, MI_WHMAST b
        ...
        public IEnumerable<COMBO_MODEL> GetReturnReasonCombo()
        {
            string sql = @"select distinct DATA_VALUE as VALUE, DATA_DESC as TEXT from PARAM_D 
                            where GRP_CODE = 'ME_DOCD' and DATA_NAME = 'RETURN_REASON'
                            order by DATA_VALUE
                            ";
            return DBWork.Connection.Query<COMBO_MODEL>(sql);
        }
        ...
        public string GetINV_QTY(string FRWH, string MMCODE, string LOT_NO)
        {
            ...
            return DBWork.Connection.QueryFirstOrDefault<string>(sql, new { FRWH = FRWH, MMCODE = MMCODE, LOT_NO = LOT_NO }, DBWork.Transaction);
        }
        ...
        public IEnumerable<COMBO_MODEL> GetDocnoCombo()
        {
            string sql = @"SELECT DISTINCT A.DOCNO as VALUE, A.DOCNO as TEXT,
                        ...
                        AND A.DOCTYPE = 'EX' 
                        AND A.FRWH = ( select frwh from ME_DOCM where docno = A.DOCNO ) AND A.MAT_CLASS = '01'
        ...
        public int InsertWexpinv(string wh_no, string mmcode, string lot_no, string exp_date, string qty, string userId, string updateIp) {
            string sql = @"
                ...
                values (:wh_no, :mmcode, to_date(:exp_date, 'yyyy/mm/dd'), :lot_no, :qty * -1 ,
        ...
        public IEnumerable<AA0151ReportMODELM> GetPrintDataM(string docno)
        {
            var p = new DynamicParameters();

            string sql = @"select A.DOCNO F1,
                            (C.AGEN_NO || ' ' || C.AGEN_NAMEC) as F2,
                            C.AGEN_TEL F3 ,
                            C.AGEN_FAX F4,
                            C.AGEN_ADD F5,
                            (select TWN_DATE(APPTIME) from ME_DOCM where DOCNO=A.DOCNO) AS F6,
                            A.SEQ F7,
                            A.MMCODE F8 ,
                            B.MMNAME_E F9,
                            B.MMNAME_C F10,
                            ((case when a.apvqty< 0 then -1 else 1 end) * A.APVQTY) F11,
                            B.BASE_UNIT F12,
                            B.M_PURUN F13,
                            (select APPLY_NOTE from ME_DOCM where DOCNO=A.DOCNO)||('批號'||A.LOT_NO) || ' 效期' || twn_date(A.EXP_DATE) as F14,
                            A.REASON as F15
                            FROM ME_DOCEXP A
                            INNER JOIN MI_MAST B ON B.MMCODE=A.MMCODE
                            INNER JOIN PH_VENDER C ON B.MMCODE=A.MMCODE and C.AGEN_NO=B.M_AGENNO 
                            WHERE DOCNO = :DOCNO 
                            ";

            p.Add(":DOCNO", docno);

            return DBWork.Connection.Query<AA0151ReportMODELM>(sql, p, DBWork.Transaction);
        }

        public class AA0151ReportMODELM : JCLib.Mvc.BaseModel
        {
            public string F1 { get; set; }
            public string F2 { get; set; }
            public string F3 { get; set; }
        ...
        //2023-10-07
        public int InsertPhInvoice(string docno, string userId, string update_ip) {
            string sql = string.Format(@"
                insert into PH_INVOICE(po_no, mmcode, transno, create_time,
                                      po_qty, po_price, po_amt, in_amount, 
                                      invoice_type, memo,
                                      create_user, update_time, update_user, update_ip,
                                      data_ym)
                select '-' as po_no, a.mmcode, '{0}' , sysdate,
                       abs(a.appqty), b.disc_cprice, abs(a.appqty * b.disc_cprice), abs(a.appqty * b.disc_cprice), 
                       '2', twn_sysdate||'退貨單號：'||:docno,
                       :userId, sysdate, :userId, :update_ip,
                       cur_setym
                  from ME_DOCD a, MI_MAST b
                 where a.docno=:docno
                   and a.mmcode = b.mmcode
            ", DateTime.Now.ToString("yyyyMMddHHmmss"));
            return DBWork.Connection.Execute(sql, new { docno, userId, update_ip }, DBWork.Transaction);
        }

        //檢查是否存在重複的退貨項目
        public bool CheckExpExisted(ME_DOCEXP me_docexp, bool isUpdate)
        {
            ...
            if (me_docexp.INOUT != "" && me_docexp.INOUT != null)
            {
                sql += "AND C_TYPE=:INOUT "; //如果存在C_TYPE(INOUT)要增加條件
            }
