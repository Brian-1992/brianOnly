...
Ext.onReady(function () {
    // var T1Get = '/api/AB0051/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "調帳單維護";
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AB0051/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_doctype = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AB0051/GetDoctypeCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1Tool = Ext.create('Ext.PagingToolbar', {
        ...
        buttons: [
            ...
            {
                itemId: 'apply', text: '送審核', disabled: true, handler: function () {
                    var selection = T1Grid.getSelection();
                    if (selection.length === 0) {
                        Ext.Msg.alert('提醒', '請勾選項目');
                    }
                        ...
                        Ext.MessageBox.confirm('送審核', '是否確定送審核？單號如下<br>' + name, function (btn, text) {
                            if (btn === 'yes') {
                                Ext.Ajax.request({
                                    url: '/api/AB0051/Apply',
                                    method: reqVal_p,
                                    params: {
                                        DOCNO: docno
                                    },
            ...
            {
                itemId: 'apply2', text: '核可調帳', disabled: true, handler: function () {
                    var selection = T1Grid.getSelection();
                    if (selection.length === 0) {
                        Ext.Msg.alert('提醒', '請勾選項目');
                    }
                        ...
                        Ext.MessageBox.confirm('核可調帳', '是否確定核可調帳？單號如下<br>' + name, function (btn, text) {
                            if (btn === 'yes') {
                                Ext.Ajax.request({
                                    url: '/api/AB0051/Apply2',
                                    method: reqVal_p,
                                    params: {
                                        DOCNO: docno
                                    },
                                    ...
                                    success: function (response) {
                                        var data = Ext.decode(response.responseText);
                                        if (data.success) {
                                            Ext.MessageBox.alert('訊息', '核可調帳成功');
                                            T2Store.removeAll();
                                            T1Grid.getSelectionModel().deselectAll();
                                            T1Load();
                                        }
    ...
    function setFormT1(x, t) {
        ...
        if (x === "I") {
            ...
            f.findField('FRWH_N').setValue(st_getlogininfo.getAt(0).get('INID_WHNAME'));
            f.findField('FRWH').setValue(st_getlogininfo.getAt(0).get('INID_WHNO'));
            f.findField('TOWH').setValue(st_getlogininfo.getAt(0).get('INID_WHNO'));
    ...
    function setFormT1a() {
        ...
        if (T1LastRec) {
            ...
            if (T1F3 == '2') {
                T1Grid.down('#apply2').setDisabled(false);
            }
            else {
                T1Grid.down('#apply2').setDisabled(true);
            }
        }
        else {
    ...
    var T2QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AB0051/GetMMCodeDocd', //指定查詢的Controller路徑
                    ...
                    T2FormMMCode,
                    {
                        ...
                        handler: function () {
                            ...
                            if (!f.findField("MMCODE").readOnly) {
                                popMmcodeForm(viewport, '/api/AB0051/GetMmcode', {
        ...
        buttons: [
            {
                itemId: 'add', text: '新增', disabled: true, handler: function () {
                    T11Set = '../../../api/AB0051/CreateD';
                    setFormT2('I', '新增');
                }
            },
            {
                itemId: 'edit', text: '修改', disabled: true, handler: function () {
                    T11Set = '../../../api/AB0051/UpdateD';
                    setFormT2("U", '修改');
                }
            },
            {
