...
Ext.onReady(function () {
    ...
    var reportUrl = '/Report/A/AB0062.aspx';
    // var T1Get = '/api/AB0062/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "單位調撥明細表";
    var T1GetExcel = '/api/AB0062/Excel';
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AB0062/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AB0062/GetMMCodeDocd', //指定查詢的Controller路徑
            ...
            items: [
                ...
                },
                {
                    ...
                    handler: function () {
                        ...
                        msglabel('訊息區:');
                        if (fid == '1') {
                            f.findField('P4').setValue(st_whno.getAt(0).get('VALUE'));
                        }
                    }
                }
            ]
        ...
        buttons: [
            {
                ...
                handler: function () {
                    var p = new Array();
                    p.push({ name: 'FN', value: '單位調撥明細表.xls' });
    ...
    function showReport() {
        if (!win) {
            var p0 = T1Query.getForm().findField('P0').getValue() == null ? '' : T1Query.getForm().findField('P0').rawValue;
            var p1 = T1Query.getForm().findField('P1').rawValue == null ? '' : T1Query.getForm().findField('P1').rawValue;
            var p2 = T1Query.getForm().findField('P2').rawValue == null ? '' : T1Query.getForm().findField('P2').getValue();
            ...
            var qstring = '?fid=' + fid + '&p0=' + p0  + '&p1=' + p1 + '&p2=' + p2 + '&p3=' + p3 + '&p4=' + p4 + '&p5=' + p5;
