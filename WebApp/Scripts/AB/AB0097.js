...
Ext.onReady(function () {
    ...
    //var St_MatclassGet = '../../../api/AB0097/GetMatclassCombo';
    var St_FrwhGet = '../../../api/AB0097/GetFrwhCombo';
    var T1GetExcel = '../../../api/AB0097/Excel';

   
    var reportUrl = '/Report/A/AB0097.aspx';　　　　　　　　//明細
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AB0097/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1QuryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AB0097/GetMMCODECombo', //指定查詢的Controller路徑
        ...
        buttons: [
            {
                ...
                handler: function () {
                    ...
                    p.push({ name: 'MAT_CLASS', value: T1Query.getForm().findField('MAT_CLASS').getValue() });
                    p.push({ name: 'FRWH', value: session['Inid'] });
                    PostForm(T1GetExcel, p);
                    msglabel('訊息區:匯出完成');
                    //} else {
                    //    //msglabel('訊息區:需要選匯出選項' + T1Query.getForm().findField('SHOWDATA').getChecked()[0].inputValue);
    ...
    function showReport() {
        if (!win) {
            ...
            var winform = Ext.create('Ext.form.Panel', {
                ...
                //html: '<iframe src="' + reportUrl + '?type=' + "AB0097"
                //+ '&APPTIME_START=' + T1Query.getForm().findField('APPTIME_START').getValue()
                //+ '&APPTIME_END=' + T1Query.getForm().findField('APPTIME_END').getValue()
                //+ '&MAT_CLASS=' + T1Query.getForm().findField('MAT_CLASS').getValue()
                //+ '&FRWH=' + session['Inid']
