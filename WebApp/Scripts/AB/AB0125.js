...
Ext.onReady(function () {
    ...
    //var St_MatclassGet = '../../../api/AB0125/GetMatclassCombo';

    var T1GetExcel = '../../../api/AB0125/Excel';

   
    var reportUrl = '/Report/A/AB0125.aspx';　　　　　　　　//明細
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AB0125/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1QuryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AB0125/GetMMCODECombo', //指定查詢的Controller路徑
        ...
        buttons: [
            {
                ...
                handler: function () {
                    ...
                    //}
                    //if (T1Query.getForm().findField('SHOWDATA').getChecked()[0].inputValue != "") {
                    p.push({ name: 'APPTIME_START', value: T1Query.getForm().findField('APPTIME_START').getValue() });
                    p.push({ name: 'APPTIME_END', value: T1Query.getForm().findField('APPTIME_END').getValue() });
                    p.push({ name: 'MAT_CLASS', value: T1Query.getForm().findField('MAT_CLASS').getValue() });
    ...
    function showReport() {
        if (!win) {
            var p0 = T1Query.getForm().findField('APPTIME_START').getValue() == null ? '' : T1Query.getForm().findField('APPTIME_START').rawValue;
            var p1 = T1Query.getForm().findField('APPTIME_END').rawValue == null ? '' : T1Query.getForm().findField('APPTIME_END').rawValue;
            var p2 = T1Query.getForm().findField('MAT_CLASS').rawValue == null ? '' : T1Query.getForm().findField('MAT_CLASS').getValue();
            var p3 = T1Query.getForm().findField('FRWH').rawValue == null ? '' : T1Query.getForm().findField('FRWH').getValue();
            var qstring = '?p0=' + p0 + '&p1=' + p1 + '&p2=' + p2 + '&p3=' + p3 ;
            var winform = Ext.create('Ext.form.Panel', {
                ...
                //html: '<iframe src="' + reportUrl + '?type=' + "AB0125"
                //+ '&APPTIME_START=' + T1Query.getForm().findField('APPTIME_START').getValue()
                //+ '&APPTIME_END=' + T1Query.getForm().findField('APPTIME_END').getValue()
                //+ '&MAT_CLASS=' + T1Query.getForm().findField('MAT_CLASS').getValue()
                //+ '&FRWH=' + session['Inid']
    ...
    var T1Grid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            ...
            {
                text: "繳回單號", dataIndex: 'DOCNO', width: 110, sortable: true
            }, {
                ...
                text: "品名", dataIndex: 'MMNAME_CE', width: 200, sortable: true
            }, {
