...
Ext.onReady(function () {
                            ...
                            listeners: {
                                change: function (field, nValue, oValue, eOpts) {
                                    ...
                                    var task = new Ext.util.DelayedTask(function () {
                                        chkBarcode(T1Query.getForm().findField('P2').getValue(), T1Query.getForm().findField('P1').getRawValue(), T1Query.getForm().findField('P11').getRawValue());
                                    });
                                    var nValue = T1Query.getForm().findField('P2').getValue()
                                    if (nValue.length >= 6) {
                                        task.delay(1500);//將使用輸入後…延遲 1000=1 秒執行task的任務
                                    }
                                }
                            }
                        }, {
        ...
        if (records.length > 0) {
            for (var i = 0; i < records.length; i++) {
                s = records[i].data.MMCODE + "^" + records[i].data.WEXP_ID + "^"
                    ...
                    + records[i].data.ACC_QTY + "^" + records[i].data.PH_REPLY_SEQ;
                            ...
                            items: [
                                {
                                    ...
                                    items: [{
                                        ...
                                        handler: function () {
                                            ...
                                            if (f.isValid()) {
                                                //如果輸入負值,先檢查存量是否足夠
                                                if (parseInt(f.findField('INQTY').getValue()) < 0) {
                                                    Ext.Ajax.request({
                                                        url: '/api/CC0002/CC0006_ChkWHIINV',
                                                        params: {
                                                            po_no: f.findField('PO_NO').getValue(),
                                                            mmcode: f.findField('MMCODE').getValue(),
                                                            ...
                                                            storeid: f.findField('STOREID').getValue()
                                                        },
                                                        method: reqVal_p,
                                                        success: function (response) {
                                                            var data = Ext.decode(response.responseText);
                                                            if (data.success) {
                                                                if (data.msg == "N") {
                                                                    Ext.MessageBox.alert("提示", "此品項未點收量不夠扣減");
            ...
            function T4Submit(t) {
                setTimeout(function () {
                    ...
                    if (loadData.UNIT_SWAP == "1" && loadData.STOREID == "0" && chkMSG != "") {
                        Ext.getCmp('ScanDistConfirm').enable(); Ext.getCmp('ScanDistConfirm').enable();
                        Ext.Msg.alert('訊息', chkMSG);
                        ...
                        var toUrl = '/api/CC0002/UpdateScanDist';
                        if (t == '1') toUrl = '/api/CC0002/CC0006_UpdateScanDist';
    ...
    // 主畫面查詢:檢查條碼
    function chkBarcode(barcode, p1, p11) {
        var myMask = new Ext.LoadMask(viewport, { msg: '處理中...' });
        myMask.show();
        Ext.Ajax.request({
            url: chkBarcodeGet,
