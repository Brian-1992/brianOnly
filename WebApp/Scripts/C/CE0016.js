...
Ext.onReady(function () {
    ...
    var T1Query = Ext.widget({
        ...
        items: [
        {
            ...
            items: [
                ...
                //        parent.link2('/Form/Index/CE0003', '初盤數量輸入作業(CE0003)', true);
                //    }
                //},
                {
                    xtype: 'combo',
                    ...
                    items: [
                        {
                            ...
                            text: '初盤確認',
                            handler: function () {
                                parent.link2('/Form/Index/CE0004', ' 初盤確認(CE0004)', true);
    ...
    T1Store.on('beforeload', function (store, options) {
        ...
        T1Store.getProxy().setExtraParam('chk_level', '1');
        ...
        buttons: [

            {
                ...
                handler: function () {
                    ...
                    var index2_bak_outqty = findColumnIndex(T2Grid.columns, 'BAK_OUTQTY');
                    var index4_pre_inv_qty = findColumnIndex(T3Grid.columns, 'PRE_INV_QTY');
                    var index4_aplinqty = findColumnIndex(T3Grid.columns, 'APL_INQTY');
                    var index4_aploutqty = findColumnIndex(T3Grid.columns, 'APL_OUTQTY');
                    var index4_store_qty = findColumnIndex(T3Grid.columns, 'STORE_QTYC');
                    var index4_use_qty = findColumnIndex(T3Grid.columns, 'USE_QTY');
                    var index4_bak_outqty = findColumnIndex(T3Grid.columns, 'BAK_OUTQTY');
                    ...
                    if (multiCheckList[0].CHK_WH_KIND_CODE == 'E' || multiCheckList[0].CHK_WH_KIND_CODE == 'C') {
                        ...
                        T2Grid.getColumns()[index2_bak_outqty + 1].hide();
                        T2Grid.getColumns()[index2_use_qty + 1].hide();
                        T2Grid.getColumns()[index2_store_qty + 1].show();

                        T3Grid.getColumns()[index4_pre_inv_qty + 1].hide();
                        T3Grid.getColumns()[index4_aplinqty + 1].hide();
                        T3Grid.getColumns()[index4_aploutqty + 1].hide();
                        T3Grid.getColumns()[index4_bak_outqty + 1].hide();
                        T3Grid.getColumns()[index4_use_qty + 1].hide();
                        T3Grid.getColumns()[index4_store_qty + 1].show();
                    } else {
                        ...
                        T2Grid.getColumns()[index2_bak_outqty + 1].show();
                        T2Grid.getColumns()[index2_use_qty + 1].show();
                        T2Grid.getColumns()[index2_store_qty + 1].hide();

                        T3Grid.getColumns()[index4_pre_inv_qty + 1].show();
                        T3Grid.getColumns()[index4_aplinqty + 1].show();
                        T3Grid.getColumns()[index4_aploutqty + 1].show();
                        T3Grid.getColumns()[index4_bak_outqty + 1].show();
                        T3Grid.getColumns()[index4_use_qty + 1].show();
                        T3Grid.getColumns()[index4_store_qty + 1].hide();
                    }
                    ...
                    multiCheckWindow.setTitle("初盤數量輸入作業: " + session['UserName']);
    ...
    var T1Grid = Ext.create('Ext.grid.Panel', {
        ...
        listeners: {
            selectionchange: function (model, records) {
                ...
                Ext.getCmp('btnMultiChk').disable();
                Ext.getCmp('btnMultiPrint').disable();

                if (records.length > 0) {
                    Ext.getCmp('btnMultiChk').enable();
            ...
            cellclick: function (view, cell, cellIndex, record, row, rowIndex, e) {
                ...
                T2Query.getForm().findField('chk_time_status').setValue('');
                ...
                popformINI.setTitle("初盤數量輸入作業: " + T1LastRec.data.CHK_WH_NO + " " + T1LastRec.data.CHK_YM + " " + T1LastRec.data.CHK_PERIOD + " " + T1LastRec.data.CHK_TYPE +
                        ...
                        success: function (response) {
                            ...
                            if (data.success) {
                                ...
                                if (tb_msg == "尚有品項未盤點!") {  //尚有負責的品項未盤點
                                    ...
                                    T1Load();                                      //更新主畫面
                                    T1LastRec.data.CHK_STATUS_CODE = "2";
                                    popformINI.hide();
    ...
    function getChkTypeColumn(record) {
        ...
        var pSpan = '<span>';
        if (record.data.M_TRNID == '1' && record.data.STORE_QTYC != record.data.CHK_QTY && record.data.CHK_TIME != "" && record.data.CHK_TIME != null) {
        ...
        buttons: [
            {
                ...
                handler: function () {
                    ...
                    if (T25Form.getForm().findField('multiPrintType').getValue()['multiPrintType'] == 'mmcode') {
                        if (selection[0].data.CHK_WH_KIND_CODE == 'E' || selection[0].data.CHK_WH_KIND_CODE == 'C') {
