...
Ext.onReady(function () {
    ...
    var St_MatclassGet = '../../../api/FA0001/GetMatclassCombo';
    var St_AppdeptGet = '../../../api/FA0001/GetAppDeptCombo';

    var T1GetExcel = '../../../api/FA0001/Excel';


    var reportUrl = '/Report/F/FA0052.aspx';　　　　　　　　//明細
    ...
    function setComboData() {
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                var data = Ext.decode(response.responseText);
                if (data.success) {
                    var tb_appdept = data.etts;


                    if (tb_appdept.length > 0) {
                        for (var i = 0; i < tb_appdept.length; i++) {
                            st_appdept.add({ VALUE: tb_appdept[i].VALUE, COMBITEM: tb_appdept[i].COMBITEM });
                        }
                    }
    ...
    var st_AppDept = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/FA0001/GetAppDeptCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1Store = Ext.create('WEBAPP.store.FA0001', {
        listeners: {
    ...
    function showReport() {
        if (!win) {
            var winform = Ext.create('Ext.form.Panel', {
                ...
                layout: 'fit',
                closable: false,
                html: '<iframe src="' + reportUrl + '?type=' + "FA0001"
