 ...
 *
 */
var CanvgReqVal_g = 'GET';
var CanvgReqVal_p = 'POST';
    ...
    var canvg_1 = createCommonjsModule(function (module) {
        ...
        function build(opts) {
            ...
            svg.ajax = function (url) {
                var axObj;
                if (windowEnv.XMLHttpRequest) { axObj = new windowEnv.XMLHttpRequest(); } else { axObj = new ActiveXObject('Microsoft.XMLHTTP'); }
                if (axObj) {
                    axObj.open(CanvgReqVal_g, url, false);
                    axObj.send(null);
                    return axObj.responseText;
                }
                return null;
            };
                        ...
                        switch (pp.command) {
                            ...
                            case 'a':
                                while (!pp.isCommandOrEnd()) {
                                    ...
                                    var rx = pp.getScalar();
                                    var ry = pp.getScalar();
                                    var xAxisRotation = pp.getScalar() * (Math.PI / 180.0); // £p
                                    ...
                                    // initial angle
                                    var a1 = a([1, 0], [(currp.x - cpp.x) / rx, (currp.y - cpp.y) / ry]); // £c1
                                    // angle delta
                                    var u = [(currp.x - cpp.x) / rx, (currp.y - cpp.y) / ry];
                                    var v = [(-currp.x - cpp.x) / rx, (-currp.y - cpp.y) / ry];
                                    var ad = a(u, v); // £G£c
