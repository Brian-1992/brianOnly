...
Ext.onReady(function () {
    // var T1Get = '/api/AA0151/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "退貨作業";
    ...
    var reportMUrl = '/Report/A/AA0151.aspx';
    ...
    var st_MatclassQ = Ext.create('Ext.data.Store', {
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST' // by default GET
            },
            url: '/api/AA0151/GetMatclassQCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_Matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0151/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_returnReason = Ext.create('Ext.data.Store', {
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST' // by default GET
            },
            url: '/api/AA0151/GetReturnReasonCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1Tool = Ext.create('Ext.PagingToolbar', {
        ...
        buttons: [
            ...
            {
                itemId: 'apply', text: '核可扣帳', disabled: true, handler: function () {
                    var selection = T1Grid.getSelection();
                    if (selection.length === 0) {
                        Ext.Msg.alert('提醒', '請勾選項目');
                    }
                        ...
                        Ext.MessageBox.confirm('核可扣帳', '是否確定核可扣帳？單號如下<br>' + name, function (btn, text) {
                            if (btn === 'yes') {
                                Ext.Ajax.request({
                                    url: '/api/AA0151/Apply',
                                    method: reqVal_p,
                                    params: {
                                        DOCNO: docno
                                    },
                                    ...
                                    success: function (response) {
                                        var data = Ext.decode(response.responseText);
                                        if (data.success) {
                                            //Ext.MessageBox.alert('訊息', '核可扣帳成功');
                                            T2Store.removeAll();
                                            T1Grid.getSelectionModel().deselectAll();
                                            T1Load();
                                            msglabel('訊息區:核可扣帳成功');
    ...
    function setFormT1(x, t) {
        ...
        if (x === "I") {
            ...
            f.findField('FLOWID_N').setValue('退貨中');
    ...
    function showReportM() {
        var qstring = '?p0=' + T1LastRec.data.DOCNO;
        if (!winM) {
            var winform = Ext.create('Ext.form.Panel', {
                id: 'iframeReport',
                layout: 'fit',
                closable: false,
                html: '<iframe src="' + reportMUrl + qstring + '" id="mainContent" name="mainContent" width="100%" height="100%" frameborder="0" style="background-color:#FFFFFF"></iframe>',
                buttons: [{
                    text: '關閉',
                    handler: function () {
            ...
            var winM = GetPopWin(viewport, winform, '', viewport.width - 20, viewport.height - 20);
        }
        winM.show();
    }
    ...
    var GetEXP_DATE = '../../../api/AA0151/GetEXP_DATE';
    var GetINV_QTY = '../../../api/AA0151/GetINV_QTY';
    var GetM_DISCPERC = '../../../api/AA0151/GetM_DISCPERC';
    var GetM_CONTPRICE = '../../../api/AA0151/GetM_CONTPRICE';
    ...
    var T2QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AA0151/GetMMCodeDocd', //指定查詢的Controller路徑
    ...
    var T2FormMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0151/GetMMCodeCombo', //指定查詢的Controller路徑
        ...
        listeners: {
            select: function (c, r, i, e) {
                ...
                if (r.get('MMCODE') !== '') {
                    ...
                    f.findField('IN_PRICE').setValue(r.get('M_CONTPRICE'));
        ...
        },
        {
                ...
                T2FormMMCode,
                {
                    ...
                    handler: function () {
                        ...
                        if (!f.findField("MMCODE").readOnly) {
                            popMmcodeForm_14(viewport, '/api/AA0151/GetMmcode', {
            ...
            listeners: {
                change: function (field, newValue, oldValue) {
                    ...
                    var objP1 = T2Form.getForm().findField("IN_PRICE");
        ...
        buttons: [
            {
                itemId: 'add', text: '新增', disabled: true, handler: function () {
                    T11Set = '../../../api/AA0151/CreateD';
                    setFormT2('I', '新增');
                }
            },
            {
                itemId: 'edit', text: '修改', disabled: true, handler: function () {
                    T11Set = '../../../api/AA0151/UpdateD';
                    setFormT2("U", '修改');
                }
            },
            {
    ...
    function setFormT2(x, t) {
        ...
        f2.findField('REASON').setReadOnly(false);
