...
Ext.onReady(function () {
    // var T1Get = '/api/AA0124/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "戰備調撥歸墊";
    ...
    var st_apply_kind = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0124/GetApplyKindCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_appdept = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0124/GetAppDeptCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0124/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_storeid = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0124/GetStoreidCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    // 查詢欄位
    var T1QueryFormMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        name: 'P6',
        fieldLabel: '院內碼',
        labelAlign: 'right',
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0124/GetMMCodeCombo', //指定查詢的Controller路徑
    ...
    var T2QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        labelAlign: 'right',
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0124/GetMMCodeDocd', //指定查詢的Controller路徑
        extraFields: ['MMNAME_C', 'MMNAME_E', 'BASE_UNIT'], //查詢完會回傳的欄位
        getDefaultParams: function () { //查詢時Controller固定會收到的參數
            return {
                ...
                handler: function () {
                    ...
                    for (var i = 0; i < tempData.length; i++) {
                        if (tempData[i].dirty) {
                            ...
                            stat = tempData[i].data.STAT;
                            rv_mqty = Number(tempData[i].data.RV_MQTY);
                            bw_mqty = Number(tempData[i].data.BW_MQTY);
                            if (rv_mqty == 0) {
                                isZero = true;
                            }
                            if (stat == "M") {
                    ...
                    if (isZero) {
                        Ext.MessageBox.alert('錯誤', '調撥歸墊不得為0');
                        return;
                    }
                    if (ismerr) {
                        Ext.MessageBox.alert('錯誤', '調撥歸墊必須等於調撥量');
                        return;
                    }
    ...
    function setFormT2(x, t) {
        ...
        f2.findField('RV_MQTY').setReadOnly(false);
    ...
    var T2Grid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            text: "<b><font color=red>調撥歸墊</font></b>",
            dataIndex: 'RV_MQTY',
            style: 'text-align:left',
            width: 100,
            editor: {
