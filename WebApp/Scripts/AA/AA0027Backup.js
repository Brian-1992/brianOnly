...
Ext.onReady(function () {
    ...
    function SetWH_NO() { //建立調帳庫別的下拉式選單
        Ext.Ajax.request({
            url: GetWH_NO,
            method: reqVal_g,
    ...
    var mmCodeCombo = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        listeners: {
            select: function (c, r, i, e) {
                ...
                f.findField("NEW_LOT_NO").setValue("");
                f.findField("NEW_LOT_NO").clearInvalid();
                f.findField("NEW_EXP_DATE").setValue("");
                f.findField("NEW_EXP_DATE").clearInvalid();
                f.findField("NEW_APVQTY").setValue("");
                f.findField("NEW_APVQTY").clearInvalid();
            }
        }
    });
    ...
    function setMmcode(args) {
        ...
        T2Form.getForm().findField("NEW_LOT_NO").setValue("");
        T2Form.getForm().findField("NEW_EXP_DATE").setValue("");
        T2Form.getForm().findField("NEW_EXP_DATE").clearInvalid();
        T2Form.getForm().findField("NEW_APVQTY").setValue("");
        T2Form.getForm().findField("NEW_APVQTY").clearInvalid();
    }
                ...
                if (data.success) {
                    ...
                    if (lot_nos.length > 0) {
                        ...
                        for (var i = 0; i < lot_nos.length; i++) {
                            ...
                            T2Form.getForm().findField("NEW_LOT_NO").setValue(lot_nos[0].LOT_NO);
                            T2Form.getForm().findField("NEW_EXP_DATE").setValue(lot_nos[0].EXP_DATE);
                            qty_temp = lot_nos[0].INV_QTY;
                        }
                        T2Form.getForm().findField("NEW_APVQTY").setMaxValue(T2Form.getForm().findField("INV_QTY").getValue());
                        T2Form.getForm().findField("NEW_APVQTY").setValue(qty_temp);
                    }
                }
            },
            failure: function (response, options) {
    ...
    function SetM_DISCPERC() { //取得進貨單價
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                NEW_APVQTY = T2Form.getForm().findField("NEW_APVQTY").getValue();
    ...
    var T2Grid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            text: "批號",
            dataIndex: 'NEW_LOT_NO',
            width: 80
        }, {
            text: "效期",
            dataIndex: 'NEW_EXP_DATE',
            width: 70
        }, {
            text: "調整量",
            dataIndex: 'NEW_APVQTY',
            width: 80,
            xtype: 'numbercolumn',
            align: 'right',
    ...
    var T2Form = Ext.widget({
            ...
            items: [{
                ...
                listeners: {
                    select: function (oldValue, newValue, eOpts) {
                        ...
                        T2Form.getForm().findField("NEW_LOT_NO").setValue(newValue.data.LOT_NO);
                        T2Form.getForm().findField("NEW_EXP_DATE").setValue(newValue.data.EXP_DATE);
                        T2Form.getForm().findField("NEW_APVQTY").setMaxValue(T2Form.getForm().findField("INV_QTY").getValue());
                        T2Form.getForm().findField("NEW_APVQTY").setValue(newValue.data.INV_QTY);
                    }
                }
            }, {
                fieldLabel: '批號', //白底顯示
        ...
        buttons: [{
            itemId: 'T2Submit', text: '儲存', hidden: true, handler: function () {
                ...
                } else if ((T2Form.getForm().findField("NEW_LOT_NO").getValue()) == "" ||
                    (T2Form.getForm().findField("NEW_LOT_NO").getValue() == null)) {
                    Ext.Msg.alert('提醒', "<span style='color:red'>新批號不可為空</span>，請重新輸入。");
                    msglabel(" <span style='color:red'>新批號不可為空</span>，請重新輸入。");
                } else if ((T2Form.getForm().findField("NEW_EXP_DATE").getValue()) == "" ||
                    (T2Form.getForm().findField("NEW_EXP_DATE").getValue() == null)) {
                    Ext.Msg.alert('提醒', "<span style='color:red'>新效期不可為空</span>，請重新輸入。");
                    msglabel(" <span style='color:red'>新效期不可為空</span>，請重新輸入。");
                } else if ((T2Form.getForm().findField("NEW_APVQTY").getValue()) == "" ||
                    (T2Form.getForm().findField("NEW_APVQTY").getValue() == null)) {
                    Ext.Msg.alert('提醒', "<span style='color:red'>調整量不可為空</span>，請重新輸入。");
                    msglabel(" <span style='color:red'>調整量不可為空</span>，請重新輸入。");
    ...
    function setFormT2(x, t) {
        ...
        if (x === "I") {
            ...
            setCmpShowCondition_D(true, false, true, false, true, false, true, false, false, false);
        ...
        f2.findField('NEW_LOT_NO').setReadOnly(false);
        f2.findField('NEW_EXP_DATE').setReadOnly(false);
        f2.findField('NEW_APVQTY').setReadOnly(false);
        f2.findField('NEW_APVQTY').clearInvalid();
    ...
    function T2Submit() {
        ...
        if (f.isValid()) {
            ...
            f.submit({
                ...
                success: function (form, action) {
                    myMask.hide();
                    setCmpShowCondition_D(false, true, false, true, false, true, false, true, true, true);
                    ...
                    switch (action.failureType) {
                        ...
                        case Ext.form.action.Action.SERVER_INVALID:
                            ...
                            if (T2Form.getForm().findField("x").getValue() == 'I') {
                                ...
                                T2Form.getForm().findField('INV_QTY').setValue('');
                                T2Form.getForm().findField('NEW_LOT_NO').setValue('');
                                T2Form.getForm().findField('NEW_LOT_NO').clearInvalid();
                                T2Form.getForm().findField('NEW_EXP_DATE').setValue('');
                                T2Form.getForm().findField('NEW_EXP_DATE').clearInvalid();
                                T2Form.getForm().findField('NEW_APVQTY').setValue('');
                                T2Form.getForm().findField('NEW_APVQTY').clearInvalid();
    ...
    function setCmpShowCondition_D(MMCODE, MMCODE_TEXT, LOT_NO, LOT_NO_TEXT, NEW_EXP_DATE, NEW_EXP_DATE_TEXT, NEW_APVQTY, NEW_APVQTY_TEXT, IN_PRICE, CONTPRICE) {
        var f = T2Form.getForm();
        ...
        NEW_EXP_DATE ? showComponent(f, "NEW_EXP_DATE") : hideComponent(f, "NEW_EXP_DATE");
        NEW_EXP_DATE_TEXT ? showComponent(f, "NEW_EXP_DATE_TEXT") : hideComponent(f, "NEW_EXP_DATE_TEXT");
        NEW_APVQTY ? showComponent(f, "NEW_APVQTY") : hideComponent(f, "NEW_APVQTY");
        NEW_APVQTY_TEXT ? showComponent(f, "NEW_APVQTY_TEXT") : hideComponent(f, "NEW_APVQTY_TEXT");
