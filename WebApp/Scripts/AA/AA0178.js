...
Ext.onReady(function () {
    ...
    var SetMatClassComboGet = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0178/GetMatClassSubCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AA0178/GetMMCodeCombo', //指定查詢的Controller路徑
    ...
    var uriWH_NoCombo = '/api/AA0178/GetWH_NoCombo';
    var T1QueryWH_NoCombo = Ext.create('WEBAPP.form.WH_NoCombo', {
        ...
        listeners: {
                    ...
                    success: function (response) {
                        ...
                        if (data.success) {
                            if (data.etts) {
                                ...
                                data.etts[0].WH_NO == "全院藥局" ? Ext.getCmp('export').setDisabled(true) : Ext.getCmp('export').setDisabled(false);
                            }
                        }
                    }
                });
            ...
            select: function (combo, record, eOpts) {
                record.data.WH_NO == "全院藥局" ? Ext.getCmp('export').setDisabled(true) : Ext.getCmp('export').setDisabled(false);
            }
        }
    });
    // 查詢欄位
            ...
            items: [
                ...
                T1QueryWH_NoCombo,
                {
                    ...
                    handler: function () {
                        ...
                        Search_p2 = this.up('form').getForm().findField('P2').getValue();
                        T1reset();
                        T1Load();
                        msglabel('訊息區:');
                    }
    ...
    var T1Store = Ext.create('WEBAPP.store.AA.AA0178', {
        listeners: {
                ...
                handler: function () {
                    ...
                    Ext.MessageBox.confirm('匯出', '是否確定匯出？', function (btn, text) {
                        if (btn === 'yes') {
                            ...
                            PostForm('/api/AA0178/GetExcel', p);
                            msglabel('訊息區:匯出完成');
                        }
                    });
                }
    ...
    function setFormDefaultDisplay() {
        ...
        T1Form.getForm().findField("SAFE_QTY").labelEl.update('藥局安全庫存量(X' + T1LastRec.data.SAFE_PERC + 'RO)');
        T1Form.getForm().findField("NORMAL_QTY").labelEl.update('藥局正常庫存量(X' + T1LastRec.data.NORMAL_PERC + 'RO)');
        T1Form.getForm().findField("G34_MAX_APPQTY").labelEl.update('護理病房最大請領量(X' + T1LastRec.data.G34_PERC + 'RO)');
    }
    //按下修改按鈕
    ...
    function CalcRoType_1() {
        ...
        if (gap >= base) {
            T1Form.getForm().findField("NOW_RO").setValue(Math.ceil(DAY_USE_10));
        }
        else {
            T1Form.getForm().findField("NOW_RO").setValue(Math.ceil(DAY_USE_90));
        }
    ...
    function CalcRoType_2() {
        ...
        T1Form.getForm().findField("NOW_RO").setValue(Math.ceil(MON_AVG_USE_3 / 90));
        ...
        items: [
            {
                ...
                text: '範本', handler: function () {
                    var p = new Array();
                    PostForm('/api/AA0178/GetExcelExample', p);
                    msglabel('訊息區:匯出完成');
                }
            },
            {
