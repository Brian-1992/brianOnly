...
Ext.onReady(function () {
    ...
    var T1Store = Ext.create('Ext.data.Store', {
        ...
        pageSize: 30, // 每頁顯示筆數
        remoteSort: true,
        sorters: [{ property: 'APP_INID', direction: 'ASC' }, { property: 'DOCNO', direction: 'ASC' }],
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST' // by default GET
    ...
    var T1Query = Ext.widget({
        ...
        items: [{
                ...
                id: 'P3',
                items: [
                    { boxLabel: '待核可', name: 'P3', inputValue: '1', width: 75, checked: true },
                    { boxLabel: '已核可', name: 'P3', inputValue: '2', width: 75, checked: false },
                    { boxLabel: '刪除', name: 'P3', inputValue: '3', width: 75, checked: false },
                ],
                getValue: function () {
                    var val = [];
    ...
    var T1Tool = Ext.create('Ext.PagingToolbar', {
        ...
        buttons: [
            {
                itemId: 'apply', text: '核可確認', disabled: true, handler: function () {
                    ...
                    if (selection.length === 0) {
                        ...
                        Ext.MessageBox.confirm('核可確認', '補藥單：' + name + '，是否確定核可？', function (btn, text) {
                            if (btn === 'yes') {
                                Ext.Ajax.request({
                                    url: '/api/AA0180/Apply',
                                    method: reqVal_p,
                                    params: {
                                        DOCNO: docno
                                    },
                ...
                itemId: 'cancel', text: '取消通知單', disabled: true, handler: function () {
                    var selection = T1Grid.getSelection();
                    if (selection.length === 0) {
                        Ext.Msg.alert('提醒', '請勾選項目');
                    }
                        ...
                        Ext.MessageBox.confirm('取消通知單', '補藥單：' + name + '，是否確定取消？', function (btn, text) {
                            if (btn === 'yes') {
                                Ext.Ajax.request({
                                    url: '/api/AA0180/Cancel',
                                    method: reqVal_p,
                                    params: {
                                        DOCNO: docno
                                    },
    ...
    function setFormT1a() {
        T1Grid.down('#apply').setDisabled(T1Rec.length === 0);
        T1Grid.down('#cancel').setDisabled(T1Rec.length === 0);
        if (T1LastRec) {
            T1F1 = T1LastRec.data["DOCNO"];

            // 勾選項目所有資料之 IS_DEL = 'S' 且 APVTIME 空白 才可打開 apply、cancel
            var isDelCheck_S = 0;
            var apvtimeCheck = 0;
            for (var i = 0; i < T1Rec.length; i++) {
                ...
                if (T1Rec[i].data["APVTIME"] !== null) {
                    apvtimeCheck++;
                }
            }

            if (isDelCheck_S == 0 && apvtimeCheck == 0) {
                T1Grid.down('#apply').setDisabled(false);
                T1Grid.down('#cancel').setDisabled(false);
                T2Grid.down('#btnT1edit').setDisabled(true);
            }
            else {
    ...
    function setFormT2a() {
        if (T2LastRec) {
            ...
            // 勾選項目之master資料其 IS_DEL = 'S' 且 APVTIME 空白 才可打開 btnT1edit
            if (T1LastRec.data["IS_DEL"] == 'S' && T1LastRec.data["APVTIME"] == null) {
