...
Ext.onReady(function () {
    // var T1Get = '/api/AA0037/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "單位申請單新增修改";
    ...
    var st_pkdocno = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: '/api/AA0037/GetDocnopkCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            },
    ...
    var st_pknote = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: '/api/AA0037/GetDocpknoteCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            },
    ...
    var st_apply_kind = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0037/GetApplyKindCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0037/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_storeid = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0037/GetStoreidCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_towhcombo = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            beforeload: function (store, options) {
                // 載入前將查詢條件P0值代入參數
                var np = {
                    p0: T1Form.getForm().findField('APPDEPT').getValue()
                };
                Ext.apply(store.proxy.extraParams, np);
            },
            load: function (store, records, successful, eOpts) {
                //T1Form.findField('TOWH').setValue(records[0].data.VALUE);
            }
        }
        ,
        autoLoad: true
    ...
    var st_docmappdeptcombo = Ext.create('Ext.data.Store', {
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST' // by default GET
            },
            url: '/api/AA0037/GetDocmAppDeptCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_appdeptcombo = Ext.create('Ext.data.Store', {
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST' // by default GET
            },
            url: '/api/AA0037/GetAppDeptCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var st_doctype = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0037/GetDoctypeCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T1QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AA0037/GetMMCodeQCombo', //指定查詢的Controller路徑
        ...
        buttons: [
            {
                itemId: 'add', text: '新增', handler: function () {
                    T1Set = '/api/AA0037/CreateM'; // AA0037Controller的Create
                    msglabel('訊息區:');
                    setFormT1('I', '新增');
    ...
    function setFormT1(x, t) {
        ...
        if (x === "I") {
            ...
            f.findField('FRWH').setValue('560000');
            ...
            f.findField('M_STOREID_N').setReadOnly(false);
            f.findField('M_STOREID_N').setValue(st_storeid.getAt(0).get('VALUE'));
        }
        else {
            u = f.findField('DOCNO');
        }
    ...
    var T1Grid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            text: "送核撥人員",
            dataIndex: 'SENDAPV_NAME',
            width: 140
        }, {
                text: "送核撥時間",
            dataIndex: 'SENDAPVTIME_T',
            width: 140
        }, {
            header: "",
            flex: 1
    ...
    function setFormT1a() {
        ...
        if (T1LastRec) {
            ...
            T1F8 = f.findField('M_STOREID_N').getValue();
    ...
    var T1Form = Ext.widget({
        ...
        items: [{
            ...
            listeners: {
                "select": function (c, r, i, e) {
                    var f = T1Form.getForm();
                    f.findField('TOWH').setValue(r.get('EXTRA1'));
                    f.findField('TOWH_N').setValue(r.get('EXTRA2'));
                    //st_towhcombo.load();
                }, boxready: function () {
                    //this.store.load();
                }
    ...
    var T2QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        labelAlign: 'right',
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0037/GetMMCodeDocd', //指定查詢的Controller路徑
        extraFields: ['MMNAME_C', 'MMNAME_E', 'BASE_UNIT'], //查詢完會回傳的欄位
        getDefaultParams: function () { //查詢時Controller固定會收到的參數
            return {
                p1: T1Form.getForm().findField('DOCNO').getValue()
    ...
    var T2FormMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        width: 220,
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0037/GetMMCodeCombo', //指定查詢的Controller路徑
        extraFields: ['MMNAME_C', 'MMNAME_E', 'BASE_UNIT', 'M_CONTPRICE', 'AVG_PRICE', 'INV_QTY', 'AVG_APLQTY', 'HIGH_QTY', 'TOT_APVQTY', 'TOT_DISTUN'], //查詢完會回傳的欄位
        getDefaultParams: function () { //查詢時Controller固定會收到的參數
            return {
                ...
                p5: T1Form.getForm().findField('M_STOREID_N').getValue()
            };
        },
        listeners: {
            select: function (c, r, i, e) {
                    ...
                    T2FormMMCode,
                    {
                        ...
                        handler: function () {
                            ...
                            if (!f.findField("MMCODE").readOnly) {
                                popMmcodeForm(viewport, '/api/AA0037/GetMmcode', {
            ...
            itemId: 'T2Submit', text: '儲存', hidden: true, handler: function () {
                ...
                if (this.up('form').getForm().findField('APPQTY').getValue() == '0') {
                    ...
                    if (this.up('form').getForm().findField('HIGH_QTY').getValue() != null) {
                        ...
                        var minodrqty = Number(this.up('form').getForm().findField('TOT_DISTUN').getValue());
                        if ((tot_apvqty + appqty) > highqty && this.up('form').getForm().findField('GTAPL_RESON').getValue() == null) {
                            ...
                            if ((appqty % minodrqty) != 0) {
                                Ext.Msg.alert('提醒', '申請量必須為撥發包裝率的倍數!');
                                isSub = false;
                            }
                            else {
                                isSub = true;
    ...
    var T2Tool = Ext.create('Ext.PagingToolbar', {
        ...
        border: false,
        plain: true,
        //#region 2020-06-16 按鈕隱藏 家任大哥說不要拿掉
        //buttons: [
        //    {
