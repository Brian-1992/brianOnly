...
Ext.onReady(function () {
    var T1Name = '藥品庫存現況查詢';
    var T1GetExcel = '/api/AA0137/Excel';
    ...
    var QMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AA0137/GetMmcodeCombo'
        //extraFields: ['MMNAME_C', 'MMNAME_E', 'BASE_UNIT'], //查詢完會回傳的欄位
    });

    // 管制用藥選項
    var REST_Query = Ext.create('Ext.data.Store', {
        fields: ['VALUE', 'COMBITEM'],
        data: [
            { 'VALUE': '', 'COMBITEM': '' },
            { 'VALUE': '0', 'COMBITEM': '1~3級管制用藥' },
            { 'VALUE': '1', 'COMBITEM': '第四級管制用藥' },
            { 'VALUE': '2', 'COMBITEM': 'N非管制用藥' },
            { 'VALUE': '3', 'COMBITEM': '0其它列管藥品' }
        ]
    });
        ...
        proxy: {
            ...
            beforeload: function (store, options) {
                var np = {
                    ...
                    p1: T1Query.getForm().findField('MMCODE').getValue(),
        ...
        buttons: [
            {
                ...
                handler: function () {
                    var p = new Array();
                    p.push({ name: 'FN', value: '藥品庫存現況.xls' });
                    p.push({ name: 'p0', value: T1Query.getForm().findField('P0').getValue() });
                    p.push({ name: 'p1', value: T1Query.getForm().findField('MMCODE').getValue() });
