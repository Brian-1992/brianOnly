...
Ext.onReady(function () {
    // var T1Get = '/api/AA0024/All'; // 查詢(改為於store定義)
    var T1Set = ''; // 新增/修改/刪除
    var T1Name = "換貨維護";
    ...
    var st_matclass = Ext.create('Ext.data.Store', {
        proxy: {
            ...
            url: '/api/AA0024/GetMatclassCombo',
            reader: {
                type: 'json',
                rootProperty: 'etts'
            }
    ...
    var T2QueryMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        queryUrl: '/api/AA0024/GetMMCodeDocd', //指定查詢的Controller路徑
    ...
    var T2FormMMCode = Ext.create('WEBAPP.form.MMCodeCombo', {
        ...
        width: 220,
        limit: 10, //限制一次最多顯示10筆
        queryUrl: '/api/AA0024/GetMMCodeCombo', //指定查詢的Controller路徑
        extraFields: ['MMNAME_C', 'MMNAME_E', 'BASE_UNIT', 'INV_QTY', 'M_CONTPRICE', 'WEXP_ID'], //查詢完會回傳的欄位
        getDefaultParams: function () { //查詢時Controller固定會收到的參數
            return {
                p1: T1Form.getForm().findField('MAT_CLASS').getValue(),
                p2: T1Form.getForm().findField('AGEN_NO').getValue()
            };
        },
        listeners: {
            select: function (c, r, i, e) {
                ...
                T2Form.getForm().findField('WEXP_ID').setValue(r.get('WEXP_ID'));
                st_lotno.load();

                if (r.get('WEXP_ID') == "N") {
                    T2Form.getForm().findField('LOT_NO').setReadOnly(false);
                    T2Form.getForm().findField('LOT_NO').setValue('--');
                    T2Form.getForm().findField('EXP_DATE_T').setValue('1880101');
                }
                else {
                    T2Form.getForm().findField('LOT_NO').setReadOnly(false);
                }
    ...
    var T2Form = Ext.widget({
        ...
        items: [{
            ...
            listeners: {
                select: function (c, r, i, e) {
                    var f = T2Form.getForm();
                    //f.findField('INV_QTY').setValue('0');
                }
            }
        },
        {
                ...
                T2FormMMCode,
                {
                    ...
                    handler: function () {
                        ...
                        if (!f.findField("MMCODE").readOnly) {
                            popMmcodeForm(viewport, '/api/AA0024/GetMmcode', {
            ...
            itemId: 'T2Submit', text: '儲存', hidden: true, handler: function () {
                if ((this.up('form').getForm().findField('LOT_NO').getValue() == '' || this.up('form').getForm().findField('LOT_NO').getValue() == null)
                    ...
                    if (this.up('form').getForm().findField('APVQTY').getValue() == '0')
                        ...
                        if (this.up('form').getForm().findField('INOUT').getValue() == '2' && apvqty > invqty) {
        ...
        buttons: [
            {
                itemId: 'add', text: '新增', disabled: true, handler: function () {
                    T11Set = '../../../api/AA0024/CreateD';
                    setFormT2('I', '新增');
                }
            },
            {
                itemId: 'edit', text: '修改', disabled: true, handler: function () {
                    T11Set = '../../../api/AA0024/UpdateD';
                    setFormT2("U", '修改');
                }
            },
            {
    ...
    function setFormT2a() {
        ...
        if (T2LastRec) {
            ...
            //u.setReadOnly(true);
            //u.setFieldStyle('border: 0px');
            T2F1 = f.findField('INOUT').getValue();
            if (T1F3 === '1') {
